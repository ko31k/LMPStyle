...
(function() {
    ...
    var styleTracks = "<style id=\"lampa_tracks_styles\">" +
        ...
        " z-index: 10 !important;" + // z-index трохи вищий за рейтинг (у якого 1), щоб мітка була "поверх".
        "}" +

        // Додатковий клас, який застосовується динамічно, якщо плагін RatingUp активний.
        ".card__tracks.positioned-below-rating {" +
        // ЗМІНЕНО: Відступ зменшено для більш щільного розташування.
        " top: 2.15em !important; " + // Нова позиція зверху (стара була 2.3em).
        "}" +
        
        // Стиль для тексту всередині мітки.
        ".card__tracks div {" +
        // ЗМІНЕНО: Прибрано 'text-transform: uppercase', щоб текст був 'Ukr', а не 'UKR'.
    ...
    function getBestReleaseWithUkr(normalizedCard, cardId, callback) {
        enqueueTask(function(done) {
            // ПІДТВЕРДЖЕНО: Цей блок коду коректно обробляє майбутні релізи.
            // Він перевіряє, чи дата виходу не в майбутньому.
            ...
            if (releaseDate && releaseDate.getTime() > Date.now()) {
                if (LTF_CONFIG.LOGGING_TRACKS) console.log(`LTF-LOG [${cardId}]: Майбутній реліз, пошук скасовано.`);
                callback(null);
                done();
                return;
            }

            // ПІДТВЕРДЖЕНО: Цей блок коду коректно обробляє релізи з невідомою датою.
            // Якщо release_date буде "Невідомо" або відсутнє, `parseInt` дасть NaN, і пошук буде скасовано.
            var year = '';
            if (normalizedCard.release_date && normalizedCard.release_date.length >= 4) {
                year = normalizedCard.release_date.substring(0, 4);
            }
            if (!year || isNaN(parseInt(year, 10))) {
                 if (LTF_CONFIG.LOGGING_TRACKS) console.log(`LTF-LOG [${cardId}]: Некоректний рік ('${year}'), пошук скасовано.`);
                callback(null);
                done();
                return;
            }
    ...
    function updateCardListTracksElement(cardView, trackCount) {
        ...
        // --- ЛОГІКА ДИНАМІЧНОГО ПОЗИЦІОНУВАННЯ ---
        ...
        if (parentCard) {
            ...
            if (voteElement && getComputedStyle(voteElement).top !== 'auto' && parseInt(getComputedStyle(voteElement).top) < 100) {
                ...
                if (LTF_CONFIG.LOGGING_CARDLIST) console.log("LTF-LOG: RatingUp знайдено, мітка зміщена вниз.");
            }
        }
