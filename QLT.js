(function() {
    ...
    // ===== КОНФІГУРАЦІЯ ПЛАГІНА LAMPA QUALITY ENHANCER =====
    var LQE_CONFIG = {
        ...
        LOGGING_CARDLIST: false, // Логування для карток у списках
        ...
        CACHE_KEY: 'lampa_quality_cache', // Ключ для зберігання кешу
        JACRED_PROTOCOL: 'http://', // Протокол для JacRed API
        ...
        PROXY_TIMEOUT_MS: 5000, // Таймаут для проксі-запитів
        SHOW_QUALITY_FOR_TV_SERIES: true, // Показувати якість для серіалів
        
        // Стилі для лейбла якості на повній картці фільму
        ...
        // Стилі для лейбла якості на картках у списку
        ...
        // Ручні перевизначення якості для конкретних ID фільмів/серіалів
        MANUAL_OVERRIDES: {
        ...
        // Адаптивні селектори DOM для кращої сумісності з різними версіями Lampa
        SELECTORS: {
            RATE_LINE: '.full-start-new__rate-line, .maxsm-rate-line, .rating-line, .info-line',
            CARD_VIEW: '.card__view, .card-view, .movie-card',
            QUALITY_LABEL: '.lqe-quality-pp',
            CARD_QUALITY: '.lqe-card-quality'
        }
    };

    var currentGlobalMovieId = null; // Поточний ID фільму для глобального доступу

    // ===== ВІДПОВІДНІСТЬ НАЗВ ЯКОСТІ (ТОРРЕНТ → КОРИСТУВАЧ) =====
    ...
    // ===== ПРОРІОРИТЕТИ СКЛАДНИХ ЯКОСТЕЙ ПРИ ФОРМУВАННІ ЛЕЙБЛА =====
    var QUALITY_PRIORITY_ORDER = [
        'resolution', // Роздільна здатність (найважливіша)
        'source',     // Джерело (BluRay, WEB-DL, тощо)
    ];

    // ===== CSS СТИЛІ З АДАПТИВНИМИ СЕЛЕКТОРАМИ ТА !important =====
    var styleLQE = "<style id=\"lampa_quality_styles\">" +
        ".full-start-new__rate-line, .maxsm-rate-line, .rating-line, .info-line {" +
        ...
        "}" +
        ".card__view, .card-view, .movie-card {" +
    ...
    // ===== CSS СТИЛІ ДЛЯ АНІМАЦІЇ ЗАВАНТАЖЕННЯ (ТРИ КРАПКИ) =====
    ...
    // ===== ФУНКЦІЯ ІНІЦІАЛІЗАЦІЇ АДАПТИВНИХ СЕЛЕКТОРІВ DOM =====
    function initializeSelectors() {
        if (LQE_CONFIG.LOGGING_GENERAL) console.log("LQE-LOG", "Initializing adaptive selectors");
        
        // Додаємо CSS стилі до DOM
        ...
        // Адаптуємо селектори на основі доступних елементів DOM
        if ($('.maxsm-rate-line').length > 0) {
            LQE_CONFIG.SELECTORS.RATE_LINE = '.maxsm-rate-line';
        }
        if ($('.maxsm-modal-ratings').length > 0) {
            LQE_CONFIG.SELECTORS.RATE_LINE = '.maxsm-modal-ratings';
        }
        if ($('.card-view').length > 0) {
            LQE_CONFIG.SELECTORS.CARD_VIEW = '.card-view';
        }
        if ($('.movie-card').length > 0) {
            LQE_CONFIG.SELECTORS.CARD_VIEW = '.movie-card';
        }
        
        if (LQE_CONFIG.LOGGING_GENERAL) console.log("LQE-LOG", "Final selectors:", LQE_CONFIG.SELECTORS);
    }

    // ===== ФУНКЦІЇ ДЛЯ РОБОТИ З ПРОКСІ-СЕРВЕРАМИ =====
    ...
    // ===== ФУНКЦІЇ ДЛЯ АНІМАЦІЇ ЗАВАНТАЖЕННЯ =====
    function addLoadingAnimation(cardId, renderElement) {
        ...
        var rateLine = $(LQE_CONFIG.SELECTORS.RATE_LINE, renderElement).first();
    ...
    function translateQualityLabel(qualityCode, fullTorrentTitle) {
        ...
        // Шукаємо точні відповідності в повній назві торрента
        ...
        // Аналізуємо компоненти якості з назви торрента
        let extracted = {
            resolution: '',
            source: '',
            hdr: '',
        ...
        // Визначаємо джерело (BluRay, WEB-DL, тощо)
        ...
        // Визначаємо профіль (для Dolby Vision)
        ...
        // Складаємо фінальний лейбл згідно з пріоритетами
        let assembledParts = [];
        ...
        // Резервні варіанти, якщо не вдалося скласти лейбл
    ...
    // ===== ФУНКЦІЇ ДЛЯ РОБОТИ З UI ЕЛЕМЕНТАМИ =====
    function clearFullCardQualityElements(cardId, renderElement) {
        if (renderElement) {
            var existingElements = $(LQE_CONFIG.SELECTORS.QUALITY_LABEL, renderElement);
    ...
    function updateFullCardQualityElement(qualityCode, fullTorrentTitle, cardId, renderElement, bypassTranslation = false) {
        ...
        var element = $(LQE_CONFIG.SELECTORS.QUALITY_LABEL, rateLine);
        ...
        if (element.length) {
            ...
            div.className = 'full-start__status lqe-quality-pp';
    ...
    function updateCardListQualityElement(cardView, qualityCode, fullTorrentTitle, bypassTranslation = false) {
        ...
        var existingQualityElements = cardView.getElementsByClassName('lqe-card-quality');
        Array.from(existingQualityElements).forEach(el => el.parentNode.removeChild(el));

        var qualityDiv = document.createElement('div');
        qualityDiv.className = 'lqe-card-quality';
    ...
    // ===== ОСНОВНІ ФУНКЦІЇ ОБРОБКИ ЯКОСТІ =====
    function processFullCardQuality(cardData, renderElement) {
        ...
        // Перевірка ручних налаштувань (найвищий пріоритет)
        ...
        if (!(isTvSeries && LQE_CONFIG.SHOW_QUALITY_FOR_TV_SERIES === false)) {
            ...
            if (cachedQualityData) {
                ...
                // Фонове оновлення кешу, якщо дані застаріли
    ...
    function updateCardListQuality(cardElement) {
        ...
        var cardView = cardElement.querySelector(LQE_CONFIG.SELECTORS.CARD_VIEW);
        ...
        // Перевірка ручних налаштувань для карток у списку
        var manualOverrideData = LQE_CONFIG.MANUAL_OVERRIDES[cardId];
        if (manualOverrideData) {
        ...
        if (cachedQualityData) {
            ...
            // Фонове оновлення кешу для спискових карток
            if (Date.now() - cachedQualityData.timestamp > LQE_CONFIG.CACHE_REFRESH_THRESHOLD_MS) {
        ...
        // Отримання даних з JacRed API для спискових карток
        getBestReleaseFromJacred(normalizedCard, cardId, function(jrResult) {
    ...
    // ===== СПОСТЕРІГАЧ ЗА DOM ДЛЯ НОВИХ КАРТОК =====
    ...
    // ===== ІНІЦІАЛІЗАЦІЯ ПЛАГІНА LAMPA QUALITY ENHANCER =====
    function initializeLampaQualityPlugin() {
        ...
        // Ініціалізація адаптивних селекторів DOM
        initializeSelectors();
        
        window.lampaQualityPlugin = true;
        
        // Запуск спостерігача за DOM для нових карток
        ...
        // Підписка на події відкриття повної картки
    ...
    // ===== ЗАПУСК ПЛАГІНА (ЯКЩО ЩЕ НЕ ЗАПУЩЕНИЙ) =====
