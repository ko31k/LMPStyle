...
(function () {
    ...
    var LQE_CONFIG = {
        ...
        FULL_BADGES_POSITION: 'after_details', // 'after_details' (Режим A) або 'in_rate_line' (Режим B)
...
// ===================== FULL CARD SVG BADGES (Parser-based) =====================

// RAW GitHub base (імена з %20)
var LQE_SVG_BASE = 'https://raw.githubusercontent.com/ko31k/LMP/main/wwwroot/img/';

var LQE_SVG_ICONS = {
  '4K': LQE_SVG_BASE + '4K.svg',
  '2K': LQE_SVG_BASE + '2K.svg',
  'FULL HD': LQE_SVG_BASE + 'FULL%20HD.svg',
  'HD': LQE_SVG_BASE + 'HD.svg',
  'HDR': LQE_SVG_BASE + 'HDR.svg',
  'Dolby Vision': LQE_SVG_BASE + 'Dolby%20Vision.svg',
  '7.1': LQE_SVG_BASE + '7.1.svg',
  '5.1': LQE_SVG_BASE + '5.1.svg',
  '4.0': LQE_SVG_BASE + '4.0.svg',
  '2.0': LQE_SVG_BASE + '2.0.svg',
  'DUB': LQE_SVG_BASE + 'DUB.svg',
  'UKR': LQE_SVG_BASE + 'UKR.svg'
};

// окремий кеш лише для FULL->Parser->SVG, щоб не мішати з JacRed кешем
...
function lqeFullSvgCacheGet(key) {
  var m = LQE_fullSvgMemoryCache[key];
  if (m && (Date.now() - m.ts < LQE_CONFIG.CACHE_VALID_TIME_MS)) return m;

  var sc = lqeGetFullSvgStorageCache();
  var it = sc[key];
  if (it && it.ts && (Date.now() - it.ts < LQE_CONFIG.CACHE_VALID_TIME_MS)) {
    LQE_fullSvgMemoryCache[key] = it;
    return it;
  }
  return null;
}

function lqeFullSvgCacheSet(key, bestObj) {
  var payload = { best: bestObj, ts: Date.now() };
  var sc = lqeGetFullSvgStorageCache();
  sc[key] = payload;
  LQE_fullSvgMemoryCache[key] = payload;
  Lampa.Storage.set(LQE_FULLSVG_CACHE_KEY, sc);
}

function lqeFullSvgCacheCleanup() {
  var sc = lqeGetFullSvgStorageCache();
  var now = Date.now();
  var changed = false;
  for (var k in sc) {
    ...
    var it = sc[k];
    if (!it || !it.ts || (now - it.ts) > LQE_CONFIG.CACHE_VALID_TIME_MS) {
      delete sc[k];
      changed = true;
    }
  }
  if (changed) Lampa.Storage.set(LQE_FULLSVG_CACHE_KEY, sc);
}

// ------------- UI / styles -------------
function lqeEnsureFullSvgStylesOnce() {
  if (document.getElementById('lqe_full_svg_badges_css')) return;

  var css = ''
    + '<style id="lqe_full_svg_badges_css">'
    + '.lqe-quality-badges-container{display:flex;gap:.3em;margin:0 0 .4em 0;min-height:1.2em;pointer-events:none;}'
    + '.lqe-quality-badge{height:1.2em;opacity:0;transform:translateY(8px);animation:lqe_qb_in .4s ease forwards;}'
    + '@keyframes lqe_qb_in{to{opacity:1;transform:translateY(0);}}'
    + '.lqe-quality-badge img{height:100%;width:auto;display:block;}'
    + '@media (max-width:768px){.lqe-quality-badges-container{gap:.25em;margin:0 0 .35em 0;min-height:1em;}'
    + '.lqe-quality-badge{height:1em;}}'
    + '</style>';

  $('body').append(css);
}

function lqeCreateBadgeImg(type, index) {
  var iconPath = LQE_SVG_ICONS[type];
  if (!iconPath) return '';
  var delay = (index * 0.08) + 's';
  return '<div class="lqe-quality-badge" style="animation-delay:' + delay + '">'
    + '<img src="' + iconPath + '" draggable="false" oncontextmenu="return false;">'
    + '</div>';
}

function lqeClearFullSvgBadges(renderElement) {
  if (!renderElement) return;
  try { $('.lqe-quality-badges-container', renderElement).remove(); } catch (e) {}
}

function lqeEnsureFullSvgContainer(renderElement) {
  if (!renderElement) return null;

  var $render = $(renderElement);

  // якщо контейнер уже є — просто повертаємо
  var $c = $render.find('.lqe-quality-badges-container').first();
  if ($c.length) return $c;

  // Позиції:
  // A) after_details — як SVG-плагін
  // B) in_rate_line — як "в тебе було"
  var pos = (window.LQE_CONFIG && LQE_CONFIG.FULL_BADGES_POSITION) ? LQE_CONFIG.FULL_BADGES_POSITION : 'after_details';

  if (pos === 'in_rate_line') {
    var $rl = $render.find('.full-start-new__rate-line, .full-start__rate-line').first();
    if ($rl.length) {
      $rl.append('<div class="lqe-quality-badges-container"></div>');
      return $render.find('.lqe-quality-badges-container').first();
    }
  }
  ...
  if ($details.length) {
    $details.after('<div class="lqe-quality-badges-container"></div>');
    return $render.find('.lqe-quality-badges-container').first();
  }
...
function lqeRenderFullSvgBadges(renderElement, best) {
  lqeEnsureFullSvgStylesOnce();

  var $c = lqeEnsureFullSvgContainer(renderElement);
  if (!$c || !$c.length) return;

  var badges = [];
  if (best && best.ukr) badges.push(lqeCreateBadgeImg('UKR', badges.length));
  if (best && best.resolution) badges.push(lqeCreateBadgeImg(best.resolution, badges.length));
  if (best && best.hdr) badges.push(lqeCreateBadgeImg('HDR', badges.length));
  if (best && best.audio) badges.push(lqeCreateBadgeImg(best.audio, badges.length));
  if (best && best.dub) badges.push(lqeCreateBadgeImg('DUB', badges.length));
  if (best && best.dolbyVision) badges.push(lqeCreateBadgeImg('Dolby Vision', badges.length));

  $c.html(badges.join(''));
}

// ------------- UKR token detector (без випадкових "ua" як підрядок) -------------
function lqeHasUkrAudioToken(s) {
  var t = (s || '').toLowerCase();

  // ukr/ukrainian
  if (/\b(ukr|ukrainian)\b/.test(t)) return true;

  // укр як слово
  if (/\bукр\b/.test(t)) return true;

  // ua лише як токен (роздільники/пробіли)
  if (/(^|[\s\.\-\_\[\(\]])ua([\s\.\-\_\]\)\,]|$)/.test(t)) return true;

  // часті варіанти
  if (/\b2x\s*ukr\b|\bukr\s*audio\b|\bukr\s*dub\b/.test(t)) return true;

  return false;
}

// ------------- SAFE HD detection (не ловить HDR/UHD) -------------
function lqeHasHdToken(s) {
  var t = (s || '').toLowerCase();

  // "hd" як окремий токен, але не частина hdr/uhd
  // приклади: " 720p ", " hd ", "[HD]"
  if (/(^|[^a-z0-9])hd([^a-z0-9]|$)/i.test(t)) {
    // відсікаємо uhd/hdr поруч
    if (/uhd/.test(t)) return false;
    if (/hdr/.test(t)) return false;
    return true;
  }
  return false;
}

// ------------- getBest for Parser Results (з покращеннями) -------------
function lqeGetBestFromParserResults(results) {
  ...
  for (var i = 0; i < limit; i++) {
    var item = results[i] || {};
    var title = String(item.Title || item.title || '').toLowerCase();

    // UKR — тільки токени
    if (lqeHasUkrAudioToken(title)) best.ukr = true;

    // Resolution by title (safe rules)
    var foundRes = null;
    if (/(^|[^a-z0-9])(4k|2160|uhd)([^a-z0-9]|$)/.test(title)) foundRes = '4K';
    else if (/(^|[^a-z0-9])(2k|1440)([^a-z0-9]|$)/.test(title)) foundRes = '2K';
    else if (/(^|[^a-z0-9])(1080|fhd|full\s*hd)([^a-z0-9]|$)/.test(title)) foundRes = 'FULL HD';
    else if (/(^|[^a-z0-9])720([^a-z0-9]|$)/.test(title) || lqeHasHdToken(title)) foundRes = 'HD';
    ...
    // DV/HDR/DUB by title (додатково)
    if (title.indexOf('vision') >= 0 || title.indexOf('dovi') >= 0) best.dolbyVision = true;
    if (/(^|[^a-z0-9])hdr([^a-z0-9]|$)/.test(title)) best.hdr = true;
...
// ------------- Full card fetch via Parser (кеш + inflight + queue + loader) -------------
function lqeFetchFullSvgBadgesViaParser(normalizedCard, cardId, renderElement) {
  // якщо вимкнено — прибираємо контейнер і виходимо
  if (window.LQE_CONFIG && LQE_CONFIG.SHOW_FULL_CARD_LABEL === false) {
    lqeClearFullSvgBadges(renderElement);
    return;
  }

  // потрібні Lampa.Parser + parser_use
  if (!Lampa || !Lampa.Parser || !Lampa.Storage || !Lampa.Storage.field || !Lampa.Storage.field('parser_use')) {
    lqeClearFullSvgBadges(renderElement);
    return;
  }

  var cacheKey = 'fullsvg_' + LQE_CONFIG.CACHE_VERSION + '_' + cardId;

  // manual override: показуємо хоча б резолюцію (інше Parser не дасть гарантовано)
  var manual = LQE_CONFIG.MANUAL_OVERRIDES[cardId];
  if (manual) {
    var res = null;
    var sl = String(manual.simple_label || manual.full_label || '').toUpperCase();
    if (sl.indexOf('4K') >= 0) res = '4K';
    else if (sl.indexOf('QHD') >= 0 || sl.indexOf('2K') >= 0) res = '2K';
    else if (sl.indexOf('FHD') >= 0 || sl.indexOf('1080') >= 0) res = 'FULL HD';
    else if (sl.indexOf('HD') >= 0 || sl.indexOf('720') >= 0) res = 'HD';

    lqeRenderFullSvgBadges(renderElement, { resolution: res, hdr: false, dolbyVision: false, audio: null, dub: false, ukr: false });
    return;
  }

  // cache draw
  var cached = lqeFullSvgCacheGet(cacheKey);
  if (cached && cached.best) {
    lqeRenderFullSvgBadges(renderElement, cached.best);
    ...
    if ((Date.now() - cached.ts) > LQE_CONFIG.CACHE_REFRESH_THRESHOLD_MS) {
      ...
      enqueueTask(function (done) {
        Lampa.Parser.get(
          { search: normalizedCard.title || normalizedCard.original_title, movie: normalizedCard, page: 1 },
          function (resp) {
            clearTimeout(kill);
            try {
              if (resp && resp.Results && resp.Results.length) {
                var best = lqeGetBestFromParserResults(resp.Results);
                lqeFullSvgCacheSet(cacheKey, best);
...
function lqeSchedulePendingRetry(cardId) {
    ...
    function processFullCardQuality(cardData, renderElement) {
        ...
        if (window.LQE_CONFIG && LQE_CONFIG.SHOW_FULL_CARD_LABEL === false) {
          // прибираємо і текст (якщо колись залишився) і svg
          // manual override -> SVG (резолюція з simple/full)
        ...
        lqeFetchFullSvgBadgesViaParser(normalizedCard, cardId, renderElement);
        removeLoadingAnimation(cardId, renderElement); // (лоадер SVG-функція сама покаже за потреби)
        return;
    ...
    (function () {
        ...
        function load() {
            ...
            return {
                ...
                full_badges_pos: s.full_badges_pos || 'after_details'
            };
        }
        ...
        function apply() {
            ...
            LQE_CONFIG.FULL_BADGES_POSITION = st.full_badges_pos || 'after_details';
        }
        ...
        function registerUI() {
            ...
            // 4b) Позиція іконок у повній картці
            Lampa.SettingsApi.addParam({
              component: 'lqe',
              param: {
              name: 'lqe_full_badges_pos',
              type: 'select',
              values: {
                after_details: 'Режим A: окремий рядок (як SVG)',
                in_rate_line: 'Режим B: в рядку рейтингів (як було)'
              },
              default: st.full_badges_pos || 'after_details'
              },
              field: { name: 'Позиція іконок у повній картці' },
              onChange: function (v) { st.full_badges_pos = v; save(); }
            });
