(function() {
    ...
    var LQE_CONFIG = {
        ...
        CACHE_VALID_TIME_MS: 3 * 24 * 60 * 60 * 1000, // 3 дні
        ...
        LIST_CARD_LABEL_FONT_SIZE: '1.4em',
    ...
    // ==================== ПРІОРИТЕТИ СКЛАДАННЯ ЯКОСТІ ====================
    var QUALITY_PRIORITY_ORDER = [
        'resolution',
        'source',
    ];

    // ==================== CSS СТИЛІ ДЛЯ ЯРЛИКІВ ЯКОСТІ ====================
    var styleLQE = "<style id=\"lqe-quality-styles\">" +
        ".maxsm-rate-line {" +
    ...
    Lampa.Template.add('lqe_quality_css', styleLQE);
    $('body').append(Lampa.Template.get('lqe_quality_css', {}, true));

    // ==================== CSS СТИЛІ ДЛЯ АНІМАЦІЇ ЗАВАНТАЖЕННЯ ====================
    var loadingStylesLQE = "<style id=\"lqe-quality-loading-animation\">" +
        ".lqe-loading-dots-container {" +
        "    position: absolute !important;" +
        "    top: 50% !important;" +
        "    left: 0 !important;" +
        "    right: 0 !important;" +
        ...
        "}" +
        ".lqe-loading-dots__text {" +
        "    margin-right: 1em !important;" +
        "}" +
        ".lqe-loading-dots__dot {" +
        "    width: 0.5em !important;" +
        "    height: 0.5em !important;" +
        "    border-radius: 50% !important;" +
        "    background-color: currentColor !important;" +
        "    opacity: 0.3 !important;" +
        "    animation: lqe-loading-dots-fade 1.5s infinite both !important;" +
        "}" +
        ".lqe-loading-dots__dot:nth-child(1) {" +
        "    animation-delay: 0s !important;" +
        "}" +
        ".lqe-loading-dots__dot:nth-child(2) {" +
        "    animation-delay: 0.5s !important;" +
        "}" +
        ".lqe-loading-dots__dot:nth-child(3) {" +
        ...
        "}" +
        "@media screen and (max-width: 480px) { .lqe-loading-dots-container { -webkit-justify-content: center !important; justify-content: center !important; text-align: center !important; max-width: 100% !important; }}" +
        "</style>";

    Lampa.Template.add('lqe_quality_loading_animation_css', loadingStylesLQE);
    $('body').append(Lampa.Template.get('lqe_quality_loading_animation_css', {}, true));

    // ==================== ФУНКЦІЇ РОБОТИ З ПРОКСІ ====================
    ...
    function addLoadingAnimation(cardId, renderElement) {
        ...
        var rateLine = $('.maxsm-rate-line', renderElement);
        ...
        $('.lqe-loading-dots-container', rateLine).css({
            'opacity': '1',
            'visibility': 'visible'
        });
    }
    ...
    // ==================== ФУНКЦІЇ ДЛЯ РОБОТИ З ПОВНОЮ КАРТКОЮ ====================
    function clearFullCardQualityElements(cardId, renderElement) {
        if (renderElement) {
            var existingElements = $('.full-start__status.lqe-quality-pp', renderElement);
    ...
    function showFullCardQualityPlaceholder(cardId, renderElement) {
        ...
        var rateLine = $('.maxsm-rate-line', renderElement);
        if (!rateLine.length) {
            if (LQE_CONFIG.LOGGING_QUALITY) console.log("LQE-QUALITY", "card: " + cardId + ", Cannot show placeholder, .maxsm-rate-line not found.");
            return;
        }
        if (!$('.full-start__status.lqe-quality-pp', rateLine).length) {
    ...
    function updateFullCardQualityElement(qualityCode, fullTorrentTitle, cardId, renderElement, bypassTranslation = false) {
        if (!renderElement) return;
        var element = $('.full-start__status.lqe-quality-pp', renderElement);
        var rateLine = $('.maxsm-rate-line', renderElement);
    ...
    function processFullCardQuality(cardData, renderElement) {
        ...
        if (rateLine.length) {
            ...
            if (LQE_CONFIG.LOGGING_GENERAL) console.log("LQE-LOG", "card: " + cardId + ", .maxsm-rate-line not found, skipping loading animation.");
        }
    ...
    // ==================== ФУНКЦІЇ ДЛЯ РОБОТИ З КАРТКАМИ У СПИСКУ ====================
