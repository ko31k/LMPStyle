...
"</style>";
    ...
    var loadingStylesLQE = "<style id=\"lampa_quality_loading_animation\">" +
        ".loading-dots-container { position: absolute; top: 50%; left: 0; right: 0; " +
        "text-align: left; transform: translateY(-50%); z-index: 10; }" +
        ".full-start-new__rate-line { position: relative; }" +
        ".loading-dots { display: inline-flex; align-items: center; gap: 0.4em; " +
        ...
        ".loading-dots__dot { width: 0.5em; height: 0.5em; border-radius: 50%; " +
        ...
        ".loading-dots-container { justify-content: center; text-align: center; max-width: 100%; } }" +
        "</style>";
    ...
    function updateCardListQuality(cardElement) {
        ...
        if (cachedQualityData) {
            ...
            if (Date.now() - cachedQualityData.timestamp > LQE_CONFIG.CACHE_REFRESH_THRESHOLD_MS) {
                ...
                getBestReleaseFromJacred(normalizedCard, cardId, function(jrResult) {
                    if (jrResult && jrResult.quality && jrResult.quality !== 'NO') {
                        saveQualityCache(cakeKey, {quality_code: jrResult.quality, full_label: jrResult.full_label}, cardId);
