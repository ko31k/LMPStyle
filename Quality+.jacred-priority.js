        ...
        function searchJacredApi(searchTitle, searchYear, exactMatch, strategyName, apiCallback) {
            ...
            fetchWithProxy(apiUrl, cardId, function(error, responseText) {
                ...
                try {
                    ...
                    // prioritize torrents that include Ukrainian audio: pick best quality among them if any, otherwise best overall
                    var bestFoundTorrent = null;
                    var bestNumericQuality = -1;
                    var bestFoundTorrentTitleLength = 0;
                    var bestUkrTorrent = null;
                    var bestUkrNumericQuality = -1;
                    var bestUkrTitleLength = 0;

                    function detectUkrAudioInTitle(tt) {
                        if (!tt) return null;
                        var ttt = tt.toLowerCase();
                        var m = ttt.match(/(\d+)\s*[xх]\s*(ukr|ua)/i);
                        if (m) return m[1] + 'xUkr';
                        if (ttt.match(/(ukr|ua)[^a-zA-Z0-9]{0,3}dub/i)) return 'Dub';
                        if (ttt.match(/(ukr|ua)[^a-zA-Z0-9]{0,3}mvo/i)) return 'MVO';
                        if (ttt.match(/(ukr|ua)[^a-zA-Z0-9]{0,3}dvo/i)) return 'DVO';
                        if (ttt.match(/(ukr|ua)[^a-zA-Z0-9]{0,3}avo/i)) return 'AVO';
                        if (ttt.match(/(ukr|ua)[^a-zA-Z0-9]{0,3}svo/i)) return 'AVO';
                        if (ttt.match(/(ukr|ua)[^a-zA-Z0-9]{0,3}line/i)) return 'Line';
                        if (ttt.match(/звук\s*з\s*ts/i)) return 'TS';
                        if (ttt.match(/звук\s*с\s*ts/i)) return 'TS';
                        if (ttt.match(/(ukr|ua)[^a-zA-Z0-9]{0,3}ts/i)) return 'TS';
                        if (ttt.match(/(ukr|ua)[^a-zA-Z0-9]{0,3}mic/i)) return 'Mic';
                        if (ttt.match(/(ukr|ua)(?![a-zA-Z0-9])/i)) return 'Ukr';
                        return null;
                    }
                    ...
                    for (var i = 0; i < torrents.length; i++) {
                        ...
                        if (isYearValid && !isNaN(searchYearNum) && parsedYear !== searchYearNum) {
                            if (LQE_CONFIG.LOGGING_QUALITY) console.log("LQE-QUALITY", "card: " + cardId + ", JacRed: Skip torrent by year. Searched: " + searchYearNum + ", Found: " + parsedYear);
                            continue;
                        }

                        var titleLower = (currentTorrent.title || '').toLowerCase();
                        var audioTag = detectUkrAudioInTitle(titleLower);

                        // update overall best candidate
                        if (currentNumericQuality > bestNumericQuality) {
                            bestNumericQuality = currentNumericQuality;
                            bestFoundTorrent = currentTorrent;
                            bestFoundTorrentTitleLength = (currentTorrent.title || '').length;
                        } else if (currentNumericQuality === bestNumericQuality) {
                            var tlen = (currentTorrent.title || '').length;
                            if (tlen > bestFoundTorrentTitleLength) {
                                bestFoundTorrent = currentTorrent;
                                bestFoundTorrentTitleLength = tlen;
                            }
                        }

                        // update best among ukr-audio torrents
                        if (audioTag) {
                            if (currentNumericQuality > bestUkrNumericQuality) {
                                bestUkrNumericQuality = currentNumericQuality;
                                bestUkrTorrent = currentTorrent;
                                bestUkrTitleLength = (currentTorrent.title || '').length;
                            } else if (currentNumericQuality === bestUkrNumericQuality) {
                                var tlen2 = (currentTorrent.title || '').length;
                                if (tlen2 > bestUkrTitleLength) {
                                    bestUkrTorrent = currentTorrent;
                                    bestUkrTitleLength = tlen2;
                                }
                            }
                        }
                    }

                    // prefer bestUkrTorrent if available (highest quality among ukr-audio torrents); otherwise keep overall bestFoundTorrent
                    if (bestUkrTorrent) {
                        bestFoundTorrent = bestUkrTorrent;
                        bestNumericQuality = bestUkrNumericQuality;
                    }
if (bestFoundTorrent) {
