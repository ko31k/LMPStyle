...
function translateQualityLabel(qualityCode, fullTorrentTitle) {
    ...
    // 1) Try explicit direct matches from QUALITY_DISPLAY_MAP (prefer longer keys)
    var bestDirectMatchKey = '';
    var maxDirectMatchLength = 0;
    // treat some short tokens as simple components and skip them for direct match
    ...
    if (bestDirectMatchKey) {
        ...
        if (LQE_CONFIG.LOGGING_QUALITY) console.log("LQE-QUALITY", "translateQualityLabel: Direct map match:", bestDirectMatchKey, "=>", direct);
        return direct;
    }

    // 2) If no direct match, extract components: resolution, source, audio (ukr)
    // Define small helper maps (these complement QUALITY_DISPLAY_MAP)
    var RESOLUTION_MAP = {
  ...
  const AUDIO_MAP_UKR = [
    ...
    function findBestFromMap(map) {
        var best = '';
        // sort keys by length descending to prefer specific tokens (e.g. 'blu-ray remux' before 'blu-ray')
    ...
    // If no source found, try to detect via simple regex token like 'web', 'bluray', etc.
    if (!source) {
    ...
    // assemble according to QUALITY_PRIORITY_ORDER (resolution, source)
    var parts = [];
    ...
    if (!base && qualityCode) {
        // fallback to qualityCode or mapping
