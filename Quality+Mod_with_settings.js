...
function updateCardListQualityElement(cardView, qualityCode, fullTorrentTitle, bypassTranslation) {
    ...
    function processFullCardQuality(cardData, renderElement) {
        ...
        if (LQE_CONFIG && LQE_CONFIG.SHOW_FULL_CARD_LABEL === false) { if (LQE_CONFIG.LOGGING_GENERAL) console.log('LQE-LOG','Full card quality label disabled by setting'); return; }
...
/* ===================== LQE SETTINGS UI (Interface -> "Мітки якості") ===================== */
(function(){
    'use strict';

    var SETTINGS_KEY = 'lqe_user_settings_v1';
    var defaults = { show_tv: true, show_full_card: true, label_style: 'short' };

    function loadSettings(){
        var s = (Lampa && Lampa.Storage ? (Lampa.Storage.get(SETTINGS_KEY) || {}) : {});
        return {
            show_tv: (typeof s.show_tv === 'boolean') ? s.show_tv : defaults.show_tv,
            show_full_card: (typeof s.show_full_card === 'boolean') ? s.show_full_card : defaults.show_full_card,
            label_style: s.label_style || defaults.label_style
        };
    }
    function applySettings(s){
        try {
            if (typeof LQE_CONFIG.SHOW_FULL_CARD_LABEL === 'undefined') LQE_CONFIG.SHOW_FULL_CARD_LABEL = true;
            LQE_CONFIG.SHOW_QUALITY_FOR_TV_SERIES = !!s.show_tv;
            LQE_CONFIG.SHOW_FULL_CARD_LABEL = !!s.show_full_card;
            LQE_CONFIG.USE_SIMPLE_QUALITY_LABELS = (s.label_style === 'short');
        } catch(e){}
    }
    function saveSettings(s){
        if (Lampa && Lampa.Storage) Lampa.Storage.set(SETTINGS_KEY, s);
        applySettings(s);
        if (Lampa && typeof Lampa.Noty === 'function') Lampa.Noty('Збережено');
    }
    function clearQualityCache(){
        try {
            if (Lampa && Lampa.Storage) Lampa.Storage.set(LQE_CONFIG.CACHE_KEY, {});
            if (Lampa && typeof Lampa.Noty === 'function') Lampa.Noty('Кеш очищено');
        } catch(e){}
    }
    ...
    function addUiViaSettingsApi(){
        if (!Lampa || !Lampa.SettingsApi || typeof Lampa.SettingsApi.addCategory !== 'function') return false;
        try {
            // Create a category under Interface
            Lampa.SettingsApi.addCategory({
                group: 'interface',
                component: 'lqe_settings',
                name: 'Мітки якості',
                icon: 'tag'
            });
            // Add controls
            Lampa.SettingsApi.addBlock({
                component: 'lqe_settings',
                name: 'Керування плагіном',
                params: [
                    {
                        name: 'show_tv',
                        type: 'switch',
                        title: 'Відображати мітки якості для серіалів',
                        default: st.show_tv,
                        onChange: function(v){ st.show_tv = !!v; saveSettings(st); }
                    },
                    {
                        name: 'show_full_card',
                        type: 'switch',
                        title: 'Відображати мітку якості у повній картці',
                        default: st.show_full_card,
                        onChange: function(v){ st.show_full_card = !!v; saveSettings(st); }
                    },
                    {
                        name: 'label_style',
                        type: 'select',
                        title: 'Стиль мітки якості',
                        values: [
                            {title:'Скорочене відображення (4K, FHD)', value:'short'},
                            {title:'Повне відображення (2160, 1080)', value:'full'}
                        ],
                        default: st.label_style,
                        onChange: function(v){ st.label_style = v; saveSettings(st); }
                    },
                    {
                        name: 'clear_cache',
                        type: 'button',
                        title: 'Очистити кеш',
                        onChange: function(){ clearQualityCache(); }
                    }
                ]
            });
            return true;
    ...
    function addUiViaLegacySettings(){
        if (!Lampa || !Lampa.Settings || typeof Lampa.Settings.add !== 'function') return false;
        try {
            // Add a legacy settings page into Interface section
            Lampa.Settings.add({
                group: 'interface',
                name: 'lqe_settings',
                title: 'Мітки якості',
                subtitle: 'Налаштування плагіна Quality+Mod',
                render: function(body){
                    function addSwitch(title, value, onToggle){
                        var item = $('<div class="settings-param selector"><div class="settings-param__name">'+title+'</div><div class="settings-param__value">'+(value?'Так':'Ні')+'</div></div>');
                        item.on('hover:enter', function(){
                            value = !value;
                            onToggle(value);
                            $('.settings-param__value', item).text(value ? 'Так' : 'Ні');
                        });
                        body.append(item);
                    }
                    function addSelect(title, options, current, onSelect){
                        var values = options.map(function(o){ return o.title; });
                        var item = $('<div class="settings-param selector"><div class="settings-param__name">'+title+'</div><div class="settings-param__value"></div></div>');
                        $('.settings-param__value', item).text(options.find(function(o){return o.value===current;}).title);
                        item.on('hover:enter', function(){
                            Lampa.Select.show({
                                title: title,
                                items: options,
                                onSelect: function(opt){
                                    onSelect(opt.value);
                                    $('.settings-param__value', item).text(opt.title);
                    ...
                    function addButton(title, onClick){
                        var item = $('<div class="settings-param selector"><div class="settings-param__name">'+title+'</div><div class="settings-param__value">ОК</div></div>');
                        item.on('hover:enter', function(){ onClick(); });
                        body.append(item);
                    }

                    body.empty();
                    addSwitch('Відображати мітки якості для серіалів', st.show_tv, function(v){ st.show_tv = v; saveSettings(st); });
                    addSwitch('Відображати мітку якості у повній картці', st.show_full_card, function(v){ st.show_full_card = v; saveSettings(st); });
                    addSelect('Стиль мітки якості', [
                        {title:'Скорочене відображення (4K, FHD)', value:'short'},
                        {title:'Повне відображення (2160, 1080)', value:'full'}
                    ], st.label_style, function(v){ st.label_style = v; saveSettings(st); });
                    addButton('Очистити кеш', clearQualityCache);
                }
            });
            return true;
        } catch(e){
    ...
    // Try to register UI via modern API; fallback to legacy.
    if (!addUiViaSettingsApi()){
        addUiViaLegacySettings();
    }
})();
