(function() {
    ...
    var LQE_CONFIG = {
        ...
        PROXY_TIMEOUT_MS: 4000,                             // Таймаут проксі-запиту (зменшено для швидкості)
        SHOW_QUALITY_FOR_TV_SERIES: true,                   // Показувати якість для серіалів
        MAX_PARALLEL_REQUESTS: 16,                          // Максимум паралельних запитів (збільшено для швидкості)
        
        // 🔧 НОВИЙ ПЕРЕМИКАЧ: спрощене відображення якості
        USE_SIMPLE_QUALITY_LABELS: true,                    // true - короткі формати (4K, FHD, TS), false - повні назви
    ...
    // 🔧 НОВА МАПА: автоматичне спрощення повних назв до коротких форматів
    var QUALITY_SIMPLIFIER_MAP = {
        ...
        // === ПОГАНА ЯКІСТЬ (джерело) - мають пріоритет ===
    ...
    function fetchWithProxy(url, cardId, callback) {
        ...
        function tryNextProxy() {
            ...
            // Таймаут для проксі (зменшено для швидкості)
    ...
    function addLoadingAnimation(cardId, renderElement) {
        ...
        // renderElement - це jQuery об'єкт, використовуємо .find() для пошуку
    ...
    function removeLoadingAnimation(cardId, renderElement) {
        ...
        // Використовуємо .find() для пошуку та видалення анімації
    ...
    function translateQualityLabel(qualityCode, fullTorrentTitle) {
        ...
        // 🔧 НОВА ЛОГІКА: автоматичне спрощення якості
    ...
    /**
     * Виправлена функція визначення якості з назви (нова система балів)
     * @param {string} title - Назва торренту
     * @returns {number} - Бали якості
     */
    function extractNumericQualityFromTitle(title) {
        var lower = (title || '').toLowerCase();
        
        // Високоякісні формати (високі бали)
        ...
        // Низькоякісні формати (низькі бали, але не негативні)
        if (/ts|telesync/.test(lower)) return 100;   // Низький пріоритет
        if (/camrip|камрип/.test(lower)) return 50;  // Дуже низький пріоритет
        if (/tc|telecine/.test(lower)) return 150;   // Низький пріоритет
        
        return 0;
    }

    /**
     * Знаходить найкращий реліз в JacRed API з виправленою системою балів
    ...
    function getBestReleaseFromJacred(normalizedCard, cardId, callback) {
        enqueueTask(function (done) {
            ...
            // Функція пошуку в JacRed API з виправленою системою балів
            function searchJacredApi(searchTitle, searchYear, exactMatch, contentType, apiCallback) {
                ...
                fetchWithProxy(apiUrl, cardId, function (error, responseText) {
                    ...
                    try {
                        ...
                        // Аналізуємо кожен торрент з виправленою системою балів
                        for (var i = 0; i < torrents.length; i++) {
                            var t = torrents[i];
                            
                            // Визначаємо якість з виправленою системою балів
                            ...
                            // Виправлені бонуси за збіг назви (зменшено для уникнення переваги TS)
                            var titleBonus = 0;
                            if (containsWholeWord(t.title, normalizedCard.original_title)) {
                                titleBonus = 300;  // ↓↓↓ ЗМЕНШЕНО з 500
                            } else if (containsWholeWord(t.title, normalizedCard.title)) {
                                titleBonus = 100;  // ↑↑↑ ЗБІЛЬШЕНО з 50
                            }
                            ...
                            // Загальний бал (виправлена формула)
                        ...
                        if (bestTorrent) {
                            ...
                            if (LQE_CONFIG.LOGGING_QUALITY) console.log("LQE-QUALITY", "card: " + cardId + ", Best torrent found:", result, "Score:", bestScore);
                            apiCallback(result);
                        } else {
    ...
    function showFullCardQualityPlaceholder(cardId, renderElement) {
        ...
        if (!$('.full-start__status.lqe-quality', rateLine).length) {
            ...
            // Додаємо плейсхолдер до лінії рейтингу
    ...
    function processFullCardQuality(cardData, renderElement) {
        ...
        if (!(isTvSeries && LQE_CONFIG.SHOW_QUALITY_FOR_TV_SERIES === false)) {
            ...
            if (cachedQualityData) {
                ...
                // Перевіряємо, чи не застарів кеш для фонового оновлення
                ...
                // Якщо даних немає в кеші - показуємо плейсхолдер і робимо запит
            ...
            // Якщо якість вимкнено для серіалів
    ...
    function debouncedProcessNewCards(cards) {
        ...
        observerDebounceTimer = setTimeout(function() {
            // Спрощена перевірка унікальності (без зайвих WeakSet)
            var uniqueCards = [];
            var seenIds = new Set();
        ...
        }, 15); // 🔧 ВИПРАВЛЕНО: зменшено дебаунсинг до 15ms для швидшого відображення
    }
