(function() {
    ...
    var LQE_CONFIG = {
        ...
        USE_SIMPLE_QUALITY_LABELS: false, // Використовувати спрощені мітки якості
    ...
    var QUALITY_SIMPLIFIER_MAP = {
        ...
        // Погана якість (джерело) - мають пріоритет над роздільністю
        "camrip": "CamRip", "cam": "CamRip", "hdcam": "CamRip", "камрип": "CamRip",
        "telesync": "TS", "ts": "TS", "hdts": "TS", "телесинк": "TS",
        "telecine": "TC", "tc": "TC", "hdtc": "TC", "телесин": "TC", // Додано TC
    ...
    Lampa.Template.add('lampa_quality_css', styleLQE); // Додаємо шаблон стилів
    $('body').append(Lampa.Template.get('lampa_quality_css', {}, true)); // Вставляємо стилі в body
    ...
    var fadeStyles = "<style id='lampa_quality_fade'>" +
        ...
        "}" +
        "</style>";
    Lampa.Template.add('lampa_quality_fade', fadeStyles); // Додаємо шаблон
    $('body').append(Lampa.Template.get('lampa_quality_fade', {}, true)); // Вставляємо в DOM
    ...
    Lampa.Template.add('lampa_quality_loading_animation_css', loadingStylesLQE); // Додаємо шаблон
    $('body').append(Lampa.Template.get('lampa_quality_loading_animation_css', {}, true)); // Вставляємо в DOM
    ...
    /**
     * Виправлена функція визначення якості з назви (правильні пріоритети з Quality+)
    ...
    function extractNumericQualityFromTitle(title) {
        ...
        if (/ts|telesync/.test(lower)) return 1;   // Низький пріоритет - TS
        if (/camrip|камрип/.test(lower)) return 2; // Дуже низький пріоритет - CamRip
        if (/tc|telecine/.test(lower)) return 3;   // Додано TC з низьким пріоритетом
        
        return 0; // Якість не визначена
    }

    /**
     * Знаходить найкращий реліз в JacRed API з правильними пріоритетами
            ...
            function searchJacredApi(searchTitle, searchYear, exactMatch, contentType, apiCallback) {
                ...
                fetchWithProxy(apiUrl, cardId, function (error, responseText) {
                    ...
                    try {
                        ...
                        // Аналізуємо кожен торрент з правильними пріоритетами
                        for (var i = 0; i < torrents.length; i++) {
                            var t = torrents[i];
                            
                            // Визначаємо якість з правильними пріоритетами
                            ...
                            // Правильні бонуси за збіг назви
                            var titleBonus = 0;
                            ...
                            // Загальний бал (правильна формула)
