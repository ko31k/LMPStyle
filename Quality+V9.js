(function() {
    ...
    var LQE_CONFIG = {
        ...
        USE_SIMPLE_QUALITY_LABELS: false, // Використовувати спрощені мітки якості (4K, FHD, TS, TC тощо)
    ...
    /**
     * Визначає якість з назви торренту (ПРАВИЛЬНА версія з Quality+)
    ...
    function extractNumericQualityFromTitle(title) {
        ...
        // ✅ ПРАВИЛЬНІ ПРІОРИТЕТИ З QUALITY+:
    ...
    function getBestReleaseFromJacred(normalizedCard, cardId, callback) {
        enqueueTask(function (done) {
                ...
                fetchWithProxy(apiUrl, cardId, function (error, responseText) {
                    ...
                    try {
                        ...
                        for (var i = 0; i < torrents.length; i++) {
                            ...
                            // Перевірка року релізу (проста логіка з Quality+)
                            ...
                            // Якщо рік не збігається - пропускаємо (крім випадків, коли рік не вказано)
                            ...
                            // ✅ ЛОГІКА ВИБОРУ ТОРРЕНТУ З QUALITY+:
            ...
            // ✅ СТРАТЕГІЇ ПОШУКУ З QUALITY+ (прості та ефективні)
...
// =====================================================================
// ==================== PATCHED IMPLEMENTATION V9 ======================
// =====================================================================

// === Нова функція translateQualityLabel (розбір resolution + source) ===
...
// === Нова getBestReleaseFromJacred (з перевіркою року, вибором кращого) ===
function getBestReleaseFromJacred(normalizedCard, cardId, callback) {
    enqueueTask(function(done) {
        if (!LQE_CONFIG.JACRED_URL) {
            callback(null); done(); return;
        }
        var year = '';
        var dateStr = normalizedCard.release_date || '';
        if (dateStr.length >= 4) year = dateStr.substring(0, 4);
        if (!year || isNaN(year)) { callback(null); done(); return; }
        function searchJacredApi(searchTitle, searchYear, exactMatch, strategyName, apiCallback) {
            var userId = Lampa.Storage.get('lampac_unic_id', '');
            var apiUrl = LQE_CONFIG.JACRED_PROTOCOL + LQE_CONFIG.JACRED_URL + '/api/v1.0/torrents?search=' +
                encodeURIComponent(searchTitle) +
                ...
                (exactMatch ? '&exact=true' : '') +
                '&uid=' + userId;
            var timeoutId = setTimeout(function() { apiCallback(null); }, LQE_CONFIG.PROXY_TIMEOUT_MS * LQE_CONFIG.PROXY_LIST.length + 1000);
            fetchWithProxy(apiUrl, cardId, function(error, responseText) {
                clearTimeout(timeoutId);
                ...
                try {
                    ...
                    function extractYearFromTitle(title) {
                        if (!title) return 0;
                        var regex = /(?:^|[^\d])(\d{4})(?:[^\d]|$)/g;
                        var match; var lastYear = 0; var currentYear = new Date().getFullYear();
                        while ((match = regex.exec(title)) !== null) {
                            var extractedYear = parseInt(match[1], 10);
                            if (extractedYear >= 1900 && extractedYear <= currentYear + 1) {
                                lastYear = extractedYear;
                    ...
                    for (var i = 0; i < torrents.length; i++) {
                        ...
                        var isYearValid = false; var parsedYear = 0;
                        if (torrentYear && !isNaN(torrentYear) && torrentYear > 1900) {
                            parsedYear = parseInt(torrentYear, 10); isYearValid = true;
                        }
                        if (!isYearValid) {
                            parsedYear = extractYearFromTitle(currentTorrent.title);
                            if (parsedYear > 0) { torrentYear = parsedYear; isYearValid = true; }
                        }
                        if (isYearValid && !isNaN(searchYearNum) && parsedYear !== searchYearNum) continue;
                        if (currentNumericQuality > bestNumericQuality) {
                            bestNumericQuality = currentNumericQuality;
                            bestFoundTorrent = currentTorrent;
        ...
        var searchStrategies = [];
        if (normalizedCard.original_title) searchStrategies.push({ title: normalizedCard.original_title.trim(), year: year, exact: true, name: "OriginalTitle Exact Year" });
        if (normalizedCard.title) searchStrategies.push({ title: normalizedCard.title.trim(), year: year, exact: true, name: "Title Exact Year" });
        function executeNextStrategy(index) {
            if (index >= searchStrategies.length) { callback(null); done(); return; }
            ...
            searchJacredApi(strategy.title, strategy.year, strategy.exact, strategy.name, function(result) {
                if (result !== null) { callback(result); done(); }
