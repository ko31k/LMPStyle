...
// ===================== LQE_CONFIG =====================
    var LQE_CONFIG = {
        CACHE_VERSION: 4, // Збільшено через зміни в логіці Ukr
...
// ===================== UKR АУДІО КОНФІГУРАЦІЯ =====================
    var UKR_CONFIG = {
        SEARCH_BOTH_LANGUAGES: true,
        MIN_QUALITY_FOR_UKR: 720,
        REQUIRE_YEAR_MATCH: true,
        FILTER_OUT_ANIME: true,
...
// ===================== UKR АУДІО ФУНКЦІЇ =====================
function hasUkrAudio(torrentTitle) {
    if (!torrentTitle) return false;
    
    var title = torrentTitle.toLowerCase();
    
    // 1. ВИКЛЮЧАЄМО СУБТИТРИ
    if (isSubtitle(title)) {
        return false;
    }
    
    // 2. ШУКАЄМО АУДІО
    return hasAudioPattern(title);
}
...
function isSubtitle(title) {
    var subPatterns = [
        ...
        /sub\s*-\s*[^-]*ukr[^-]*/i,
        /sub\s*\/\s*[^\/]*ukr[^\/]*/i,
        /\bsub\s+ukr\s*\/\s*[a-z]{2,4}/i,
        /\bsub\s+укр\s*\/\s*[а-яa-z]{2,4}/i
    ];
    
    for (var i = 0; i < subPatterns.length; i++) {
        if (subPatterns[i].test(title)) {
            return true;
        }
    }
    return false;
...
function hasAudioPattern(title) {
    var audioPatterns = [
        /\b\d*x\s*ukr\s*\/\s*[a-z]{2,4}/i,
        /\b\d*x\s*укр\s*\/\s*[а-яa-z]{2,4}/i,
        /\bukr\s*\/\s*[a-z]{2,4}/i,
        /\bукр\s*\/\s*[а-яa-z]{2,4}/i,
        /\b[a-z]{2,4}\s*\/\s*ukr/i,
        /\b[а-яa-z]{2,4}\s*\/\s*укр/i,
        /\bukr\b(?!\w)/i,
        /\букр\b(?!\w)/i,
        /\b\d*xukr\b/i,
        /\b\d*xукр\b/i,
    ...
    for (var j = 0; j < audioPatterns.length; j++) {
        if (audioPatterns[j].test(title)) {
            return true;
        }
    }
    return false;
...
function isAnimeContent(torrentTitle, cardTitle) {
    var title = (torrentTitle + ' ' + (cardTitle || '')).toLowerCase();
    var animeKeywords = [
        'anime', 'аніме', 'ova', 'oav', 'седзе', 'сёнен', 'токусацу',
        'studio ghibli', 'макото сінкай', 'хаяо миядзаки', 'аниме'
    ];
    
    for (var i = 0; i < animeKeywords.length; i++) {
        if (title.includes(animeKeywords[i])) {
            return true;
        }
    }
    return false;
...
function extractNumericQualityFromTitle(title) {
    ...
    if (/1080p|full.hd|fhd/.test(lower)) return 1080;
    if (/720p|hd.ready/.test(lower)) return 720;
...
function isRelevantContent(torrentTitle, cardTitle, targetYear) {
    if (!torrentTitle) return false;
    
    var title = torrentTitle.toLowerCase();
    var card = (cardTitle || '').toLowerCase();
    
    if (UKR_CONFIG.REQUIRE_YEAR_MATCH) {
        var torrentYear = extractYearFromTitle(torrentTitle);
        if (targetYear && torrentYear && torrentYear !== parseInt(targetYear)) {
            return false;
        }
    }
    
    if (UKR_CONFIG.FILTER_OUT_ANIME) {
        var irrelevantKeywords = [
            'anime', 'аніме', 'ova', 'oav', 'седзе', 'сёнен',
            'токусацу', 'мульт', 'cartoon', 'аниме'
        ];
        
        for (var i = 0; i < irrelevantKeywords.length; i++) {
            if (title.includes(irrelevantKeywords[i]) && 
                !card.includes(irrelevantKeywords[i])) {
                return false;
            }
        }
    }
...
function translateQualityLabel(qualityCode, fullTorrentTitle, hasUkrAudio) {
    if (LQE_CONFIG.LOGGING_QUALITY) console.log("LQE-QUALITY", "translateQualityLabel:", qualityCode, fullTorrentTitle, hasUkrAudio);
    ...
    // 6) Додаємо (Ukr) якщо є українське аудіо
    if (hasUkrAudio) {
        finalLabel += " (Ukr)";
    }
...
// ===================== getBestReleaseFromJacred =====================
    function getBestReleaseFromJacred(normalizedCard, cardId, callback) {
        enqueueTask(function(done) {
            ...
            // Додаємо англійські назви
            if (normalizedCard.original_title && /[a-zа-яё]/i.test(normalizedCard.original_title)) {
                searchQueries.push(normalizedCard.original_title.trim());
            }
            
            // Додаємо локалізовані назви
            if (normalizedCard.title && normalizedCard.title !== normalizedCard.original_title) {
                searchQueries.push(normalizedCard.title.trim());
            }

            if (searchQueries.length === 0) {
                if (LQE_CONFIG.LOGGING_QUALITY) console.log("LQE-QUALITY", "card: " + cardId + ", JacRed: No valid search titles.");
                callback(null);
                done();
                return;
            }
            ...
            var hasUkrAudioInAnyTorrent = false;
            ...
            function processSearchResults(results, query) {
                ...
                for (var i = 0; i < results.length; i++) {
                    ...
                    if (!isRelevantContent(torrent.title, normalizedCard.title, year)) {
                        continue;
                    }
                    
                    if (hasUkrAudio(torrent.title)) {
                        hasUkrAudioInAnyTorrent = true;
                    }
            ...
            function executeSearch(query, index) {
                var apiUrl = LQE_CONFIG.JACRED_PROTOCOL + LQE_CONFIG.JACRED_URL + 
                    '/api/v1.0/torrents?search=' + encodeURIComponent(query) +
                    '&year=' + year + '&exact=true&uid=' + 
                    Lampa.Storage.get('lampac_unic_id', '');

                if (LQE_CONFIG.LOGGING_QUALITY) console.log("LQE-QUALITY", "card: " + cardId + ", JacRed: Searching with title: " + query);
                ...
                fetchWithProxy(apiUrl, cardId, function(error, responseText) {
                    ...
                    if (!error && responseText) {
                        try {
                            ...
                            processSearchResults(torrents, query);
                        } catch (e) {
            ...
            function finalizeSearch() {
                // Видаляємо дублікати
                if (UKR_CONFIG.FILTER_OUT_DUPLICATES) {
                    var uniqueResults = [];
                    var seenTitles = {};
                    
                    for (var i = 0; i < allResults.length; i++) {
                        var normTitle = allResults[i].title.toLowerCase().replace(/[^\w]/g, '');
                ...
                // Знаходимо найкращий реліз
                var bestTorrent = null;
                var bestQuality = -1;
                
                for (var i = 0; i < allResults.length; i++) {
                    var torrent = allResults[i];
                    var quality = torrent.quality || extractNumericQualityFromTitle(torrent.title);
                    
                    if (quality >= UKR_CONFIG.MIN_QUALITY_FOR_UKR && quality > bestQuality) {
                        bestQuality = quality;
                        bestTorrent = torrent;
                    }
                }
                ...
                if (bestTorrent) {
                    var result = {
                        ...
                        has_ukr_audio: hasUkrAudioInAnyTorrent
                    };
            ...
            // Запускаємо всі пошуки
            for (var i = 0; i < searchQueries.length; i++) {
                executeSearch(searchQueries[i], i);
            }
        });
    }
...
// ===================== CACHE HELPERS =====================
    ...
    function saveQualityCache(key, data, cardId) {
        ...
        cache[key] = {
            ...
            has_ukr_audio: data.has_ukr_audio || false,
            timestamp: Date.now()
        };
...
// ===================== UI helpers =====================
    ...
    function updateFullCardQualityElement(qualityCode, fullTorrentTitle, hasUkrAudio, cardId, renderElement, bypassTranslation) {
        if (!renderElement) return;
        var element = $('.full-start__status.lqe-quality', renderElement);
        var rateLine = $('.full-start-new__rate-line', renderElement);
        if (!rateLine.length) return;

        var displayQuality = bypassTranslation ? 
            fullTorrentTitle + (hasUkrAudio ? " (Ukr)" : "") : 
            translateQualityLabel(qualityCode, fullTorrentTitle, hasUkrAudio);
    ...
    function updateCardListQualityElement(cardView, qualityCode, fullTorrentTitle, hasUkrAudio, bypassTranslation) {
        var displayQuality = bypassTranslation ? 
            fullTorrentTitle + (hasUkrAudio ? " (Ukr)" : "") : 
            translateQualityLabel(qualityCode, fullTorrentTitle, hasUkrAudio);
...
// ===================== processFullCardQuality =====================
    function processFullCardQuality(cardData, renderElement) {
        ...
        if (manualOverrideData) {
            ...
            updateFullCardQualityElement(null, manualOverrideData.full_label, false, cardId, renderElement, true);
        ...
        if (!(isTvSeries && LQE_CONFIG.SHOW_QUALITY_FOR_TV_SERIES === false)) {
            ...
            if (cachedQualityData) {
                ...
                updateFullCardQualityElement(cachedQualityData.quality_code, cachedQualityData.full_label, cachedQualityData.has_ukr_audio, cardId, renderElement);
                    ...
                    getBestReleaseFromJacred(normalizedCard, cardId, function(jrResult) {
                        if (jrResult && jrResult.quality && jrResult.quality !== 'NO') {
                            saveQualityCache(cacheKey, {
                                ...
                                has_ukr_audio: jrResult.has_ukr_audio
                            }, cardId);
                            updateFullCardQualityElement(jrResult.quality, jrResult.full_label, jrResult.has_ukr_audio, cardId, renderElement);
                ...
                getBestReleaseFromJacred(normalizedCard, cardId, function(jrResult) {
                    ...
                    var hasUkrAudio = (jrResult && jrResult.has_ukr_audio) || false;
                    if (LQE_CONFIG.LOGGING_QUALITY) console.log("LQE-QUALITY", "JacRed returned - qualityCode:", qualityCode, "full label:", fullTorrentTitle, "hasUkrAudio:", hasUkrAudio);
                    if (qualityCode && qualityCode !== 'NO') {
                        if (LQE_CONFIG.LOGGING_QUALITY) console.log('LQE-QUALITY', 'card: ' + cardId + ', JacRed found quality code: ' + qualityCode + ', full label: ' + fullTorrentTitle + ', hasUkrAudio: ' + hasUkrAudio);
                        saveQualityCache(cacheKey, {
                            quality_code: qualityCode,
                            full_label: fullTorrentTitle,
                            has_ukr_audio: hasUkrAudio
                        }, cardId);
                        updateFullCardQualityElement(qualityCode, fullTorrentTitle, hasUkrAudio, cardId, renderElement);
                    } else {
...
// ===================== updateCardListQuality =====================
    function updateCardListQuality(cardElement) {
        ...
        if (manualOverrideData) {
            ...
            updateCardListQualityElement(cardView, null, manualOverrideData.full_label, false, true);
            return;
        }
        ...
        if (cachedQualityData) {
            ...
            updateCardListQualityElement(cardView, cachedQualityData.quality_code, cachedQualityData.full_label, cachedQualityData.has_ukr_audio);
                ...
                getBestReleaseFromJacred(normalizedCard, cardId, function(jrResult) {
                    if (jrResult && jrResult.quality && jrResult.quality !== 'NO') {
                        ...
                        if (document.body.contains(cardElement)) {
                            updateCardListQualityElement(cardView, jrResult.quality, jrResult.full_label, jrResult.has_ukr_audio);
        ...
        getBestReleaseFromJacred(normalizedCard, cardId, function(jrResult) {
            ...
            var fullTorrentTitle = (jrResult && jrResult.full_label) || null;
            var hasUkrAudio = (jrResult && jrResult.has_ukr_audio) || false;
            if (LQE_CONFIG.LOGGING_CARDLIST) console.log("LQE-CARDLIST: JacRed returned - qualityCode:", qualityCode, "full label:", fullTorrentTitle, "hasUkrAudio:", hasUkrAudio);
            if (qualityCode && qualityCode !== 'NO') {
                if (LQE_CONFIG.LOGGING_CARDLIST) console.log('LQE-CARDLIST', 'card: ' + cardId + ', JacRed found quality code: ' + qualityCode + ', full label: ' + fullTorrentTitle + ', hasUkrAudio: ' + hasUkrAudio);
                saveQualityCache(cacheKey, {
                    quality_code: qualityCode,
                    full_label: fullTorrentTitle,
                    has_ukr_audio: hasUkrAudio
                }, cardId);
                updateCardListQualityElement(cardView, qualityCode, fullTorrentTitle, hasUkrAudio);
