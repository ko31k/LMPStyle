            ...
            function searchJacredApi(searchTitle, searchYear, exactMatch, strategyName, apiCallback) {
                ...
                fetchWithProxy(apiUrl, cardId, function(error, responseText) {
                    ...
                    try {
                        ...
                        // === NEW: check across all torrents for Ukrainian audio (not subs-only) ===
                        var hasUkrAudioGlobal = false;
                        var reUkrAudio = /\b(ukr|ukrainian|ukrdub|ukrvoice|укр|україн|укрдуб|дубляж|дуб)\b/i;
                        var reUkrSub = /\b(subs?|subtitle|subbed|subtitl|субтитр)\b/i;

                        for (var u = 0; u < torrents.length; u++) {
                            var tU = torrents[u];
                            var titleLower = (tU.title || '').toString();
                            var hasUkr = reUkrAudio.test(titleLower);
                            var hasUkrSub = reUkrSub.test(titleLower);
                            if (hasUkr && !hasUkrSub) { hasUkrAudioGlobal = true; break; }
                        }
                        ...
                        if (bestFoundTorrent) {
                            var fullTitle = bestFoundTorrent.title;
                            if (hasUkrAudioGlobal) fullTitle += " (Ukr)";
                            apiCallback({
                                quality: bestFoundTorrent.quality || bestNumericQuality,
                                full_label: fullTitle
                            });
                        } else {
                            apiCallback(null);
                        }
