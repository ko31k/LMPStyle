...
// ===================== loading animation styles (залишаємо) =====================
...
// ===================== network helper with proxy and timeouts (збережено логіку) =====================
...
// ===================== translateQualityLabel (без змін логіки, але з невеликими оптимізаціями) =====================
...
// ============================== Request queue (реалізовано класом, виправлено) ===============================
    class RequestQueue {
        ...
        add(fn) {
            if (LQE_CONFIG.LOGGING_GENERAL) console.log("LQE-LOG", "RequestQueue: add task, queue length before:", this.queue.length);
        ...
        _process() {
            ...
            var doneCallback = function() {
                ...
                setTimeout(function(){ self._process(); }, 0);
            };
            try {
                item.fn(doneCallback);
            } catch (e) {
                if (LQE_CONFIG.LOGGING_GENERAL) console.error("LQE-LOG", "RequestQueue task error", e);
                doneCallback();
            }
        }
    }
    ...
    // backward-compatible enqueueTask wrapper
    function enqueueTask(fn) {
        return requestQueue.add(fn);
    }

// ===================== getBestReleaseFromJacred (оновлена, без зміни логіки) =====================
function getBestReleaseFromJacred(normalizedCard, cardId, callback) {
    enqueueTask(function (done) {
        ...
        // --- Утиліти (винесено локально для читабельності) ---
...
// ===================== UI helpers (нативні селектори де можливо) =====================
    ...
    function updateCardListQualityElement(cardView, qualityCode, fullTorrentTitle, bypassTranslation) {
        ...
        // Check existing and avoid DOM update if same
    ...
    function processFullCardQuality(cardData, renderElement) {
        try {
            ...
            if (rateLine) {
                ...
                // restore "done" class as in original implementation
                try { rateLine.classList.add('done'); } catch(e){}
            ...
            if (manualOverrideData) {
                ...
                if (rateLine) { rateLine.style.visibility = 'visible'; try { rateLine.classList.remove('done'); } catch(e){} }
                return;
            }
...
// ===================== MutationObserver optimization: дебаунсинг та батчинг (виправлено) =====================
    ...
    function debouncedProcessNewCards(cards) {
        ...
        observerDebounceTimer = setTimeout(function() {
            ...
            for (var i = 0; i < cards.length; i++) {
                var c = cards[i];
                if (!c) continue;
                var cid = c.card_data && c.card_data.id;
                if (cid) {
            ...
            if (LQE_CONFIG.LOGGING_CARDLIST) {
                console.log("LQE-CARDLIST", "Processing", unique.length, "unique cards");
            }
            unique.forEach(updateCardListQuality);
        }, 50); // 50ms debounce
    }
