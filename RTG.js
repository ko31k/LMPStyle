(function() {
    ...
    /**
     * =========================
     *  USER SETTINGS (saved in Lampa.Storage)
     * =========================
     */
    var SETTINGS_KEYS = {
        omdb: 'ratings_plugin_omdb_key',
        mdblist: 'ratings_plugin_mdblist_key',
        monochrome: 'ratings_plugin_monochrome',
        showawards: 'ratings_plugin_showawards',
        showaverage: 'ratings_plugin_showaverage',
        icon_offset_px: 'ratings_plugin_icon_offset_px',
        number_offset: 'ratings_plugin_number_offset',
        badge_size_offset_px: 'ratings_plugin_badge_size_offset_px',
        badge_opacity: 'ratings_plugin_badge_opacity',
        badge_tone: 'ratings_plugin_badge_tone',
        margin_steps: 'ratings_plugin_margin_steps'
    };
    ...
    function readUserSettings() {
        var s = {};
        try {
            s.omdb = (Lampa.Storage.get(SETTINGS_KEYS.omdb) || '').trim();
        } catch(e){ s.omdb = ''; }
        try {
            s.mdblist = (Lampa.Storage.get(SETTINGS_KEYS.mdblist) || '').trim();
        } catch(e){ s.mdblist = ''; }

        s.monochrome = parseBoolSafe(Lampa.Storage.get(SETTINGS_KEYS.monochrome), false);
        s.showawards = parseBoolSafe(Lampa.Storage.get(SETTINGS_KEYS.showawards), true);
        s.showaverage = parseBoolSafe(Lampa.Storage.get(SETTINGS_KEYS.showaverage), true);

        s.icon_offset_px = parseIntSafe(Lampa.Storage.get(SETTINGS_KEYS.icon_offset_px), 0);
        s.number_offset = parseIntSafe(Lampa.Storage.get(SETTINGS_KEYS.number_offset), 0);
        s.badge_size_offset_px = parseIntSafe(Lampa.Storage.get(SETTINGS_KEYS.badge_size_offset_px), 0);
        s.badge_opacity = Lampa.Storage.get(SETTINGS_KEYS.badge_opacity);
        s.badge_tone = parseIntSafe(Lampa.Storage.get(SETTINGS_KEYS.badge_tone), 0);
        s.margin_steps = Math.max(0, parseIntSafe(Lampa.Storage.get(SETTINGS_KEYS.margin_steps), 0));

        // normalize opacity: if empty or '0' -> use standard (0.15) as per requirement
        var op = parseFloat(s.badge_opacity);
        if (isNaN(op) || s.badge_opacity === '' ) {
            s.badge_opacity = 0.15; // default Lampa
        } else if (s.badge_opacity === '0') {
            // per requirement: "0" -> return standard Lampa
            s.badge_opacity = 0.15;
        } else {
            s.badge_opacity = Math.min(1, Math.max(0, op));
        }

        return s;
    }

    var USER_SETTINGS = readUserSettings();

    function applyUserConfig() {
        // Override API keys if provided
        if (USER_SETTINGS.omdb) LMP_ENH_CONFIG.apiKeys.omdb = USER_SETTINGS.omdb;
        if (USER_SETTINGS.mdblist) LMP_ENH_CONFIG.apiKeys.mdblist = USER_SETTINGS.mdblist;

        LMP_ENH_CONFIG.monochromeIcons = !!USER_SETTINGS.monochrome;

        if (LMP_ENH_CONFIG.monochromeIcons) $('body').addClass('lmp-enh--mono');
        else $('body').removeClass('lmp-enh--mono');
    }

    function computeNumberScale() {
        // 0 -> 1.0; +1 -> 1.1; -1 -> 0.9
        return 1 + (USER_SETTINGS.number_offset * 0.1);
    }

    function computeMarginRight() {
        // base 1em + 0.1em * steps
        var add = USER_SETTINGS.margin_steps * 0.1;
        return 'calc(1em + ' + add + 'em)';
    }

    function effectiveIconSize(basePx) {
        var off = USER_SETTINGS.icon_offset_px || 0;
        var size = basePx + off;
        if (size < 8) size = 8; // clamp to reasonable minimum
        return size;
    }

    // Inject / update dynamic style for badge backgrounds, margins and sizes
    function applyCustomStyles() {
        var tone = Math.min(255, Math.max(0, USER_SETTINGS.badge_tone || 0));
        var alpha = USER_SETTINGS.badge_opacity;
        var badgePad = USER_SETTINGS.badge_size_offset_px || 0;
        var numberScale = computeNumberScale();
        var marginRight = computeMarginRight();
        ...
        + '  background: rgba(' + tone + ',' + tone + ',' + tone + ',' + alpha + ');'
        + '  border-radius: 0.3em;'
        + '  padding-left:' + Math.max(0,badgePad) + 'px;'
        + '  padding-right:' + Math.max(0,badgePad) + 'px;'
        + '}'
        + '.full-start-new__rate-line .full-start__rate > div:first-child{'
        + '  background: rgba(' + tone + ',' + tone + ',' + tone + ',' + alpha + ');'
        + '  border-radius: 0.3em;'
        + '  font-size: ' + numberScale + 'em;'
        + '}';

        var id = 'ratings-plugin-dynamic';
    ...
    // Minimal settings overlay (fallback if we can't hook into Lampa Settings)
    function openRatingsOverlay() {
        var existing = document.getElementById('ratings-plugin-overlay');
        if (existing) existing.remove();

        var wrap = document.createElement('div');
        wrap.id = 'ratings-plugin-overlay';
        wrap.setAttribute('style',
            'position:fixed; z-index:99999; inset:0; background:rgba(0,0,0,.6); display:flex; align-items:center; justify-content:center;'
        );
        var panel = document.createElement('div');
        panel.setAttribute('style','background:#1a1a1a; color:#fff; width:90%; max-width:720px; max-height:85vh; overflow:auto; border-radius:12px; padding:16px;');
        panel.innerHTML =
            '<h2 style="margin:0 0 12px 0;">★ Рейтинги — налаштування</h2>'
          + '<div style="display:grid; grid-template-columns: 1fr 180px; gap:8px; align-items:center;">'
          + '<label>API ключ (OMDb)</label><input id="rp_in_omdb" type="text" value="'+ (USER_SETTINGS.omdb||'') + '"/>'
          + '<label>API ключ (MDBList)</label><input id="rp_in_mdblist" type="text" value="'+ (USER_SETTINGS.mdblist||'') + '"/>'
          + '<label>Ч/Б логотипи</label><input id="rp_tg_mono" type="checkbox" '+ (USER_SETTINGS.monochrome?'checked':'') + '/>'
          + '<label>Нагороди (Оскар/Еммі/інші)</label><input id="rp_tg_awards" type="checkbox" '+ (USER_SETTINGS.showawards?'checked':'') + '/>'
          + '<label>Середній рейтинг (★)</label><input id="rp_tg_avg" type="checkbox" '+ (USER_SETTINGS.showaverage?'checked':'') + '/>'
          + '<label>Δ висоти логотипів (px)</label><input id="rp_in_iconoff" type="number" value="'+ (USER_SETTINGS.icon_offset_px||0) + '"/>'
          + '<label>Δ розміру числа</label><input id="rp_in_numoff" type="number" value="'+ (USER_SETTINGS.number_offset||0) + '"/>'
          + '<label>Δ розміру бейджа (px)</label><input id="rp_in_badgeoff" type="number" value="'+ (USER_SETTINGS.badge_size_offset_px||0) + '"/>'
          + '<label>Прозорість фону (0…1)</label><input id="rp_in_opacity" type="text" value="'+ (USER_SETTINGS.badge_opacity||0.15) + '"/>'
          + '<label>Яскравість (0…255)</label><input id="rp_in_tone" type="number" value="'+ (USER_SETTINGS.badge_tone||0) + '"/>'
          + '<label>Кроки відступу (0=1em)</label><input id="rp_in_margin" type="number" min="0" value="'+ (USER_SETTINGS.margin_steps||0) + '"/>'
          + '</div>'
          + '<div style="margin-top:12px; display:flex; gap:8px; justify-content:flex-end;">'
          + '<button id="rp_btn_close">Закрити</button>'
          + '<button id="rp_btn_save">Зберегти</button>'
          + '</div>';
        ...
        panel.querySelector('#rp_btn_close').addEventListener('click', close);
        panel.querySelector('#rp_btn_save').addEventListener('click', function(){
            function set(k,v){ try{ Lampa.Storage.set(k,v); }catch(e){} }

            set(SETTINGS_KEYS.omdb,     panel.querySelector('#rp_in_omdb').value.trim());
            set(SETTINGS_KEYS.mdblist,  panel.querySelector('#rp_in_mdblist').value.trim());
            set(SETTINGS_KEYS.monochrome, panel.querySelector('#rp_tg_mono').checked ? 'true' : 'false');
            set(SETTINGS_KEYS.showawards, panel.querySelector('#rp_tg_awards').checked ? 'true' : 'false');
            set(SETTINGS_KEYS.showaverage, panel.querySelector('#rp_tg_avg').checked ? 'true' : 'false');

            set(SETTINGS_KEYS.icon_offset_px, panel.querySelector('#rp_in_iconoff').value);
            set(SETTINGS_KEYS.number_offset, panel.querySelector('#rp_in_numoff').value);
            set(SETTINGS_KEYS.badge_size_offset_px, panel.querySelector('#rp_in_badgeoff').value);
            set(SETTINGS_KEYS.badge_opacity, panel.querySelector('#rp_in_opacity').value);
            set(SETTINGS_KEYS.badge_tone, panel.querySelector('#rp_in_tone').value);
            set(SETTINGS_KEYS.margin_steps, panel.querySelector('#rp_in_margin').value);

            USER_SETTINGS = readUserSettings();
            applyUserConfig();
            applyCustomStyles();

            // force rerender current line
            try{
                var render = Lampa.Activity.active().activity.render();
                var rateLine = $('.full-start-new__rate-line', render);
                if (rateLine.length) {
                    // remove our badges to rebuild with toggles
                    rateLine.find('.rate--avg, .rate--awards, .rate--emmy, .rate--oscars, .rate--mc, .rate--rt, .rate--popcorn').remove();
                    // restart build using cached data if present
                    fetchAdditionalRatings(Lampa.Activity.active().data || {});
                }
            }catch(e){}
            close();
        });
    ...
    // Add a small gear to rate line to open overlay
    function ensureGearButton() {
        try {
            ...
            if (rateLine.find('.ratings-gear-btn').length) return;

            var btn = $('<div class="full-start__rate ratings-gear-btn" style="cursor:pointer;">'
                + '<div>⚙️</div>'
                + '<div class="source--name">Settings</div>'
                + '</div>');
            btn.on('click', openRatingsOverlay);
            rateLine.append(btn);
        } catch(e){}
    }
};
...
var pluginStyles = "<style>" +
    ...
    */
    ...
    var ID_MAPPING_CACHE = 'lmp_enh_id_mapping';
    ...
    function iconImg(url, alt, sizePx, extraStyle) {
        ...
        var finalSize = effectiveIconSize(sizePx);
        return '<img style="' +
            'height:' + finalSize + 'px; width:auto; display:inline-block; vertical-align:middle; ' +
            'object-fit:contain; ' +
            (extraStyle || '') + ' ' +
            filter + '" ' +
            'src="' + url + '" alt="' + (alt || '') + '">';
...
function insertRatings(data) {
    ...
    if (USER_SETTINGS.showawards && data.awards && data.awards > 0 && !$('.rate--awards', rateLine).length) {
        var awardsElement = $(
    ...
    // Emmy
    if (USER_SETTINGS.showawards && data.emmy && data.emmy > 0 && !$('.rate--emmy', rateLine).length) {
        var emmyElement = $(
    ...
    // Oscars
    if (USER_SETTINGS.showawards && data.oscars && data.oscars > 0 && !$('.rate--oscars', rateLine).length) {
        var oscarsElement = $(
