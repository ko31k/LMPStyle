(function () {
     ...
     *
     * - Додає блоки рейтингів (IMDb, TMDB, MC, Rotten, Popcorn, AVG)
     *   і нагороди (Оскар, Еммі, інші перемоги) в картку тайтла.
     *
     * - Має власний розділ "Рейтинги" в налаштуваннях Lampa:
     *     • API ключ (OMDb)
     *     • API ключ (MDBList)
     *     • Ч/Б логотипи
     *     • Нагороди
     ...
     *     • Тон фону
     *
     * Всі значення зберігаються у Lampa.Storage локально.
     ********************************************************************/
    ...
    // стилі
    var STORAGE_KEY_ICON_OFFSET     = 'ratings_plugin_icon_offset_px';          // 0 за замовчуванням, +1 -> +1px висоти іконок
    var STORAGE_KEY_MARGIN_STEPS    = 'ratings_plugin_margin_steps';            // 0 за замовчуванням, +1 -> +0.1em до margin-right
    var STORAGE_KEY_BADGE_SIZE_OFF  = 'ratings_plugin_badge_size_offset_px';    // 0 за замовчуванням, +1 -> +1px до padding блоку (по обидвох осях)
    var STORAGE_KEY_BADGE_OPACITY   = 'ratings_plugin_badge_opacity';           // прозорість фону бейджів (alpha), дефолт 0.6
    var STORAGE_KEY_BADGE_TONE      = 'ratings_plugin_badge_tone';              // яскравість (0..255), дефолт 0 (чорний)
    ...
    // ініціалізуємо дефолтні значення, щоб у меню відразу було зрозуміло
    ...
    ensureDefault(STORAGE_KEY_BADGE_OPACITY, 0.6);    // rgba(..., ..., ..., 0.6) приблизно як стандартний фон Lampa
    ...
    function updateDynamicStyles() {
        // відступ між блоками:
        // базово 0.3em, кожен "крок" (+1) -> +0.1em
        // "0" = 0.3em; "1" = 0.4em; "-1" = 0.2em
        var marginEm = 0.3 + (0.1 * RatingsSettings.marginSteps);
        if (marginEm < 0) marginEm = 0; // не даємо піти в мінус

        // padding бейджів:
        // базово 0.6em вертикально, 1em горизонтально
        // додаємо badgeSizeOffsetPx в пікселях до обох
        var padOffsetPx = RatingsSettings.badgeSizeOffsetPx;
        if (isNaN(padOffsetPx)) padOffsetPx = 0;

        // фон бейджів
        var tone = RatingsSettings.badgeTone;       // 0..255 (0 = чорний)
        var alpha = RatingsSettings.badgeOpacity;   // 0..1 прозорість
    ...
    /*******************************
     * СТАТИЧНІ СТИЛІ (базові)
     * - лоадер
     * - золото для нагород у кольоровому режимі
     * - сірий/моно режим (через body.lmp-enh--mono)
     *******************************/
...
/* У нормальному (кольоровому) режимі цифри нагород золоті */
.rate--oscars,
.rate--emmy,
.rate--awards,
.rate--gold {
...
/* У моно-режимі:
   - прибираємо золото
   - прибираємо зелений/помаранчевий/червоний і т.д.
   - робимо цифри нейтральними
*/
...
/* дефолтно ми задаємо невеликий відступ між бейджами,
   пізніше він буде перебитий динамічним стилем через updateDynamicStyles() */
.full-start-new__rate-line .full-start__rate{
...
.full-start-new__rate-line .full-start__rate:last-child{
    ...
    function oscarIconInline() {
        var base = 18 + RatingsSettings.iconOffsetPx;
        if (base < 1) base = 1;
        var filter = RatingsSettings.monochrome ? 'filter:grayscale(100%);' : '';
        ...
        return '' +
            ...
            '" style="height:' + base + 'px; width:auto; display:inline-block; vertical-align:middle; object-fit:contain;">' +
            '</span>';
    }
    ...
    /*******************************
     * OMDb API — РЕЙТИНГИ, НАГОРОДИ, ВІКОВИЙ РЕЙТИНГ
     *******************************/
        ...
        if (imdbContainer.length) {
            if (data.imdb_display) {
                ...
                if (imdbParts.length >= 2) {
                    ...
                    var h = 22 + RatingsSettings.iconOffsetPx;
                    if (h < 1) h = 1;

                    imdbParts.eq(1).html(
                        iconImg(ICONS.imdb, 'IMDb', h)
                    );
                }
            } else {
        ...
        if (tmdbContainer.length) {
            if (data.tmdb_display) {
                ...
                if (tmdbParts.length >= 2) {
                    ...
                    var h2 = 24 + RatingsSettings.iconOffsetPx;
                    if (h2 < 1) h2 = 1;

                    tmdbParts.eq(1).html(
                        iconImg(ICONS.tmdb, 'TMDB', h2)
                    );
                }
            }
        }
     ...
     *******************************/
    function iconImg(url, alt, heightPx, extraStyle) {
        var filter = RatingsSettings.monochrome ? 'filter:grayscale(100%);' : '';
        var finalH = heightPx;
        // safety в разі якщо heightPx з урахуванням offset стане <1
    ...
    function insertRatings(data) {
        ...
        if (data.mc_display && !$('.rate--mc', rateLine).length) {
            ...
            var mcH = 22 + RatingsSettings.iconOffsetPx;
            if (mcH < 1) mcH = 1;

            mcElement.find('.source--name').html(
                iconImg(ICONS.metacritic, 'Metacritic', mcH)
            );
        ...
        if (data.rt_display && !$('.rate--rt', rateLine).length) {
            ...
            var rtH = 22 + RatingsSettings.iconOffsetPx;
            if (rtH < 1) rtH = 1;

            rtElement.find('.source--name').html(
                iconImg(rtIconUrl, 'Rotten Tomatoes', rtH, extra)
            );
        ...
        if (data.popcorn_display && !$('.rate--popcorn', rateLine).length) {
            ...
            var pcH = 22 + RatingsSettings.iconOffsetPx;
            if (pcH < 1) pcH = 1;

            pcElement.find('.source--name').html(
                iconImg(ICONS.popcorn, 'Audience', pcH)
            );
        ...
        // Нагороди (тільки якщо показ нагород увімкнено в налаштуваннях)
        if (RatingsSettings.showAwards) {
            ...
            if (data.awards && data.awards > 0 && !$('.rate--awards', rateLine).length) {
                ...
                var awH = 20 + RatingsSettings.iconOffsetPx;
                if (awH < 1) awH = 1;

                awardsElement.find('.source--name')
                    .html(iconImg(ICONS.awards, 'Awards', awH))
    ...
    function calculateAverageRating(data) {
        ...
        // якщо показ середнього вимкнено
        ...
        var starH = 20 + RatingsSettings.iconOffsetPx;
        if (starH < 1) starH = 1;

        avgElement.find('.source--name').html(
            iconImg(ICONS.total_star, 'AVG', starH)
        );
    ...
    (function registerSettingsMenu() {
        // сам розділ у списку налаштувань
        Lampa.SettingsApi.addComponent({
            component: "ratings_plugin",
            name: 'Рейтинги',
            // контурна зірка (stroke), як ти просив
            icon:
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                ...
                description:
                    'Введи число. 0 за замовчуванням. ' +
                    '1 збільшує на +1px логотипи; -1 зменшує на -1px.'
            },
            onChange: function () {
                updateDynamicStyles();
                Lampa.Settings.update();
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                ...
                description:
                    'Введи число. 0 за замовчуванням. ' +
                    '1 збільшує на +0.1em до відступу між блоками; -1 зменшує на -0.1em.'
            },
            onChange: function () {
                updateDynamicStyles();
                Lampa.Settings.update();
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                ...
                description:
                    'Введи число. 0 за замовчуванням. ' +
                    '1 збільшує блок на +1px до висоти/ширини, -1 зменшує на -1px.'
            },
            onChange: function () {
                updateDynamicStyles();
                Lampa.Settings.update();
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                ...
                description:
                    'Введи число від 0 до 1. ' +
                    '0 = повністю прозорий, 1 = непрозорий. ' +
                    'За замовчуванням зараз ' + RatingsSettings.badgeOpacity + '.'
            },
            onChange: function () {
                updateDynamicStyles();
                Lampa.Settings.update();
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name: 'Тон фону',
                description:
                    'Введи число 0–255, де 0 = майже чорний фон, ' +
                    'більше значення — світліший фон.'
            },
            onChange: function () {
                updateDynamicStyles();
                Lampa.Settings.update();
    ...
    function startPlugin() {
        ...
        Lampa.Listener.follow('full', function (e) {
            if (e.type === 'complite') {
                // під час кожного відкриття картки
                setTimeout(function () {
