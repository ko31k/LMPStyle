(function() {
    ...
    var SK = {
        // ключі API
        OMDB:     'ratings_plugin_omdb_key',
        MDBLIST:  'ratings_plugin_mdblist_key',

        // перемикачі
        MONO:        'ratings_plugin_monochrome',
        SHOWAWARDS:  'ratings_plugin_showawards',
        SHOWAVERAGE: 'ratings_plugin_showaverage',

        // стилі
        LOGO_SIZE_OFFSET:   'ratings_plugin_logo_size_offset',   // 0 => стандарт; 1 => +1px; -1 => -1px
        MARGIN_OFFSET:      'ratings_plugin_margin_offset',      // 0 => 0.3em; кожен крок = +0.1em
        BADGE_SIZE_OFFSET:  'ratings_plugin_badge_size_offset',  // 0 => стандарт; крок = 1px (до padding)
        BG_ALPHA:           'ratings_plugin_bg_alpha',           // 0..1 (дефолт 0.6)
        BG_TONE:            'ratings_plugin_bg_tone'             // 0..255 (дефолт 0 = чорний)
    };

    // дефолти для стилів
    var DEFAULTS = {
        MARGIN_EM: 0.3,         // базовий відступ між бейджами (em)
        MARGIN_STEP: 0.1,       // крок для 6.2
        LOGO_SIZE_OFF: 0,       // дефолт 0 px
        BADGE_SIZE_OFF: 0,      // дефолт 0 px
        BG_ALPHA: 0.6,          // ← 6.4 дефолт (і це вказано в описі)
        BG_TONE: 0              // ← 6.5 дефолт: чорний (0..255)
    };
     ...
     */
    Lampa.Lang.add({
        oscars_label: { ru:'Оскары', en:'Oscars', uk:'Оскар' },
        emmy_label:   { ru:'Эмми',   en:'Emmy',   uk:'Еммі'  },
        awards_other_label: { ru:'Награды', en:'Awards', uk:'Нагороди' },
        popcorn_label:      { ru:'Зрители', en:'Audience', uk:'Глядачі' },
    ...
    /**
     * =========================
     *  CSS: базові стилі + динамічні
     * =========================
     */
    var pluginStyles = "<style id='lmp_enh_base_styles'>" +
        ...
        ".loading-dots__dot:nth-child(1){animation-delay:-.32s}.loading-dots__dot:nth-child(2){animation-delay:-.16s}" +
        "@keyframes loading-dots-bounce{0%,80%,100%{transform:translateY(0);opacity:.6}40%{transform:translateY(-.5em);opacity:1}}" +

        // золотий колір у кольоровому режимі
        ".rate--oscars,.rate--emmy,.rate--awards,.rate--gold{color:gold}" +

        // мокрохром: глушимо кольори
        "body.lmp-enh--mono .rate--oscars,body.lmp-enh--mono .rate--emmy,body.lmp-enh--mono .rate--awards,body.lmp-enh--mono .rate--gold," +
        "body.lmp-enh--mono .rating--green,body.lmp-enh--mono .rating--lime,body.lmp-enh--mono .rating--orange,body.lmp-enh--mono .rating--red," +
    ...
    // Динамічний стиль (оновлюється з налаштувань)
    function ensureDynamicStyle() {
        var id = 'lmp_enh_dynamic_styles';
    ...
    function applyDynamicStyle() {
        var el = ensureDynamicStyle();

        var logoOff  = parseInt(Lampa.Storage.get(SK.LOGO_SIZE_OFFSET) || DEFAULTS.LOGO_SIZE_OFF, 10);
        if (isNaN(logoOff)) logoOff = DEFAULTS.LOGO_SIZE_OFF;

        var marginOff = parseInt(Lampa.Storage.get(SK.MARGIN_OFFSET) || 0, 10);
        if (isNaN(marginOff)) marginOff = 0;
        var marginEm = (DEFAULTS.MARGIN_EM + marginOff * DEFAULTS.MARGIN_STEP).toFixed(3);

        var badgeOff = parseInt(Lampa.Storage.get(SK.BADGE_SIZE_OFFSET) || DEFAULTS.BADGE_SIZE_OFF, 10);
        if (isNaN(badgeOff)) badgeOff = DEFAULTS.BADGE_SIZE_OFF;

        var alpha = Lampa.Storage.get(SK.BG_ALPHA);
        alpha = (alpha === undefined || alpha === null || alpha === '') ? DEFAULTS.BG_ALPHA : parseFloat(alpha);
        if (isNaN(alpha)) alpha = DEFAULTS.BG_ALPHA;

        var tone = Lampa.Storage.get(SK.BG_TONE);
        tone = (tone === undefined || tone === null || tone === '') ? DEFAULTS.BG_TONE : parseInt(tone, 10);
        if (isNaN(tone)) tone = DEFAULTS.BG_TONE;
        tone = Math.max(0, Math.min(255, tone));

        // Пояснення:
        // 1) margin-right для .full-start__rate
        // 2) padding для .full-start__rate — додаємо badgeOff px до існуючих значень (мінімально інвазивно)
        // 3) фон для .full-start__rate — rgba(tone,tone,tone,alpha), щоб контролювати прозорість + тон.
        //
        // Примітка: Lampa має свої падінги. Тут ми ставимо свої calc() з невеликим базовим значенням,
        //           але при badgeOff = 0 вигляд лишається максимально близьким до стандарту.

        el.textContent =
            ".full-start-new__rate-line .full-start__rate{margin-right:" + marginEm + "em!important}" +
            ".full-start-new__rate-line .full-start__rate:last-child{margin-right:0!important}" +
            ".full-start__rate{padding: calc(0.35em + " + badgeOff + "px) calc(0.5em + " + badgeOff + "px) !important;" +
            "background-color: rgba(" + tone + "," + tone + "," + tone + "," + alpha + ") !important;" +
            "border-radius: .35em !important;}";

        // застосувати/зняти монохром
        var mono = !!Lampa.Storage.get(SK.MONO);
        $('body').toggleClass('lmp-enh--mono', mono);

        // зберегти поточне відхилення іконок (для iconImg)
        CURRENT_LOGO_OFFSET = logoOff;
    }
     ...
     */
    function getOmdbKey() {
        var v = Lampa.Storage.get(SK.OMDB);
        return (typeof v === 'string') ? v.trim() : (LMP_ENH_CONFIG.apiKeys.omdb || '');
    }
    function getMdbListKey() {
        var v = Lampa.Storage.get(SK.MDBLIST);
        return (typeof v === 'string') ? v.trim() : (LMP_ENH_CONFIG.apiKeys.mdblist || '');
    }
    function getShowAwards() {
        var v = Lampa.Storage.get(SK.SHOWAWARDS);
        if (typeof v === 'undefined' || v === null) return true; // дефолт: так
        return !!v;
    }
    function getShowAverage() {
        var v = Lampa.Storage.get(SK.SHOWAVERAGE);
        if (typeof v === 'undefined' || v === null) return true; // дефолт: так
        return !!v;
    }

    // застосувати стилі при завантаженні
    var CURRENT_LOGO_OFFSET = DEFAULTS.LOGO_SIZE_OFF;
    applyDynamicStyle();

    /**
     * =========================
     *  Допоміжні генератори іконок
     * =========================
     */
    function iconImg(url, alt, baseSizePx, extraStyle) {
        var filter = (Lampa.Storage.get(SK.MONO) ? 'filter:grayscale(100%);' : '');
        var size = baseSizePx + (CURRENT_LOGO_OFFSET || 0); // 6.1
        if (size < 1) size = 1; // не даємо впасти в 0

        return '<img style="' +
            'height:' + size + 'px; width:auto; display:inline-block; vertical-align:middle; ' +
            'object-fit:contain; ' +
            (extraStyle || '') + ' ' +
            filter + '" ' +
            'src="' + url + '" alt="' + (alt || '') + '">';
    ...
    // Oscar статуетка (з твого прикладу)
    function oscarIconInline() {
        ...
        return '<span style="' +
            ...
            filter + '">' +
            '<img src="data:image/svg+xml;base64,. NTkyIC0xLjA3LDAgMCwxLjUyNCBjIDAsMCAtNy4zMSwtMC4wMDUgLTguNTY1LDAgMCwwIDAuNjgsMi4xNTkgLTEuNTIzLDMuMDI3IDAuMDA4LDEuMSAwLDIuNzE5IDAsMi43MTkgbCAtMS41NjksMCAwLDIuMzUzIGMgMTMuMjIxNzAzLDAgMjYuODM3OTA3LDAgMzguMTg2LDAg  style=\"display:inline;opacity:1;fill:#ffcc00\" />" +
            "</span>";
    }
    ...
    /**
     * Парсимо текст нагород OMDb → скільки виграно
     */
    ...
    function fetchMdbListRatings(card, callback) {
        ...
        $.ajax({ url: url, method: 'GET', timeout: 0 })
    ...
    /**
     * Оновити приховані елементи (PG / IMDb / TMDB)
     */
    function updateHiddenElements(data) {
        ...
        if (pgElement.length && data.ageRating) {
            ...
            if (isValid) {
                ...
                var localized = map[data.ageRating] || data.ageRating;
        ...
        if (imdbContainer.length) {
            ...
            } else imdbContainer.addClass('hide');
        }
    ...
    /**
     * Додати нові бейджі (Metacritic, RT, Popcorn, Awards)
     */
    function insertRatings(data) {
        ...
        if (data.mc_display && !$('.rate--mc', rateLine).length) {
            var mcVal = (data.mc_for_avg && !isNaN(data.mc_for_avg)) ? parseFloat(data.mc_for_avg)
                        : (!isNaN(parseFloat(data.mc_display)) ? parseFloat(data.mc_display) : null);
            var mcText = (mcVal !== null && !isNaN(mcVal)) ? mcVal.toFixed(1) : data.mc_display;
        ...
        // Нагороди — показувати лише якщо увімкнено в меню
        if (getShowAwards()) {
    ...
    /**
     * Середній рейтинг (тільки якщо увімкнено)
     */
    ...
    function fetchAdditionalRatings(card) {
        ...
        function proceedWithImdbId() {
            ...
            if (cached) { currentRatingsData = cached; renderAll(); return; }
    ...
    (function registerSettingsMenu() {
        ...
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: { name: SK.OMDB, type: "input", placeholder: '', values: '' },
            field: { name: 'API ключ (OMDb)', description: Lampa.Lang.translate('ratings_plugin_omdb_desc') },
        ...
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: { name: SK.MDBLIST, type: "input", placeholder: '', values: '' },
            field: { name: 'API ключ (MDBList)', description: Lampa.Lang.translate('ratings_plugin_mdblist_desc') },
            onChange: function(){ Lampa.Settings.update(); }
        });
        ...
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: { name: SK.MONO, type: "trigger", "default": false },
            field: { name: 'Ч/Б логотипи', description: Lampa.Lang.translate('ratings_plugin_mono_desc') },
            onChange: function(){ applyDynamicStyle(); Lampa.Settings.update(); }
        });

        // 4. Нагороди (так/ні)
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: { name: SK.SHOWAWARDS, type: "trigger", "default": true },
            field: { name: 'Нагороди', description: Lampa.Lang.translate('ratings_plugin_awards_desc') },
            onChange: function(){ Lampa.Settings.update(); }
        });
        ...
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: { name: SK.SHOWAVERAGE, type: "trigger", "default": true },
            field: { name: 'Середній рейтинг', description: Lampa.Lang.translate('ratings_plugin_average_desc') },
            onChange: function(){ Lampa.Settings.update(); }
        });
        ...
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: { name: SK.LOGO_SIZE_OFFSET, type: "input", placeholder: '0', values: '' },
            field: {
                name: 'Розмір логотипів',
                description: 'Введи число. 0 за замовчуванням, 1 збільшує на +1px логотипи; -1 зменшує на -1px;'
            },
            onChange: function(){ applyDynamicStyle(); Lampa.Settings.update(); }
        });

        // 6.2 Відступ між рейтингами
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: { name: SK.MARGIN_OFFSET, type: "input", placeholder: '0', values: '' },
            field: {
                name: 'Відступ між рейтингами',
                description: 'Введи число. 0 за замовчуванням, 1 збільшує на +0.1em до відступу між блоками; -1 зменшує на -0.1em;'
            },
            onChange: function(){ applyDynamicStyle(); Lampa.Settings.update(); }
        });
        ...
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: { name: SK.BADGE_SIZE_OFFSET, type: "input", placeholder: '0', values: '' },
            field: {
                name: 'Розмір фону під логотипи',
                description: 'Введи число. 0 за замовчуванням, 1 збільшує блок на +1px до висоти/ширини, -1 зменшує на -1px;'
            },
            onChange: function(){ applyDynamicStyle(); Lampa.Settings.update(); }
        });

        // 6.4 Прозорість фону (дефолт 0.6)
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: { name: SK.BG_ALPHA, type: "input", placeholder: String(DEFAULTS.BG_ALPHA), values: '' },
            field: {
                name: 'Прозорість фону',
                description: 'Введи число від 0 до 1. 0 за замовчуванням 0.6 (0 — повністю прозорий, 1.0 — непрозорий).'
            },
            onChange: function(){ applyDynamicStyle(); Lampa.Settings.update(); }
        });
        ...
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: { name: SK.BG_TONE, type: "input", placeholder: String(DEFAULTS.BG_TONE), values: '' },
            field: {
                name: 'Тон фону',
                description: 'Введи число 0–255, де 0 = майже чорний фон, більше значення — більш світліший фон.'
            },
            onChange: function(){ applyDynamicStyle(); Lampa.Settings.update(); }
        });
        ...
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: { name: 'ratings_plugin_info', type: "info" },
            field: { name: 'Про плагін', description: Lampa.Lang.translate('ratings_plugin_about_desc') },
            onChange: function(){}
        });
    })();
    ...
    // первинне застосування моно
    if (Lampa.Storage.get(SK.MONO)) $('body').addClass('lmp-enh--mono');
