(function () {
    ...
    /********************************************************************
     * Ratings Plugin (оновлена повна версія)
     *
     * - Окремий розділ "Рейтинги" в налаштуваннях Lampa
     * - Ключі OMDb / MDBList
     * - Ч/Б логотипи
     * - Нагороди (за замовчуванням УВІМКНЕНО)
     * - Середній рейтинг (за замовчуванням УВІМКНЕНО)
     * - Кнопка "Очистити кеш рейтингів" з повідомленням
     * - Відображення рейтингів, нагород і середнього рейтингу (ТВІЙ ОРИГІНАЛ)
     * - Нові візуальні налаштування:
     *     Розмір логотипів
     *     Розмір фону під логотипи
     *     Відступ між рейтингами
     *     Прозорість фону
     *     Тон фону
     ********************************************************************/
    ...
    var STORAGE_KEY_CLEARCACHE       = 'ratings_plugin_clearcache'; // службовий прапорець (кнопка очистки кешу)

    // Нові візуальні налаштування
    var STORAGE_KEY_ICONOFFSET       = 'ratings_plugin_icon_offset';        // px, +N до висоти іконок
    var STORAGE_KEY_BADGEOFFSET      = 'ratings_plugin_badge_offset';       // px, +N до паддінгу фону бейджа
    var STORAGE_KEY_GAPOFFSET        = 'ratings_plugin_gap_offset';         // кроки 0.1em, 0 -> 0.3em базово
    var STORAGE_KEY_BGALPHAOFFSET    = 'ratings_plugin_bg_alpha_offset';    // кроки прозорості фону
    var STORAGE_KEY_BGTONEOFFSET     = 'ratings_plugin_bg_tone_offset';     // кроки тону фону (0 чорний, 1 темно-сірий...)
    ...
    var RatingsSettings = {
        ...
        // нові візуальні налаштування
        get iconOffset() {
            var v = parseFloat(loadFromStorage(STORAGE_KEY_ICONOFFSET, 0));
            return isNaN(v) ? 0 : v; // у px, додати до базової висоти іконки
        },
        get badgeOffset() {
            var v = parseFloat(loadFromStorage(STORAGE_KEY_BADGEOFFSET, 0));
            return isNaN(v) ? 0 : v; // у px, додати до внутрішнього паддінгу бейджа
        },
        get gapOffset() {
            var v = parseFloat(loadFromStorage(STORAGE_KEY_GAPOFFSET, 0));
            return isNaN(v) ? 0 : v; // кроки 0.1em, де 0 -> базово
        },
        get bgAlphaOffset() {
            var v = parseFloat(loadFromStorage(STORAGE_KEY_BGALPHAOFFSET, 0));
            return isNaN(v) ? 0 : v; // +0.1 до альфи за крок
        },
        get bgToneOffset() {
            var v = parseFloat(loadFromStorage(STORAGE_KEY_BGTONEOFFSET, 0));
            return isNaN(v) ? 0 : v; // *20 до rgb каналів
        }
    };
    ...
    /*******************************
     * ЗАСТОСУВАТИ ВІЗУАЛЬНІ НАЛАШТУВАННЯ (CSS custom properties)
     *
     * - --ratings-plugin-icon-offset:        додаткові px до висоти іконок
     * - --ratings-plugin-badge-padding-extra: додаткові px до паддінгу капсул
     * - --ratings-plugin-gap-offset:        додаткові em до відступу між бейджами
     * - --ratings-plugin-badge-bg-alpha:    прозорість фону бейджа
     * - --ratings-plugin-badge-bg-r/g/b:    тон (наскільки сірий фон)
     *******************************/
    function applyVisualTweaks() {
        var bodyEl = document.body;
        if (!bodyEl) return;

        // 1. Зміна розміру логотипів
        var iconOff = RatingsSettings.iconOffset; // px
        bodyEl.style.setProperty('--ratings-plugin-icon-offset', iconOff + 'px');

        // 2. Розмір фону під логотипи (додаємо до паддінгу білду)
        var badgeOff = RatingsSettings.badgeOffset; // px
        bodyEl.style.setProperty('--ratings-plugin-badge-padding-extra', badgeOff + 'px');

        // 3. Відступ між рейтингами
        //   базово margin-right ~0.3em
        //   кожна одиниця додає ~0.1em
        var gapUnits = RatingsSettings.gapOffset;
        var gapEm = 0.1 * gapUnits; // наприклад 1 -> 0.1em
        bodyEl.style.setProperty('--ratings-plugin-gap-offset', gapEm + 'em');

        // 4. Прозорість фону
        //   базово alpha ~0.6
        //   кожен крок +0.1
        //   з обмеженням [0.1 .. 0.9]
        var alphaOffset = RatingsSettings.bgAlphaOffset;
        var alpha = 0.6 + 0.1 * alphaOffset;
        if (alpha < 0.1) alpha = 0.1;
        if (alpha > 0.9) alpha = 0.9;
        bodyEl.style.setProperty('--ratings-plugin-badge-bg-alpha', String(alpha));

        // 5. Тон фону
        //   базово чорний rgb(0,0,0)
        //   кожен крок -> +20 до r,g,b
        //   обмежимо до 160, щоб не ставало зовсім сіре-сіре
        var toneStep = RatingsSettings.bgToneOffset * 20; // 0->0, 1->20 ...
        if (toneStep < 0) toneStep = 0;
        if (toneStep > 160) toneStep = 160;
        bodyEl.style.setProperty('--ratings-plugin-badge-bg-r', String(toneStep));
        bodyEl.style.setProperty('--ratings-plugin-badge-bg-g', String(toneStep));
        bodyEl.style.setProperty('--ratings-plugin-badge-bg-b', String(toneStep));
    }
    ...
    Lampa.Lang.add({
        ...
        // описи нових візуальних налаштувань
        ratings_plugin_iconsize_desc: {
            uk: "Розмір логотипів у бейджах (у px). 0 = стандартно. 1 → трохи більше (+1px), 2 → +2px і т.д.",
            en: "Icon size offset in badges (px). 0 = default. 1 → +1px, 2 → +2px, etc.",
            ru: "Размер логотипов (px). 0 = стандарт. 1 → +1px, 2 → +2px и т.д."
        },
        ratings_plugin_badgesize_desc: {
            uk: "Розмір фону під логотипи (padding у px). 0 = стандарт. 1 → +1px до паддінгу, 2 → +2px...",
            en: "Badge background padding (px). 0 = default. 1 → +1px padding, 2 → +2px...",
            ru: "Размер фона под логотипы (padding, px). 0 = стандарт. 1 → +1px, 2 → +2px..."
        },
        ratings_plugin_gap_desc: {
            uk: "Відступ між рейтингами. 0 = стандарт (~0.3em), 1 → ~0.4em, 2 → ~0.5em.",
            en: "Gap between rating badges. 0 = ~0.3em, 1 → ~0.4em, 2 → ~0.5em.",
            ru: "Отступ между плашками. 0 = ~0.3em, 1 → ~0.4em, 2 → ~0.5em."
        },
        ratings_plugin_bgalpha_desc: {
            uk: "Прозорість фону бейджів. 0 ~ 0.6 (стандарт), 1 стає менш прозорим (~0.7), -1 прозоріше (~0.5).",
            en: "Badge background opacity. 0 ~0.6, 1 is denser (~0.7), -1 is more transparent (~0.5).",
            ru: "Прозрачность фона. 0 ~0.6, 1 плотнее (~0.7), -1 прозрачнее (~0.5)."
        },
        ratings_plugin_bgtone_desc: {
            uk: "Тон фону (чорний → сіріший). 0 чорний, 1 темно-сірий, 2 світліший тощо.",
            en: "Badge background tone (black → more gray). 0 black, 1 dark gray, 2 lighter, etc.",
            ru: "Тон фона (чёрный → светлее). 0 чёрный, 1 тёмно-серый, 2 светлее и т.д."
        }
    });
     ...
     * ІКОНКА ЕММІ
     *
     * ВСТАВ СВОЮ ПОВНУ SVG СТАТУЕТКУ В ЦЮ ЗМІННУ (рядок).
     * Можеш просто скопіювати той величезний var emmy_svg = '<svg ...>...</svg>';
     * Якщо залишити пустим, у бейджі буде текст "Emmy".
     *******************************/
    ...
    function emmyIconInline() {
        ...
        // висота іконки базово 16px + налаштування користувача
        var baseStyleSpan =
            'height:calc(16px + var(--ratings-plugin-icon-offset,0px)); ' +
            'width:auto; display:inline-block; vertical-align:middle; ' +
            'object-fit:contain; transform:scale(1.2); transform-origin:center; ' +
            filter;

        if (emmy_svg && emmy_svg.trim().length) {
            return '' +
                '<span class="ratings-plugin-icon" style="' + baseStyleSpan + '">' +
                    emmy_svg +
                '</span>';
        } else {
            // запасний варіант якщо SVG поки не вставив
            return '' +
                '<span class="ratings-plugin-icon" style="' + baseStyleSpan +
                'color:gold;font-weight:bold;font-size:0.8em;line-height:1;">Emmy</span>';
        }
    }
     ...
     * ІКОНКА ОСКАР
     *
     * Тут уже вшитий base64. Якщо захочеш замінити – просто заміни base64.
     * Якщо хочеш вставити свій SVG вручну як у emmy_svg,
     * можеш переробити аналогічно.
     *******************************/
     ...
     * ТУТ підтягуємо іконку як <img>, з урахуванням:
     *  - Ч/Б режиму
     *  - додаткового розміру (--ratings-plugin-icon-offset)
     *******************************/
    function iconImg(url, alt, basePx, extraStyle) {
        var filter = RatingsSettings.monochrome ? 'filter:grayscale(100%);' : '';
        return '<img style="' +
            'height:calc(' + basePx + 'px + var(--ratings-plugin-icon-offset,0px)); ' +
            'width:auto; display:inline-block; vertical-align:middle; ' +
            'object-fit:contain; ' +
            (extraStyle || '') + ' ' +
            filter + '" ' +
    ...
    /*******************************
     * СТИЛІ
     * Додаємо кастомні CSS змінні для фону бейджів і т.д.
     *******************************/
    function injectStyles() {
        if (document.getElementById('ratings_plugin_styles')) return;

        var css = `
/* Значення за замовчуванням для всіх кастомних змінних */
body {
    --ratings-plugin-icon-offset: 0px;
    --ratings-plugin-badge-padding-extra: 0px;
    --ratings-plugin-gap-offset: 0em;
    --ratings-plugin-badge-bg-alpha: 0.6;
    --ratings-plugin-badge-bg-r: 0;
    --ratings-plugin-badge-bg-g: 0;
    --ratings-plugin-badge-bg-b: 0;
}

/* Лоадер ("Loading ratings") */
...
/* Стиль капсул (бейджів) у рядку рейтингів */
.full-start-new__rate-line .full-start__rate {
    /* фон: rgba(тон,прозорість), керується налаштуваннями */
    background-color: rgba(
        var(--ratings-plugin-badge-bg-r,0),
        var(--ratings-plugin-badge-bg-g,0),
        var(--ratings-plugin-badge-bg-b,0),
        var(--ratings-plugin-badge-bg-alpha,0.6)
    ) !important;

    /* паддінг: базовий + додатковий піксельний офсет */
    padding:
        calc(0.2em + var(--ratings-plugin-badge-padding-extra,0px))
        calc(0.4em + var(--ratings-plugin-badge-padding-extra,0px));

    /* відступ між бейджами:
       базово 0.3em, плюс офсет у em */
    margin-right:
        calc(0.3em + var(--ratings-plugin-gap-offset,0em)) !important;
...
`;
    ...
    function mergeRatings(mdb, omdb) {
        ...
        // Metacritic вибір пріоритетів:
        // 1) користувацький MDBList (mc_user_*)
        // 2) критики MDBList
        // 3) критики OMDb
        var mc_display = null;
        var mc_for_avg = null;
    ...
    /*******************************
     * ОНОВЛЕННЯ ІСНУЮЧИХ ЕЛЕМЕНТІВ КАРТКИ (IMDb/TMDB/PG)
     *******************************/
     ...
     * – логіка залишається як у твоєму оригіналі.
     *******************************/
    function insertRatings(data) {
        ...
        // Нагороди (prepend), тільки якщо дозволено показ
     ...
     * (робимо аналогічно до твоєї версії з addComponent)
     *******************************/
    (function registerSettingsMenu() {
        ...
        // 4. Нагороди (так/ні) - за замовчуванням Так
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
            ...
            onChange: function () {
                // перерендер наступних карток зробиться при відкритті нової
                Lampa.Settings.update();
            }
        });

        // 5. Середній рейтинг (так/ні) - за замовчуванням Так
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                ...
                description: Lampa.Lang.translate('ratings_plugin_iconsize_desc')
            },
            onChange: function () {
                applyVisualTweaks();
                Lampa.Settings.update();
        ...
        // 7. Розмір фону під логотипи
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
                name: STORAGE_KEY_BADGEOFFSET,
                type: "input",
                placeholder: '0',
                values: ''
            },
            field: {
                ...
                description: Lampa.Lang.translate('ratings_plugin_badgesize_desc')
            },
            onChange: function () {
                applyVisualTweaks();
                Lampa.Settings.update();
        ...
        // 8. Відступ між рейтингами
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
                name: STORAGE_KEY_GAPOFFSET,
                type: "input",
                placeholder: '0',
                values: ''
            },
            field: {
                ...
                description: Lampa.Lang.translate('ratings_plugin_gap_desc')
            },
            onChange: function () {
                applyVisualTweaks();
                Lampa.Settings.update();
        ...
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
                name: STORAGE_KEY_BGALPHAOFFSET,
                type: "input",
                placeholder: '0',
                values: ''
            },
            field: {
                ...
                description: Lampa.Lang.translate('ratings_plugin_bgalpha_desc')
            },
            onChange: function () {
                applyVisualTweaks();
                Lampa.Settings.update();
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name: 'Тон фону',
                description: Lampa.Lang.translate('ratings_plugin_bgtone_desc')
            },
            onChange: function () {
                applyVisualTweaks();
                Lampa.Settings.update();
        ...
        // 11. Очистити кеш рейтингів (кнопка через trigger)
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
    ...
    function startPlugin() {
        ...
        Lampa.Listener.follow('full', function (e) {
            if (e.type === 'complite') {
                // невелика затримка, щоби DOM картки встиг зібратися
                setTimeout(function () {
