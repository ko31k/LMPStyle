(function () {
    ...
    // === СТИЛІ ЯК У QUALITY+ (але вище) ===
    var style = document.createElement('style');
    style.textContent = `
    .card--season-complete {
        position: absolute;
        bottom: calc(0.50em + 1.85em); /* вище мітки якості на 1.85em */
        left: 0;
        background-color: rgba(61, 161, 141, 0.8); /* той самий колір як у Quality+ */
        z-index: 12; /* вище мітки якості */
        width: fit-content;
    ...
    .card--season-complete div {
        ...
        font-size: 1.30em; /* той самий розмір */
        color: #ffffff;
    ...
    .card--season-complete.loading {
        opacity: 0.7;
    }
    
    /* Анімація появи як у Quality+ */
    .card--season-complete {
        opacity: 0;
    ...
    function addSeriaBadge(cardEl) {
        ...
        var checkData = function(attempt) {
            ...
            badge.className = 'card--season-complete loading';
    ...
    function initPlugin() {
        ...
        setTimeout(function() {
            document.querySelectorAll('.card:not([data-season-processed])').forEach(function(card, index) {
