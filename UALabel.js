 ...
 * - Для серіалів відфільтровує релізи, що не містять ознак сезону (для уникнення плутанини з фільмами).
...
(function() {
    ...
    var LTF_CONFIG = {
        // --- Налаштування кешу ---
        CACHE_VERSION: 4, // Оновлено версію кешу через зміну логіки
        ...
        LOGGING_GENERAL: false, // Загальні логи роботи плагіна.
    ...
    var styleTracks = "<style id=\"lampa_tracks_styles\">" +
        ...
        " top: 1.95em !important; " + // Нова позиція зверху, щоб зміститися нижче рейтингу.
        "}" +
                ...
                fetchWithProxy(apiUrl, cardId, function(error, responseText) {
                    ...
                    try {
                        ...
                        for (let i = 0; i < torrents.length; i++) {
                            ...
                            // === НОВА ЛОГІКА: Перевірка на серіал ===
                            // Якщо картка, яку ми обробляємо, - це серіал...
                            if (normalizedCard.type === 'tv') {
                                ...
                                if (!/(сезон|season|s\d{1,2}|серии|\d{1,2}\s*из\s*\d{1,2})/.test(tTitle)) {
                                    // Якщо ознак немає, це, ймовірно, фільм-тезка. Пропускаємо його.
                                    if (LTF_CONFIG.LOGGING_TRACKS) {
                                        console.log(`LTF-LOG [${cardId}]: Пропускаємо реліз (схожий на фільм для картки серіалу):`, currentTorrent.title);
                                    }
