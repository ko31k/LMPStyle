(function () {
    ...
    // Поліфіл для String.prototype.startsWith (ES5)
    ...
    // Локалізація
    /*
  БЛОК: Локалізація (Lampa.Lang.add)
  ---------------------------------
  Тут визначені ключі локалізації для плагіна (ru/en/uk).
  - Змінюйте переклади в потрібних ключах для підтримки мов інтерфейсу.
  - Кожен ключ має вкладені об'єкти для різних мов: ru, en, uk.
  - Зміни в цьому блоці не впливають на логіку — лише на текст.
*/
...
Lampa.Lang.add({
    ...
    /*
  БЛОК: Налаштування за замовчуванням
  ----------------------------------
  Опис:
  - Змінна `settings` зчитує значення з Lampa.Storage (локальне сховище плагіна).
  - Кожен ключ відповідає за окрему опцію плагіна (показ міток, тема, стиль кнопок тощо).
  - Значення за умовчанням передаються другим аргументом у Lampa.Storage.get().
  - Щоб додати нову опцію — додайте ключ сюди і використайте Lampa.Storage.set для збереження.
*/
...
/* Кінець файлу — коментарі додано блоково. */
