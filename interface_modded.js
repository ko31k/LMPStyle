...
function newInfoPanel() {
    ...
    Lampa.Listener.follow('full', function(data) {
        if (data.type === 'complite' && settings.info_panel) {
            ...
            if (!data.data || !data.data.movie) {
                console.warn('No movie data available for info panel');
                return;
            }
            ...
            setTimeout(function() {
                ...
                // Зберігаємо оригінальний вміст ПЕРЕД очищенням
    ...
    // Додатковий слухач для очищення при розмонтуванні
    Lampa.Listener.follow('full', function(data) {
        if (data.type === 'unmount') {
            // Очищаем данные при размонтировании карточки
