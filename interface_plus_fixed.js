(function () {
  ...
  Lampa.Lang.add({
    ...
    interface_mod_new_all_buttons_desc:   {
      ru:'Показывает Онлайн, Торрент, Трейлеры даже если скрыты темой. Порядок: Торренти → Онлайн → Трейлери → інші.',
  ...
  function getOriginalTitleEnabled(){
    ...
    return getBool('interface_mod_new_english_data', false); // fallback для старого ключа
  }
  ...
  /* ============================================================
   *  ДОПОМІЖНІ CSS: фолбеки + пріоритет стилів
   * ============================================================ */
  ...
  function initInterfaceModSettingsUI(){
    ...
    // Додаткові налаштування (нові фічі)
    ...
    // Закриття відкритих селектів/меню — фікс «зависає»
  ...
  /* ============================================================
   *  ІНФО-ПАНЕЛЬ (4 ряди зі старої версії)
   * ============================================================ */
  function buildInfoPanel(details, movie, isTvShow, originalDetails){
    ...
    var baseGenre = { 'border-radius':'0.3em', border:'0', 'font-size':'1.0em', padding:'0.2em 0.6em', display:'inline-block', 'white-space':'nowrap', 'line-height':'1.2em', 'margin-right':'0.4ем', 'margin-bottom':'0.2em' };
  ...
  function rebuildInfoPanelActive(){
    ...
    if (!enabled) {
      ...
      ensureStylesPriority(['interface_mod_status_enabled','interface_mod_status_disabled','interface_mod_age_enabled','interface_mod_age_disabled','ifx_fallback_css','interface_mod_theme']);
    }
  }
  ...
  /* ============================================================
   *  КОЛЬОРОВІ СТАТУСИ (зі старої версії, з фолбеком)
   * ============================================================ */
  ...
  function enableStatusColoring(){
    ...
    __statusObserver = new MutationObserver(function(muts){
      ...
      muts.forEach(function(m){
        (m.addedNodes||[]).forEach(function(n){ if (n.nodeType === 1) applyStatusOnceIn(n); });
      });
    });
