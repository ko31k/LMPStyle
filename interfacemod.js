(function () {
  ...
  // Сервісна утиліта: задати inline-стиль з пріоритетом !important
  ...
  Lampa.Lang.add({
    ...
    // *** Нові опції (Enchancer / cc+) ***
    interface_mod_new_english_data:       { ru:'Английские данные', en:'English data', uk:'Англійські дані' },
    interface_mod_new_english_data_desc:  { ru:'Добавляет оригинальное название и описание при их отсутствии',
                                            en:'Adds original English title and description if missing',
                                            uk:'Додає оригінальну англ. назву та опис, якщо відсутні' },

    interface_mod_new_all_buttons:        { ru:'Все кнопки в карточке', en:'All buttons in card', uk:'Всі кнопки в картці' },
    interface_mod_new_all_buttons_desc:   { ru:'Показывает Онлайн, Торрент, Трейлеры даже если скрыты темой',
                                            en:'Show Online, Torrent, Trailers even if hidden by theme',
                                            uk:'Показує Онлайн, Торрент, Трейлери навіть якщо приховані темою' },
    ...
    interface_mod_new_button_order_desc:  { ru:'Онлайн → Торрент → Трейлери',
    ...
    interface_mod_new_colored_buttons_desc:{ ru:'Раскрашивает кнопки и обновляет иконки (как в cc+)',
  ...
  var settings = {
    ...
    // нові
    english_data    : getBool('interface_mod_new_english_data', true),      // Так
    all_buttons     : getBool('interface_mod_new_all_buttons', false),      // Ні
    icon_only       : getBool('interface_mod_new_icon_only', false),        // Ні
    button_order    : getBool('interface_mod_new_button_order', false),     // Ні
    colored_buttons : getBool('interface_mod_new_colored_buttons', false)   // Ні
  };
  ...
  (function injectBaseCss(){
    ...
    var css = `
      ...
      *{ -webkit-tap-highlight-color:transparent; -webkit-touch-callout:none; box-sizing:border-box; outline:none; -webkit-user-drag:none; }
      ...
      /* ==== Англомовний опис: гарантуємо спадкування типографіки ==== */
      .full-descr__text > .english-desc{
        font: inherit;       /* ← успадковує розмір/вагу/міжряддя від .full-descr__text */
        color: inherit;      /* ← колір як у базового опису (Enchancer-стиль) */
        margin-top: .5em;    /* легкий відступ зверху */
      }

      /* ==== Англійська оригінальна назва під головною назвою ==== */
      .ifx-eng-title{
        display:block;
        /* ↓↓↓ МІСЦЕ ДЛЯ НАЛАШТУВАННЯ ↓↓↓
           Хочеш інший розмір/колір — змінюй тут:
        */
        font-size:0.75em;   /* ← зменшено на ~чверть від базового розміру */
        color:#9aa0a6;      /* ← акуратний сірий */
        /* ↑↑↑ МІСЦЕ ДЛЯ НАЛАШТУВАННЯ ↑↑↑ */
        margin-top:.15em;
      }
      ...
      .ifx-btn-icon-only .full-start__button .button__text{
        display:none !important;
      }

      /* ==== Кольорові кнопки (cc+-стиль, акуратно) ==== */
      .ifx-colored-buttons .full-start__button{
        border-radius:.7em !important;
      }
      .ifx-colored-buttons .full-start__button.view--torrent{
        background: linear-gradient(135deg, #2c3e50, #4ca1af) !important;
        color:#fff !important;
      }
      .ifx-colored-buttons .full-start__button.view--online{
        background: linear-gradient(135deg, #1d976c, #93f9b9) !important;
        color:#0b2 !important;
      }
      .ifx-colored-buttons .full-start__button.view--trailer{
        background: linear-gradient(135deg, #c33764, #1d2671) !important;
        color:#fff !important;
      }
      /* універсальна сетка кнопок, щоб переносились акуратно */
  ...
  function initInterfaceModSettingsUI(){
    ...
    add({
      component: 'interface_mod_new',
      param: { name: 'interface_mod_new_english_data', type: 'trigger', values: true, default: true },
    ...
    // Патч Storage.set: миттєво застосовуємо зміни
    if (!window.__ifx_patch_storage) {
      ...
      Lampa.Storage.set = function(key, val){
        ...
        if (typeof key === 'string' && key.indexOf('interface_mod_new_') === 0) {
          ...
          settings.english_data    = getBool('interface_mod_new_english_data', true);
          ...
          // КНОПКИ/АНГЛ. ДАНІ — перебудувати картку
          if (
  ...
  function buildInfoPanel(details, movie, isTvShow, originalDetails){
    ...
    var movieObj = movie || {};
    if ((movieObj.number_of_seasons || (movieObj.seasons && movieObj.seasons.length)) && Array.isArray(movieObj.seasons)) {
      var totalEps = 0, airedEps = 0, now = new Date(), hasEpisodes = false;
      movieObj.seasons.forEach(function (s) {
      ...
      if (!hasEpisodes && movieObj.next_episode_to_air && movieObj.next_episode_to_air.season_number && movieObj.next_episode_to_air.episode_number) {
        var nextS = movieObj.next_episode_to_air.season_number, nextE = movieObj.next_episode_to_air.episode_number, rem = 0;
        movieObj.seasons.forEach(function (s) {
    ...
    /* 2 — Наступна серія */
    if (movieObj && movieObj.next_episode_to_air && movieObj.next_episode_to_air.air_date) {
      var nextDate = new Date(movieObj.next_episode_to_air.air_date), today = new Date();
    ...
    /* 3 — Тривалість */
    if (movieObj && !movieObj.number_of_seasons && !movieObj.seasons && movieObj.runtime > 0) {
      var mins = movieObj.runtime, h = Math.floor(mins/60), m = mins%60;
    ...
    } else if (movieObj && (movieObj.number_of_seasons || (movieObj.seasons && movieObj.seasons.length))) {
      var avg = calculateAverageEpisodeDuration(movieObj);
    ...
    /* 4 — Сезони + Жанри */
    var seasonsCount = (movieObj.season_count || movieObj.number_of_seasons || (movieObj.seasons ? movieObj.seasons.filter(function(s){return s.season_number!==0;}).length : 0)) || 0;
    if (seasonsCount > 0) {
    ...
    var genreList = [];
    if (Array.isArray(movieObj.genres) && movieObj.genres.length) {
      genreList = movieObj.genres.map(function(g){ return g.name; });
    ...
    if (enabled){
      ...
      st.textContent =
        STATUS_BASE_SEL + '{' +
          ...
          'border:1px solid transparent!important;' + /* геометрія, без видимої рамки */
  ...
  /* ============================================================
   *  КОЛЬОРОВІ СТАТУСИ (з «Незабаром/Soon»)
   * ============================================================ */
  ...
  function applyStatusOnceIn(elRoot){
    ...
    $root.find(STATUS_BASE_SEL).each(function(){
      ...
      if (!key){
        // Fallback: повертаємо видиму білу рамку
        setImp(el,'background-color','');
        setImp(el,'color','');
        setImp(el,'border','1px solid #fff');
        return;
      }
      var c = palette[key];
      setImp(el,'background-color', c.bg);
      setImp(el,'color', c.text);
  ...
  /* ============================================================
   *  КОЛЬОРОВІ ВІКОВІ РЕЙТИНГИ (PG) — з !important
   * ============================================================ */
  ...
  function applyAgeOnceIn(elRoot){
    ...
    $root.find(AGE_BASE_SEL).each(function(){
      ...
      if (!g){
        // Fallback: якщо PG не розпізнано — повернути рамку
        setImp(el,'background-color','');
        setImp(el,'color','');
        setImp(el,'border','1px solid #fff');
        return;
      ...
      setImp(el,'background-color', col[g].bg);
      setImp(el,'color', col[g].text);
      setImp(el,'border-color','transparent');
    });
  }
  ...
  /* ============================================================
   *  ENCHANCER: АНГЛІЙСЬКІ ДАНІ (оригінальна назва + англ. опис)
   * ============================================================ */
  function applyEnglishData(fullRoot, movie){
    if (!getBool('interface_mod_new_english_data', true)) return;
    var $root = $(fullRoot||document);

    // 1) Оригінальна назва — під головною назвою (як в Enchancer)
    try{
      var engTitle = (movie && (movie.original_title || movie.original_name || movie.original || movie.name || movie.title)) || '';
      if (engTitle){
        var $titleWrap = $root.find('.full-start__head, .full-start-new__head').first();
        var $titleNode = $titleWrap.find('.full-start__title, .full-start-new__title').first();
        if ($titleNode.length){
          var $exists = $titleWrap.find('.ifx-eng-title');
          if ($exists.length) $exists.text(engTitle);
          else $('<span class="ifx-eng-title"></span>').text(engTitle).insertAfter($titleNode);
        }
      }
    }catch(e){}

    // 2) Англомовний опис — тільки якщо локальний опис порожній / «Без опису.»
    try{
      var $descr = $root.find('.full-descr__text.selector, .full-descr__text').first();
      if ($descr.length){
        var textNow = $descr.clone().children().remove().end().text().trim(); // тільки текст
        var isMissing = !textNow || /^без\s+опису\.?$/i.test(textNow) || /^no\s+description\.?$/i.test(textNow);

        if (isMissing){
          // джерело 1: уже наявний .english-desc (інші плагіни/тема могли додати)
          var engNode = $root.find('.english-desc').first();
          var engText = engNode.length ? engNode.text().trim() : '';

          // джерело 2: поля з movie/повних даних, якщо раптом є
          if (!engText && movie){
            engText = movie.overview_en || movie.overview_original || movie.original_overview || '';
            if (!engText && movie.translations && movie.translations.en) {
              engText = (movie.translations.en.overview || movie.translations.en.description || '').trim();
            }
          }

          if (engText){
            // додаємо як окремий блок, наслідує типографіку .full-descr__text
            if (!$descr.find('.english-desc').length){
              $('<div class="english-desc"></div>').text(engText).appendTo($descr);
            }
          }
        }
      }
  ...
  /* ============================================================
   *  КНОПКИ (Enchancer/cc+): показати/перевпорядкувати/іконки/кольори
   * ============================================================ */

  function replaceCcPlusIcons($scope){
    // Торрент
    $scope.find('.full-start__button.view--torrent svg').replaceWith(`
    ...
    // Онлайн
    $scope.find('.full-start__button.view--online svg').replaceWith(`
    ...
    // Трейлер
    $scope.find('.full-start__button.view--trailer svg').replaceWith(`
  ...
  function rebuildButtonsOnce(fullRoot){
    var $root = $(fullRoot||document);
    ...
    // збираємо всі можливі кнопки (деякі теми кладуть додаткові в .buttons--container)
    var $extra = $root.find('.buttons--container .full-start__button');
    var $all = $container.find('.full-start__button').add($extra);

    // Уніфікуємо категорії (за класами)
    var groups = { torrent:[], online:[], trailer:[], other:[] };
    $all.each(function(){
      var $btn = $(this);
      var cls = ($btn.attr('class')||'').toLowerCase();
      if (cls.indexOf('torrent') !== -1 || cls.indexOf('view--torrent') !== -1) groups.torrent.push($btn);
      else if (cls.indexOf('online') !== -1 || cls.indexOf('view--online') !== -1) groups.online.push($btn);
      else if (cls.indexOf('trailer') !== -1 || cls.indexOf('view--trailer') !== -1) groups.trailer.push($btn);
      else groups.other.push($btn);
    });

    // «Всі кнопки в картці»: показати все + порядок Торент → Онлайн → Трейлери → інші
    // «Порядок кнопок»: примусово Онлайн → Торент → Трейлери → інші
    var order = null;
    if (settings.button_order){
      order = ['online','torrent','trailer','other'];
    } else if (settings.all_buttons){
      order = ['torrent','online','trailer','other'];
    }

    // При «всі кнопки» — переносимо і з .buttons--container
    if (settings.all_buttons){
      $container.empty();
      (order||['torrent','online','trailer','other']).forEach(function(cat){
        groups[cat].forEach(function($b){ $container.append($b); });
      });
    } else if (order){ // тільки змінюємо порядок існуючих у контейнері
      var existing = {
        torrent: $container.find('.full-start__button.view--torrent, .full-start__button.torrent').toArray().map(function(x){return $(x);}),
        online:  $container.find('.full-start__button.view--online, .full-start__button.online').toArray().map(function(x){return $(x);}),
        trailer: $container.find('.full-start__button.view--trailer, .full-start__button.trailer').toArray().map(function(x){return $(x);}),
        other:   $container.find('.full-start__button').toArray().map(function(x){return $(x);})
      };
      // прибираємо, щоб не дублювати
      var used = new Set();
      $container.empty();
      order.forEach(function(cat){
        var arr = (cat==='torrent'? existing.torrent : cat==='online'? existing.online : cat==='trailer'? existing.trailer : existing.other);
        arr.forEach(function($b){
          if (used.has($b[0])) return;
          used.add($b[0]);
          $container.append($b);
        });
      });
    ...
    // Режим «іконки без тексту»
    if (settings.icon_only) $container.addClass('ifx-btn-icon-only');
    else $container.removeClass('ifx-btn-icon-only');

    // Кольорові кнопки + оновлення іконок (cc+)
    if (settings.colored_buttons){
      $container.addClass('ifx-colored-buttons');
      replaceCcPlusIcons($container);
    } else {
      $container.removeClass('ifx-colored-buttons');
    }
  }

  // Перебудова обох: англ.дані + кнопки
  function rebuildButtonsAndEnglish(){
    var active = $('.activity--full').last();
    if (!active.length) return;
    var fullRoot = active;
    var ctx = Lampa.Activity.active();
    var movie = (ctx && ctx._object && ctx._object.params && ctx._object.params.movie) || (ctx && ctx._object && ctx._object.card && ctx._object.card.data) || {};
    applyEnglishData(fullRoot, movie);
    rebuildButtonsOnce(fullRoot);
  }

  // Слухач картки — підхоплюємо після complite
  function wireEnhancerFeatures(){
    Lampa.Listener.follow('full', function (e) {
      ...
      setTimeout(function () {
        ...
        // Англійська назва + опис
        if (settings.english_data) applyEnglishData(fullContainer, movie);
        // Кнопки
        rebuildButtonsOnce(fullContainer);
        // Контролер назад у «full_start»
        try { Lampa.Controller.toggle("full_start"); } catch(_){}
      }, 120);
    });
  }
