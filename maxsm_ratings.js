...
Или просто введите ниже в коде плагина:
    ...
    var KP_API_KEYS   = (window.RATINGS_PLUGIN_TOKENS && window.RATINGS_PLUGIN_TOKENS.KP_API_KEYS)   || ['406']; // api ключи массивом
    ...
    */
    ...
    function insertIcons(localCurrentCard, render) {
        ...
        function replaceIcon(className, svg) {
            ...
            if (showIcons) {
                // Поиск целевого элемента для замены (название источника)
                ...
                if (!target.length) {
                    var childDivs = Element.children('div');
                    if (childDivs.length >= 2) {
                        target = childDivs.eq(0); // Берем ПЕРВЫЙ div (название)
                        isSourceName = false;
                    }
                }
