...
function newInfoPanel() {
    ...
    // Змінна для відстеження поточного оброблюваного фільму
    var currentProcessingId = null;

    function processMovieDetails(movie, originalDetails) {
        if (!movie) return;
        ...
        // Видаляємо стандартні мітки про наступну серію
        ...
        tempContainer.find('span').each(function() {
            ...
            if (matchSeasons) {
                ...
                var seasonBadge = $('<span>').text(n + ' ' + plural(n, 'Сезон', 'Сезону', 'Сезонів'))
                    .css($.extend({}, baseStyle, { 'background-color': colors.seasons.bg, 'color': colors.seasons.text }));
            ...
            if (matchEpisodes) {
                ...
                var episodeBadge = $('<span>').text(n + ' ' + plural(n, 'Серія', 'Серії', 'Серій'))
                    .css($.extend({}, baseStyle, { 'background-color': colors.episodes.bg, 'color': colors.episodes.text }));
            ...
            if (genres.length > 1) {
                genres.forEach(function(genre) {
                    ...
                    var genreBadge = $('<span>').text(genre)
                        .css($.extend({}, baseStyle, { 'background-color': color.bg, 'color': color.text, 'margin': '0.2em' }));
                    thirdRow.append(genreBadge);
                });
            } else {
                var genre = text.trim();
                var color = colors.genres[genre] || { bg: 'rgba(255, 255, 255, 0.1)', text: 'white' };
        ...
        if (isTvShow && movie.seasons && Array.isArray(movie.seasons)) {
            ...
            if (totalEpisodes > 0) {
                var episodesText = totalEpisodes + ' ' + plural(totalEpisodes, 'Серія', 'Серії', 'Серій');
                ...
                var episodesBadge = $('<span>').text(episodesText)
                    .css($.extend({}, baseStyle, { 'background-color': colors.episodes.bg, 'color': colors.episodes.text }));
                firstRow.append(episodesBadge);
            }
            
            // Додаємо інформацію про наступну серію
            ...
            // Додаємо інформацію про тривалість серії
            var avgDuration = calculateAverageEpisodeDuration(movie);
            if (avgDuration > 0) {
                var durationText = 'Тривалість серії ≈ ' + formatDurationMinutes(avgDuration);
                var baseStyle = { 'border-radius': '0.3em', 'border': '0px', 'font-size': '1.3em', 'padding': '0.2em 0.6em', 'display': 'inline-block', 'white-space': 'nowrap', 'line-height': '1.2em', 'margin-right': '0.4em', 'margin-bottom': '0.2em' };
                var durationBadge = $('<span>').text(durationText)
                    .css($.extend({}, baseStyle, { 'background-color': colors.duration.bg, 'color': colors.duration.text }));
                thirdRow.append(durationBadge);
            }
        }
        ...
        if (!isTvShow && movie.runtime > 0) {
            ...
            var durationBadge = $('<span>').text(text).css({
                'border-radius': '0.3em',
                'border': '0px',
                'font-size': '1.3em',
                'padding': '0.2em 0.6em',
            ...
            secondRow.append(durationBadge);
        }
    ...
    // Слухач для обробки нових карток
    Lampa.Listener.follow('full', function(data) {
        if (data.type === 'complite' && settings.info_panel) {
            ...
            // Уникаємо повторної обробки тієї ж картки
            if (currentProcessingId === movieId) return;
            currentProcessingId = movieId;
            ...
            setTimeout(function() {
                ...
                processMovieDetails(data.data.movie, originalDetails);
            }, 100);
        }
        
        // Очищаємо при закритті картки
        if (data.type === 'close' || data.type === 'unmount') {
            currentProcessingId = null;
            $('.full-start-new__details').empty();
        }
    });
}
	// ЗМІНЕНИЙ КОД (КІНЕЦЬ!!!!!!!!!!!!!!!!!)
