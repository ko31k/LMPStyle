...
// ===== СТИЛІ ДЛЯ СТІКЕРІВ НА ПОСТЕРІ (як у Enchanser) =====
// Коментарі ліворуч пояснюють, що міняти для кастомізації
 ...
 * Можеш змінити фон, скруглення, тінь, розмір шрифта, розташування праворуч.
 */
.card__lmerating{
  position:absolute;                /* прилипає поверх постера */
  right:-0.8em;                     /* 👉 зміни відступ від правого краю */
  ...
  z-index:5;
  pointer-events:none;              /* не заважає клікам по постеру */
}
/* На планшетах — трохи менше */
@media (max-width:768px){
  .card__lmerating{ right:.2em; font-size:.74em; }
}
/* На телефонах — ще менше */
...
function renderPosterBadgesOnDetails(data, renderRoot){
  ...
  // Шукаємо постер у повній картці
...
function wirePosterBadgesListener(){
  Lampa.Listener.follow('line', function(e){
    ...
    if (!cfg.enablePosterBadges) {
      // вимкнули — знімаємо з усіх нових/існуючих
    ...
    e.items.forEach(function(it){
      ...
      // Є щось у нашому кеші — малюємо одразу
...
// Швидке оновлення/перемальовка всіх видимих карток при зміні тумблера
...
function proceedWithImdbId() {
  ...
  fetchOmdbRatings(normalizedCard, function(r2)    { omdbRes = r2 || {}; oneDone(); });
        ...
        if (!currentRatingsData) {
            ...
            // намалювати стікери на постері в деталях (якщо тумблер увімкнено)
...
t = setTimeout(function(){
  ...
  applyStylesToAll();
  // оновити/прибрати стікери на всіх видимих картках при зміні налаштувань
...
function initRatingsPluginUI(){
    ...
    // слухач для карток у списках (рендер стікерів прямо на постерах у грідах)
