(function () {
     ...
     *
     * Що він робить:
     *  - Підтягує IMDb / TMDB / Metacritic / Rotten Tomatoes / Popcorn
     *  - Витягує віковий рейтинг (PG-13 → "13+")
     *  - Витягує нагороди (Оскари, Еммі, інші перемоги)
     *  - Рахує середній рейтинг (AVG)
     *  - Показує все в одному ряду бейджів з іконками
     *  - Дає налаштування в Settings:
     *      • API ключ (OMDb)
     *      • API ключ (MDBList)
     *      • Нагороди вкл/викл
     *      • Середній рейтинг вкл/викл
     *      • Ч/Б логотипи
     *      • Розмір логотипів
     *      • Розмір числа рейтингу
     *      • Корекція розміру фону під рейтингом
     *      • Прозорість фону плиток
     *      • Яскравість плиток
     *      • Відступи між блоками
     *
     * Особливості:
     *  - Кеш TMDB→IMDb id, і кеш даних рейтингів (3 дні)
     *  - Плавний лоадер "Loading ratings"
     *  - Підтримка ч/б режиму для іконок
     ********************************************************************/
    ...
    var STORAGE_KEY_MONO            = 'ratings_plugin_monochrome';        // Ч/Б логотипи
    var STORAGE_KEY_SHOWAWARDS      = 'ratings_plugin_showawards';        // Показувати нагороди
    var STORAGE_KEY_SHOWAVERAGE     = 'ratings_plugin_showaverage';       // Показувати середній рейтинг

    // параметри кастомізації вигляду
    var STORAGE_KEY_ICON_OFFSET     = 'ratings_plugin_icon_offset_px';    // Розмір логотипів (+/- px до висоти)
    var STORAGE_KEY_FONT_SCALE      = 'ratings_plugin_font_scale_step';   // Розмір числа рейтингу (кроки масштабу)
    var STORAGE_KEY_BADGE_SIZE_OFF  = 'ratings_plugin_badge_size_offset_px'; // Корекція габариту плитки (+/-px паддінгу)
    var STORAGE_KEY_BADGE_OPACITY   = 'ratings_plugin_badge_opacity';     // Прозорість фону (0..1)
    var STORAGE_KEY_BADGE_TONE      = 'ratings_plugin_badge_tone';        // Яскравість плиток (0..255, 0 темно)
    var STORAGE_KEY_MARGIN_STEPS    = 'ratings_plugin_margin_steps';      // Відступи між блоками (кроки по 0.1em)

    /*******************************
     * КЕШ
     *******************************/
    var CACHE_TIME_MS        = 3 * 24 * 60 * 60 * 1000; // 3 дні
    var RATINGS_CACHE_KEY    = 'ratings_plugin_cache';  // рейтинги з mdblist+omdb
    var ID_MAPPING_CACHE_KEY = 'ratings_plugin_id_map'; // TMDB -> IMDb

    /*******************************
     * ЗАГОТОВКИ В STORAGE (дефолти)
     *******************************/
    ...
    ensureDefault(STORAGE_KEY_FONT_SCALE, 0);         // крок масштабу числа рейтингу
    ensureDefault(STORAGE_KEY_BADGE_SIZE_OFF, 0);     // px
    ensureDefault(STORAGE_KEY_BADGE_OPACITY, 0.6);    // alpha
    ensureDefault(STORAGE_KEY_BADGE_TONE, 0);         // 0..255 (яскравість/тон)
    ensureDefault(STORAGE_KEY_MARGIN_STEPS, 0);       // кроки 0.1em, тільки >=0
    ...
    var RatingsSettings = {
        ...
        get fontScaleStep() {
            var n = parseFloat(loadFromStorage(STORAGE_KEY_FONT_SCALE, 0));
            return isNaN(n) ? 0 : n;
        },
        ...
        get marginSteps() {
            ...
            if (isNaN(n)) n = 0;
            if (n < 0) n = 0; // тільки невід’ємні, як домовлялись
            return n;
        }
    };
     ...
     * ДИНАМІЧНІ СТИЛІ
     *
     * Тут ми формуємо вигляд наших плиток (бейджів):
     *  - один суцільний фон під числом і логотипом (не як стандартні два квадратики)
     *  - радіус, паддінги, колір тла
     *  - розмір шрифту числа рейтингу
     *  - відступ між плитками
     *
     * Важливо: ми також глушимо дефолтні внутрішні стилі Lampa для
     * .full-start__rate > div:first-child (фоновий прямокутник під числом),
     * щоб не було подвійного шару.
     *******************************/
    function updateDynamicStyles() {
        // відступ між бейджами (base 0.3em + 0.1em * кроки)
        var marginEm = 0.3 + (0.1 * RatingsSettings.marginSteps);
        if (marginEm < 0) marginEm = 0;

        // габарити плитки (padding)
        ...
        // масштаб шрифту числа рейтингу
        // базовий Lampa був ~1.45em; множимо на (1 + 0.1 * step)
        var scale     = 1 + (0.1 * RatingsSettings.fontScaleStep);
        if (scale < 0.1) scale = 0.1;
        var baseEm    = 1.45;
        var fontEm    = (baseEm * scale).toFixed(3) + 'em';

        var css =
            /* наша кастомна плитка */
            ...
            /* прибираємо оригінальну "темну коробку" тільки під числом */
            ...
            /* підпис / логотип справа від цифри */
            ...
            /* золотий колір для нагород (якщо не ч/б режим) */
            '.rate--oscars,' +
            '.rate--emmy,' +
            '.rate--awards,' +
            '.rate--gold{' +
            ...
            /* У ч/б режимі прибираємо зелений/жовтий/червоний/золото,
               і не фарбуємо текст у кастомні кольори */
    ...
    /*******************************
     * СТАТИЧНІ БАЗОВІ СТИЛІ (одноразово)
     *******************************/
    function injectBaseStyles() {
        if (document.getElementById('ratings_plugin_base_styles')) return;

        var css = `
/* дефолтний відступ між плитками до першого апдейту */
.full-start-new__rate-line .full-start__rate{
...
/* лоадер (дублюємо тут щоб було навіть до першого updateDynamicStyles) */
.loading-dots-container {
    display:flex;
    align-items:center;
    font-size:0.85em;
...
body.lmp-enh--mono .full-start__rate {
    ...
    var ICONS = {
        ...
        // це кастомна RottenTomatoes.png з GitHub (як ти казав)
    ...
    // SVG статуетка Оскар (inline base64 <img> як у тебе)
    var oscar_svg_img_base64 =
    ...
    function refreshAllIconSizesInDom() {
        ...
        $('.lmp-rating-icon').each(function () {
            ...
            var newH = baseH + RatingsSettings.iconOffsetPx;
            if (newH < 1) newH = 1;
    ...
    /*******************************
     * MDBList API (аудиторія, tomato, mc_user ...)
     *******************************/
    function fetchMdbListRatings(card, callback) {
        ...
        $.ajax({
            ...
            response.ratings.forEach(function (r) {
                ...
                // Metacritic CRITIC → перерахунок до /10
                if (src.indexOf('metacritic') !== -1 && !isUserSource(src)) {
    ...
    /*******************************
     * OMDb API (IMDb, Metacritic critics, Awards, Rated)
     *******************************/
    ...
    /*******************************
     * ЗЛИВАЄМО MDBList + OMDb
     *******************************/
    function mergeRatings(mdb, omdb) {
        mdb = mdb || {};
        omdb = omdb || {};

        // вибираємо, який Metacritic показувати
        var mc_display = null;
        var mc_for_avg = null;
    ...
    /*******************************
     * ОНОВЛЮЄМО СТАНДАРТНІ ЕЛЕМЕНТИ Lampa
     * - Віковий рейтинг (.full-start__pg)
     * - IMDb блок
     * - TMDB блок
     *******************************/
    function updateHiddenElements(data) {
        ...
        // синхронізація логотипів з новими налаштуваннями
        refreshAllIconSizesInDom();
    }

    /*******************************
     * ДОДАЄМО НАШІ ДОДАТКОВІ ПЛИТКИ:
     * - Metacritic
     * - Rotten Tomatoes
     * - Popcorn / Audience
     * - Нагороди (Оскар, Еммі, Інші)
     *******************************/
    function insertRatings(data) {
        ...
        // Нагороди (якщо дозволено показувати)
    ...
    function calculateAverageRating(data) {
        ...
        // при кожному оновленні прибираємо старий AVG
    ...
    /*******************************
     * ПОВНИЙ РЕНДЕР ДЛЯ ПОТОЧНОЇ КАРТКИ
     *******************************/
    ...
    /*******************************
     * Локалізація тултіпів для нагород
     *******************************/
    Lampa.Lang.add({
        oscars_label: {
            ru: 'Оскары',
    ...
    (function registerSettingsMenu() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name: 'API ключ (OMDb)',
                description: 'Введи ключ OMDb (omdbapi.com)'
            },
            onChange: function () {
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                ...
                description: 'Введи ключ MDBList (mdblist.com)'
            },
            onChange: function () {
                Lampa.Settings.update();
            }
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                ...
                description: 'Показувати середній рейтинг (AVG)'
            },
            onChange: function () {
                Lampa.Settings.update();
            }
        ...
        // 1. Розмір логотипів
        // "0" - стандарт. "1" -> +1px до висоти; "-1" -> -1px.
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
            ...
            field: {
                ...
                description:
                    '"0" - стандартний розмір. "1" збільшує всі логотипи на +1px; "-1" зменшує на -1px.'
            },
            onChange: function () {
                updateDynamicStyles();
                refreshAllIconSizesInDom();
        ...
        // 2. Розмір числа рейтингу
        // "0" - стандартно. "1" робить більшим, "-1" меншим.
        // Реалізація: масштабуємо font-size цілого бейджа.
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
                name: STORAGE_KEY_FONT_SCALE,
                type: "input",
                placeholder: '0',
                values: ''
            },
            field: {
                ...
                description:
                    '"0" - стандартний розмір. Додатні значення збільшують цифру, відʼємні — зменшують.'
            },
            onChange: function () {
                updateDynamicStyles();
                Lampa.Settings.update();
        ...
        // 3. Корекція розміру фону під рейтингом
        // "0" - стандарт. "1" → +1px паддінг, "-1" → -1px.
        // Використовуємо, якщо цифра або логотип "вилазять".
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
            ...
            field: {
                name: 'Корекція розміру фону під рейтингом',
                description:
                    '"0" - стандарт. "1" → більше фону, "-1" → менше. Використовуй, якщо щось вилазить.'
            },
            onChange: function () {
                updateDynamicStyles();
                Lampa.Settings.update();
        ...
        // 4. Прозорість фону під рейтингом
        // Від 0 до 1. "0" - повністю прозорий, "1" - чорний без прозорості.
        // Підказка: введи 0.6 щоб повернути стандартну (0.6).
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
            ...
            field: {
                ...
                description:
                    '"0" - повністю прозоро. "1" - зовсім не прозоро. Введи 0.6 для стандарту.'
            },
            onChange: function () {
                updateDynamicStyles();
                Lampa.Settings.update();
        ...
        // 5. Яскравість плиток рейтингу
        // "0" - стандартно темно. Більше значення ("40") → світліше.
        // Це по суті тон/яскравість фону, 0..255.
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
            ...
            field: {
                name: 'Яскравість плиток рейтингу',
                description:
                    '"0" - стандартно темно. Більше значення, напр. "40" → світліше. Діапазон 0…255.'
            },
            onChange: function () {
                updateDynamicStyles();
                Lampa.Settings.update();
        ...
        // 6. Відступи між блоками
        // "0" - стандарт. "1" -> +0.1em. Без від’ємних значень.
        Lampa.SettingsApi.addParam({
            component: "ratings_plugin",
            param: {
            ...
            field: {
                ...
                description:
                    '"0" - стандарт. "1" додає +0.1em відступу між плитками. Без мінуса.'
            },
            onChange: function () {
                updateDynamicStyles();
                Lampa.Settings.update();
    ...
    function startPlugin() {
        ...
        // одразу підтягнути актуальні стилі
        applyMonochromeClass();
        updateDynamicStyles();
        refreshAllIconSizesInDom();

        // слухаємо відкриття повної карточки
        Lampa.Listener.follow('full', function (e) {
            if (e.type === 'complite') {
                // невеличка затримка щоб DOM встиг намалювати стандартні блоки
                setTimeout(function () {
