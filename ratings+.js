 ...
 *
 * Примітне:
 * - Виправлено баг: у renderPosterBadgesFromRaw використовувався $poster → тепер $cardView
...
(function () {
  ...
  // --- localStorage shim (на випадок вимкненого сховища) -------------------
  (function () {
    var ok = true;
    try {
      var t = '__lmp_test__';
    ...
    MiniPromise.all = function (arr) {
      return new MiniPromise(function (resolve, reject) {
        ...
        for (var i = 0; i < arr.length; i++) (function (i) {
          MiniPromise.resolve(arr[i]).then(function (v) { out[i] = v; if (--left === 0) resolve(out); }, reject);
        })(i);
      });
    };
  ...
  // --- fetch polyfill (з урахуванням Lampa.Reguest для CORS) --------------
  (function (global) {
    ...
    global.fetch = function (input, init) {
      ...
      // Використовуємо Lampa.Reguest, якщо доступний
  ...
  // --- DOM поліфіли для старих WebView ------------------------------------
  (function () {
    // NodeList.forEach
  ...
  /* =========================================================================
   * 1) КОНФІГ / КОНСТАНТИ / МОВИ / CSS
   * ========================================================================= */

  var __lmpRateLineObs = null; // перенос лоадера з "фейкового" рядка у реальний

  var LMP_ENH_CONFIG = {
    apiKeys: {
      mdblist: '', // ключ MDBList
      omdb: ''     // ключ OMDb
    },
    monochromeIcons: false // сумісність (вплив іде через body-клас)
  };
  ...
  // Базові стилі плагіна (у т.ч. лоадер, кольори, іконки, mobile)
  (function injectBaseStyles() {
    var css =
      "<style>" +
      ".loading-dots-container{display:flex;align-items:center;font-size:.85em;color:#ccc;padding:.6em 1em;border-radius:.5em}" +
      ...
      ":root{--lmp-h-imdb:24px;--lmp-h-mc:24px;--lmp-h-rt:26px;--lmp-h-popcorn:26px;--lmp-h-tmdb:26px;--lmp-h-awards:20px;--lmp-h-avg:20px;--lmp-h-oscar:22px;--lmp-h-emmy:24px}" +
      ...
      ".full-start-new__rate-line .full-start__rate{margin-right:.3em!important}" +
      ".full-start-new__rate-line .full-start__rate:last-child{margin-right:0!important}" +

      ".full-start-new__rate-line.lmp-is-loading-ratings>:not(#lmp-search-loader),.full-start__rate-line.lmp-is-loading-ratings>:not(#lmp-search-loader){opacity:0!important;pointer-events:none!important;transition:opacity .15s}" +
    ...
    Lampa.Template.add('lmp_enh_styles', css);
    $('body').append(Lampa.Template.get('lmp_enh_styles', {}, true));
  })();

  // Стили для постерних бейджів (підключається одноразово за потреби)
  function ensurePosterBadgeStylesOnce() {
    if (document.getElementById('lmp-poster-badges-css')) return;
    var css = `
      .card__lmerating{
        position:absolute; right:-0.8em; padding:.40em .60em;
        background:linear-gradient(90deg,#2c2c2c,#1a1a1a);
        color:#fff; border-radius:.35em; text-transform:uppercase;
        font-size:.80em; line-height:1; box-shadow:0 2px 6px rgba(0,0,0,.35);
        z-index:5; pointer-events:none;
      }
      @media (max-width:768px){ .card__lmerating{ right:.2em; font-size:.74em } }
      @media (max-width:480px){ .card__lmerating{ right:.2em; font-size:.66em } }
    `.trim();
  ...
  // універсальний витягач значення з сирого item (MDBList)
  function lmpRawVal(it) {
    if (!it || typeof it !== 'object') return null;
    var v = it.value; if (v == null) v = it.score;
  ...
  // фільтр списку для бейджів (ховаємо без значення + TMDB; на cub ховаємо IMDb)
  ...
  // будуємо mdblist-like масив із об'єкта merged (якщо сирого нема)
  ...
  /* =========================================================================
   * 3) РЕНДЕР ПОСТЕР-БЕЙДЖІВ
   * ========================================================================= */

  function renderPosterBadgesOnDetails(data, renderRoot) {
    var cfg = getCfg();
    if (!cfg.enablePosterBadges) { removePosterBadges(renderRoot && renderRoot[0]); return; }
    ...
    items.forEach(function (it, i) {
      ...
      $b.text(String(it.source || '').toUpperCase() + ': ' + String(lmpRawVal(it)));
      $poster.append($b);
    });
  }

  // ВАЖЛИВО: тут був баг ($poster → $cardView). Виправлено.
  function renderPosterBadgesFromRaw(rawRatings, $cardView) {
    ...
    items.forEach(function (it, i) {
      ...
      $cardView.append($b); // ← виправлено
    });
  }

  // Слухач додавання карток у грідах: догружати й малювати стікери
    ...
    Lampa.Listener.follow('line', function (e) {
      ...
      if (!cfg.enablePosterBadges) {
        e.items.forEach(function (it) {
          try { it.card.find('.card__view .card__lmerating').remove(); } catch (_) { }
        });
        return;
      }
      e.items.forEach(function (it) {
        ...
        // дотягуємо лише для стікерів (дешевше)
  ...
  // Перемалювати всі видимі картки (коли змінили тумблер у налаштуваннях)
  ...
  function emmyIconInline() { return '<span class="lmp-award-icon lmp-award-icon--emmy"><img src="' + ICONS.emmy + '" alt="Emmy"></span>'; }
  function oscarIconInline() { return '<span class="lmp-award-icon lmp-award-icon--oscar"><img src="' + ICONS.oscar + '" alt="Oscar"></span>'; }

  function dimRateLine(rateLine) { if (rateLine && rateLine.length) rateLine.addClass('lmp-is-loading-ratings'); }
  function undimRateLine(rateLine) { if (rateLine && rateLine.length) rateLine.removeClass('lmp-is-loading-ratings'); }
  ...
  function addLoadingAnimation() {
    ...
    if (rateLine.length) { rateLine.append(loaderHtml); dimRateLine(rateLine); return; }

    // ставимо тимчасову смугу
    var fake = $(
      '<div class="full-start-new__rate-line" id="lmp-loader-fake" data-lmp-fake="1" style="min-height:28px;display:flex;align-items:center;"></div>'
    );
  ...
  function getImdbIdFromTmdb(tmdbId, type, callback) {
    ...
    function fromCache(key) {
      var item = cache[key]; if (!item || !item.imdb_id) return null;
    ...
    (function next() {
      ...
      makeRequest(url, function (data) {
        ...
        if (imdbId) saveAndReturn(imdbId); else next();
      }, function () { next(); });
    })();
  }
  ...
  /* =========================================================================
   * 6) OMDb: нагороди + дублюючі джерела
   * ========================================================================= */
  ...
  function fetchMdbListRatings(card, callback) {
    ...
    new Lampa.Reguest().silent(url, onSuccess, onFail);

    function onFail() {
      new Lampa.Reguest().native(
        url,
        function (data) {
          try { onSuccess(typeof data === 'string' ? JSON.parse(data) : data); }
  ...
  function mergeRatings(mdb, omdb) {
    ...
    if (mdb.mc_user_display) { mc_display = mdb.mc_user_display; mc_for_avg = mdb.mc_user_for_avg; }
    else if (mdb.mc_critic_display) { mc_display = mdb.mc_critic_display; mc_for_avg = mdb.mc_critic_for_avg; }
    else if (omdb.mc_critic_display) { mc_display = omdb.mc_critic_display; mc_for_avg = omdb.mc_critic_for_avg; }

    return {
      tmdb_display: mdb.tmdb_display || null, tmdb_for_avg: mdb.tmdb_for_avg || null,
      imdb_display: mdb.imdb_display || omdb.imdb_display || null,
      imdb_for_avg: mdb.imdb_for_avg || omdb.imdb_for_avg || null,
      mc_display: mc_display || null, mc_for_avg: (typeof mc_for_avg === 'number' ? mc_for_avg : null),
      ...
      popcorn_display: mdb.popcorn_display || null, popcorn_for_avg: mdb.popcorn_for_avg || null,
      ageRating: omdb.ageRating || null,
      oscars: omdb.oscars || 0, emmy: omdb.emmy || 0, awards: omdb.awards || 0,
      _mdblist_ratings: Array.isArray(mdb._mdblist_ratings) ? mdb._mdblist_ratings.slice() : [] // критично для постерів
    };
  }


  /* =========================================================================
   * 9) ОНОВЛЕННЯ ДЕТАЛКИ: приховані елементи + плитки джерел
   * ========================================================================= */
  ...
  function updateHiddenElements(data) {
    ...
    if (pgElement.length && data.ageRating) {
      ...
      if (invalid.indexOf(data.ageRating) === -1) {
    ...
    (function () {
      ...
      if (!imdbContainer.length) return;
      var cfg = getCfg();
    ...
    (function () {
      ...
      if (!tmdbContainer.length) return;
      var cfg = getCfg();
  ...
  // Кольори для нагород (залежно від “кольорових оцінок”)
  ...
  // Вставка/оновлення плиток джерел у рядку рейтингів
  function insertRatings(data) {
    ...
    // --- Metacritic (user → critic → fallback) ----------------------------
    (function () {
      ...
      if (!cont.length) {
        ...
        if (afterImdb.length) cont.insertAfter(afterImdb); else rateLine.append(cont);
      } else {
    ...
    (function () {
      ...
      if (data.rt_for_avg && !isNaN(data.rt_for_avg)) rtVal = parseFloat(data.rt_for_avg);
        ...
        var rtd = parseFloat(data.rt_display); rtVal = (rtd > 10) ? (rtd / 10) : rtd;
      }
    ...
    (function () {
      ...
      if (data.popcorn_for_avg && !isNaN(data.popcorn_for_avg)) pcVal = parseFloat(data.popcorn_for_avg);
        ...
        var pcd = parseFloat(data.popcorn_display); pcVal = (pcd > 10) ? (pcd / 10) : pcd;
      }
      ...
      if (!cont.length) {
        ...
        // Після останньої рейтинг-плитки або на початок
        var anchors = rateLine.find('.rate--rt, .rate--mc, .rate--tmdb, .rate--imdb');
        if (anchors.length) cont.insertAfter(anchors.last()); else rateLine.prepend(cont);
      } else {
  ...
  function calculateAverageRating(data) {
    ...
    if (!parts.length) { removeLoadingAnimation(); undimRateLine(rateLine); return; }

    var sum = 0; for (var i = 0; i < parts.length; i++) sum += parts[i];
    ...
    if (firstRate.length) firstRate.before(avgElement); else rateLine.prepend(avgElement);
  ...
  /* =========================================================================
   * 11) ОСНОВНА ЛОГІКА: тягнемо дані, мерджимо, рендеримо
   * ========================================================================= */
  ...
  function fetchAdditionalRatings(card) {
    ...
    var normalizedCard = {
      ...
      vote: card.vote_average || card.vote || null // запасний TMDB рейтинг
    };

    var reqToken = (normalizedCard.type || getCardType(normalizedCard)) + '_' + (normalizedCard.imdb_id || normalizedCard.id || '') + '_' + Date.now();
    ...
    function proceedWithImdbId() {
      ...
      addLoadingAnimation(); // показуємо тільки якщо реально будемо тягнути

      var pending = 2, mdbRes = null, omdbRes = null;
      function oneDone() {
        if (--pending !== 0) return;

        currentRatingsData = mergeRatings(mdbRes, omdbRes);

        // fallback TMDB із vote_average
        ...
        // кешуємо лише “непорожні” результати
        if (cacheKey && currentRatingsData && (
          ...
          currentRatingsData.popcorn_display || currentRatingsData.oscars ||
          currentRatingsData.emmy || currentRatingsData.awards
        )) {
    ...
    function renderAll() {
      ...
      if (!currentRatingsData) { removeLoadingAnimation(); return; }
  ...
  /* =========================================================================
   * 12) UX-ФІКС: показувати "10" замість "10.0"
   * ========================================================================= */

  (function fixTenDotZero() {
    ...
    window.__lmpTenFixStart = function () {
      try {
        var render = Lampa && Lampa.Activity && Lampa.Activity.active() &&
          Lampa.Activity.active().activity.render && Lampa.Activity.active().activity.render();
        if (!render || !render[0]) return;

        var target = render[0].querySelector('.full-start-new__rate-line, .full-start__rate-line') || render[0];
  ...
  /* =========================================================================
   * 13) ІНІЦІАЛІЗАЦІЯ ОСНОВНОГО ПЛАГІНА
   * ========================================================================= */
  ...
  /* =========================================================================
   * 14) НАЛАШТУВАННЯ / СТИЛІ / LIVE-APPLY
   * ========================================================================= */
  ...
  function getCfg() {
    ...
    badgeAlpha = Math.min(1, Math.max(0, badgeAlpha));
    ...
    badgeTone = Math.min(255, Math.max(0, badgeTone));
    ...
    return {
      omdbKey, mdblistKey, bwLogos, showAwards, showAverage,
      logoOffset, fontOffset, badgeAlpha, badgeTone, gapStep, colorizeAll,
      enableImdb: enIMDB, enableTmdb: enTMDB, enableMc: enMC, enableRt: enRT, enablePop: enPopcorn,
  ...
  function refreshConfigFromStorage() {
    ...
    if (cfg.bwLogos) $('body').addClass('lmp-enh--mono'); else $('body').removeClass('lmp-enh--mono');
    return cfg;
  }
  ...
  // масштаб логотипів (IMDb/TMDB + усі інші іконки)
  ...
  function tuneGap(gapStep) {
    ...
    lines.forEach(function (line) {
      ...
      for (var i = 0; i < kids.length; i++) kids[i].style.setProperty('margin-right', totalEm + 'em', 'important');
  ...
  // загальна стилізація (застосовується як при старті, так і при live-змінах)
  ...
  // Lampa.Storage.set патч: щоб миттєво підхоплювати стилі після зміни налаштувань
  function patchStorageSetOnce() {
    ...
    Lampa.Storage.set = function (k, v) {
      ...
      if (typeof k === 'string' && k.indexOf('ratings_') === 0) setTimeout(applyStylesToAll, 0);
      return out;
    };
  }
  ...
  var reapplyOnResize = (function () {
    var t;
    return function () { clearTimeout(t); t = setTimeout(applyStylesToAll, 150); };
  })();
  ...
  function addSettingsSection() {
    ...
    Lampa.SettingsApi.addComponent({
      ...
      icon:
        ...
        '<path d="M12 3l3.09 6.26L22 10.27l-5 4.87L18.18 21 12 17.77 5.82 21 7 15.14l-5-4.87 6.91-1.01L12 3z" stroke="currentColor" stroke-width="2" fill="none" stroke-linejoin="round" stroke-linecap="round"/>' +
        '</svg>'
    });
    ...
    // Тумблери/налаштування
    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_bw_logos', type: 'trigger', default: RCFG_DEFAULT.ratings_bw_logos }, field: { name: 'Ч/Б логотипи', description: 'Чорно-білі логотипи рейтингів' } });
    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_show_awards', type: 'trigger', default: RCFG_DEFAULT.ratings_show_awards }, field: { name: 'Нагороди', description: 'Показувати Оскари, Еммі та інші нагороди.' } });
    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_show_average', type: 'trigger', default: RCFG_DEFAULT.ratings_show_average }, field: { name: 'Середній рейтинг', description: 'Показувати середній рейтинг' } });

    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_logo_offset', type: 'input', default: RCFG_DEFAULT.ratings_logo_offset }, field: { name: 'Розмір логотипів рейтингів', description: 'Зміна висоти логотипів. \n"0" – стандарт, вводимо від "1" чи від "-1".' } });
    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_font_offset', type: 'input', default: RCFG_DEFAULT.ratings_font_offset }, field: { name: 'Розмір числа рейтингу', description: 'Зміна розміру числа рейтингу та фону. \n"0" – стандарт, вводимо від "1" чи від "-1".' } });
    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_badge_alpha', type: 'input', default: RCFG_DEFAULT.ratings_badge_alpha }, field: { name: 'Прозорість фону під рейтингом', description: 'Стандарт: "0.15". "0" – прозорий, "1" – чорний.' } });
    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_badge_tone', type: 'input', default: RCFG_DEFAULT.ratings_badge_tone }, field: { name: 'Яскравість плиток', description: '0–255 (наприклад, "50" — темно-сірий).' } });
    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_gap_step', type: 'input', default: RCFG_DEFAULT.ratings_gap_step }, field: { name: 'Відступи між блоками (крок)', description: 'Додатковий відступ. "0" – стандарт (0.3em). \nЩоб збільшити — від "1".' } });

    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_colorize_all', type: 'trigger', default: RCFG_DEFAULT.ratings_colorize_all }, field: { name: 'Кольорові оцінки рейтингів', description: 'Кольорове виділення оцінок' } });

    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_enable_imdb', type: 'trigger', default: RCFG_DEFAULT.ratings_enable_imdb }, field: { name: 'IMDb', description: 'Показувати/ховати джерело' } });
    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_enable_tmdb', type: 'trigger', default: RCFG_DEFAULT.ratings_enable_tmdb }, field: { name: 'TMDB', description: 'Показувати/ховати джерело' } });
    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_enable_mc', type: 'trigger', default: RCFG_DEFAULT.ratings_enable_mc }, field: { name: 'Metacritic', description: 'Показувати/ховати джерело' } });
    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_enable_rt', type: 'trigger', default: RCFG_DEFAULT.ratings_enable_rt }, field: { name: 'RottenTomatoes', description: 'Показувати/ховати джерело' } });
    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_enable_popcorn', type: 'trigger', default: RCFG_DEFAULT.ratings_enable_popcorn }, field: { name: 'Popcornmeter', description: 'Показувати/ховати джерело' } });

    Lampa.SettingsApi.addParam({ component: 'lmp_ratings', param: { name: 'ratings_poster_badges', type: 'trigger', default: RCFG_DEFAULT.ratings_poster_badges }, field: { name: 'Додаткові рейтинги на постері', description: 'Показувати на постері доступні рейтинги із MDBList' } });
  }
