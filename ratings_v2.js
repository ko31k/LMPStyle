...
// ------------------------------------------------------------END--JacRed-------------------------------------------------------------------------     
    ...
    getBestReleaseFromJacred(normalizedCard, localCurrentCard, function(jrResult) {
        ...
        // 1️⃣ Беремо JacRed якість
        var quality = (jrResult && jrResult.quality) || null;

        // 2️⃣ Якщо немає JacRed — fallback на TMDB
        if (!quality && localCurrentCard.data && localCurrentCard.data.video_quality) {
            quality = localCurrentCard.data.video_quality;
            if (Q_LOGGING) console.log('MAXSM-RATINGS', ' card: ' + localCurrentCard + ', quality fallback to TMDB: ' + quality);
        }

        // 3️⃣ Присвоюємо дані для бейджа
        if (quality) localCurrentCard.data.quality = quality;
