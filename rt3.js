(function() {
    ...
    var LMP_ENH_CONFIG = {
        apiKeys: {
            mdblist: 'm8po461k1zq14sroj2ez5d7um',
            omdb:    '12c9249c'
        },
        monochromeIcons: false // Це значення тепер керується з меню
    };
...
im
        ...
        /* * ВІДСТУПИ
         * [ВИПРАВЛЕНО] Прибрано !important, щоб JavaScript міг ним керувати
        */
        ".full-start-new__rate-line .full-start__rate {" +
        "    margin-right: 0.3em;" +
        "}" +
        ".full-start-new__rate-line .full-start__rate:last-child {" +
        "    margin-right: 0 !important;" + /* !important тут потрібен, щоб перебити inline-стиль */
        "}" +
     ...
     * =========================
     * Генератори іконок
     * [ВИПРАВЛЕНО] Всі filter:grayscale() прибрані.
     * Цим тепер керує applyBwLogos()
     * =========================
     */
    ...
    // [ВИПРАВЛЕНО] Прибрано transform:scale, висота 18px
    ...
    // [ВИПРАВЛЕНО] Прибрано transform:scale
    ...
    function getCachedRatings(key) {
        ...
        var item = cache[key];
        if (!item || (Date.now() - item.timestamp > CACHE_TIME)) return null;
    ...
    function fetchMdbListRatings(card, callback) {
        ...
        $.ajax({
            ...
            function isUserSource(src) {
                return (src.indexOf('user') !== -1 || src.indexOf('metacriticuser') !== -1);
            }
    ...
    function fetchOmdbRatings(card, callback) {
        ...
        new Lampa.Reguest().silent(url, function(data) {
            if (!data || data.Response !== 'True') return callback(null);
            ...
            if (Array.isArray(data.Ratings)) {
                data.Ratings.forEach(function(r) {
                    if (r.Source === 'Rotten Tomatoes') rtScore = parseInt((r.Value || '').replace('%',''));
                    if (r.Source === 'Metacritic') mcScore = parseInt((r.Value || '').split('/')[0]);
                });
            }
    ...
    function updateHiddenElements(data) {
        ...
        if (pgElement.length && data.ageRating && ['N/A', 'Not Rated', 'Unrated'].indexOf(data.ageRating) === -1) {
            pgElement.removeClass('hide').text(AGE_RATINGS[data.ageRating] || data.ageRating);
        }
        ...
        if (tmdbContainer.length && data.tmdb_display) {
    ...
    function insertRatings(data) {
        ...
        if (data.mc_display && !$('.rate--mc', rateLine).length) {
            var mcVal = data.mc_for_avg ? parseFloat(data.mc_for_avg) : parseFloat(data.mc_display);
            var mcText = (mcVal !== null && !isNaN(mcVal)) ? mcVal.toFixed(1) : data.mc_display;
            var mcElement = $('<div class="full-start__rate rate--mc"><div>' + mcText + '</div><div class="source--name"></div></div>');
        ...
        if (data.rt_display && !$('.rate--rt', rateLine).length) {
            ...
            var rtElement = $('<div class="full-start__rate rate--rt"><div>' + data.rt_display + '</div><div class="source--name"></div></div>');
        ...
        if (data.popcorn_display && !$('.rate--popcorn', rateLine).length) {
            var pcElement = $('<div class="full-start__rate rate--popcorn"><div>' + data.popcorn_display + '</div><div class="source--name"></div></div>');
    ...
    function calculateAverageRating(data) {
        ...
        var parts = [];
        if (data.tmdb_for_avg) parts.push(data.tmdb_for_avg);
        if (data.imdb_for_avg) parts.push(data.imdb_for_avg);
        if (data.mc_for_avg) parts.push(data.mc_for_avg);
        if (data.rt_for_avg) parts.push(data.rt_for_avg);
        if (data.popcorn_for_avg) parts.push(data.popcorn_for_avg);
    ...
    var RCFG_DEFAULT = {
        ...
        ratings_show_awards:   true,  // [ВИПРАВЛЕНО]
        ratings_show_average:  true,  // [ВИПРАВЛЕНО]
    ...
    function getCfg() {
        return {
            omdbKey: Lampa.Storage.get('ratings_omdb_key', RCFG_DEFAULT.ratings_omdb_key),
            mdblistKey: Lampa.Storage.get('ratings_mdblist_key', RCFG_DEFAULT.ratings_mdblist_key),
            bwLogos: !!Lampa.Storage.get('ratings_bw_logos', RCFG_DEFAULT.ratings_bw_logos),
            showAwards: !!Lampa.Storage.get('ratings_show_awards', RCFG_DEFAULT.ratings_show_awards),
            showAverage: !!Lampa.Storage.get('ratings_show_average', RCFG_DEFAULT.ratings_show_average),
            logoOffset: parseInt(Lampa.Storage.get('ratings_logo_offset', RCFG_DEFAULT.ratings_logo_offset)) || 0,
            fontOffset: parseInt(Lampa.Storage.get('ratings_font_offset', RCFG_DEFAULT.ratings_font_offset)) || 0,
            badgeAlpha: parseFloat(Lampa.Storage.get('ratings_badge_alpha', RCFG_DEFAULT.ratings_badge_alpha)) || 0.15,
            badgeTone: parseInt(Lampa.Storage.get('ratings_badge_tone', RCFG_DEFAULT.ratings_badge_tone)) || 0,
            gapStep: parseInt(Lampa.Storage.get('ratings_gap_step', RCFG_DEFAULT.ratings_gap_step)) || 0
        };
    }
    ...
    function refreshConfigFromStorage(){
        ...
        LMP_ENH_CONFIG.monochromeIcons = cfg.bwLogos; // Оновлюємо, але реально керуємо через applyBwLogos
    ...
    function tuneRatingFont(offsetPx){
        ...
        tiles.forEach(function(tile){
            if (!tile.getAttribute('data-base-fontsize')){
                var basePx = parseFloat(window.getComputedStyle(tile).fontSize) || 23;
                tile.setAttribute('data-base-fontsize', basePx);
            }
            var baseStored = parseFloat(tile.getAttribute('data-base-fontsize')) || 23;
            var finalPx = baseStored + offsetPx;
            tile.style.fontSize = (finalPx < 1 ? 1 : finalPx) + 'px';
        });
    }

    function tuneLogos(offsetPx){
        // [ВИПРАВЛЕНО] Селектор тепер захоплює ВСІ логотипи, включно з IMDb/TMDB
        var logos = document.querySelectorAll('.full-start__rate > div:nth-child(2) img, .full-start__rate > div:nth-child(2) span');
        
        logos.forEach(function(logo){
            if (!logo.getAttribute('data-base-height')){
                var ch = parseFloat(window.getComputedStyle(logo).height) || 16;
                logo.setAttribute('data-base-height', ch);
            }
            var baseH = parseFloat(logo.getAttribute('data-base-height')) || 16;
            ...
            var innerImgs = logo.querySelectorAll('img,svg');
            if (innerImgs.length){
    ...
    // [ВИПРАВЛЕНО] Враховуємо базовий відступ 0.3em з CSS
    function tuneGap(gapStep){
        var lines = document.querySelectorAll('.full-start-new__rate-line');
        var baseGap = 0.3; // База з нашого CSS
        var totalEm = (baseGap + gapStep * 0.1);
        if (totalEm < 0) totalEm = 0;

        lines.forEach(function(line){
            var kids = line.querySelectorAll('.full-start__rate');
            kids.forEach(function(kid){
                kid.style.marginRight = totalEm + 'em';
            });
        });
    }

    function applyBwLogos(enabled){
        // [ВИПРАВЛЕНО] Селектор тепер захоплює ВСІ логотипи
        var logos = document.querySelectorAll('.full-start__rate > div:nth-child(2) img, .full-start__rate > div:nth-child(2) span');
    ...
    /**
     * Головна функція стилізації — викликається ПІСЛЯ рендеру і ПРИ ЗМІНІ НАЛАШТУВАНЬ
     */
    ...
    // [ДОДАНО] Функція, що викликається при зміні будь-якого параметра в меню
    ...
    function addSettingsSection(){
        ...
        Lampa.SettingsApi.addComponent({
            ...
            name: 'Рейтинги', // [ВИПРАВЛЕНО] Прибрано зірку
            icon:
        ...
        Lampa.SettingsApi.addParam({
            component: 'lmp_ratings',
            param: { name: 'ratings_omdb_key', type: 'input', "default": RCFG_DEFAULT.ratings_omdb_key },
            field: { name: 'API ключ (OMDb)', description: 'Введи свій ключ OMDb. Можна отримати на omdbapi.com' },
            onChange: onSettingsChange // [ДОДАНО]
        });

        // --- MDBList Key ---
        Lampa.SettingsApi.addParam({
            component: 'lmp_ratings',
            param: { name: 'ratings_mdblist_key', type: 'input', "default": RCFG_DEFAULT.ratings_mdblist_key },
        ...
        Lampa.SettingsApi.addParam({
            component: 'lmp_ratings',
            param: {
                name: 'ratings_bw_logos', type: 'trigger', "default": RCFG_DEFAULT.ratings_bw_logos,
                values: {on: 'Так', off: 'Ні'} // [ВИПРАВЛЕНО]
            },
            field: { name: 'Ч/Б логотипи', description: 'Чорно-білі логотипи рейтингів' },
            onChange: onSettingsChange // [ДОДАНО]
        });
        ...
        Lampa.SettingsApi.addParam({
            component: 'lmp_ratings',
            param: {
                name: 'ratings_show_awards', type: 'trigger', "default": RCFG_DEFAULT.ratings_show_awards,
                values: {on: 'Так', off: 'Ні'} // [ВИПРАВЛЕНО]
            },
        ...
        Lampa.SettingsApi.addParam({
            component: 'lmp_ratings',
            param: {
                name: 'ratings_show_average', type: 'trigger', "default": RCFG_DEFAULT.ratings_show_average,
                values: {on: 'Так', off: 'Ні'} // [ВИПРАВЛЕНО]
            },
        ...
        Lampa.SettingsApi.addParam({
            component: 'lmp_ratings',
            param: { name: 'ratings_logo_offset', type: 'input', "default": RCFG_DEFAULT.ratings_logo_offset },
            field: { name: 'Розмір логотипів рейтингів', description: 'Зміна висоти логотипів. "0" – стандарт, "1" → +1px, "-1" → -1px.' },
            onChange: onSettingsChange // [ДОДАНО]
        });
        ...
        Lampa.SettingsApi.addParam({
            component: 'lmp_ratings',
            param: { name: 'ratings_font_offset', type: 'input', "default": RCFG_DEFAULT.ratings_font_offset },
            field: { name: 'Розмір числа рейтингу', description: 'Зміна розміру числа рейтингу. "0" – стандарт, "+1" → +1px, "-1" → -1px.' },
            onChange: onSettingsChange // [ДОДАНО]
        });
        ...
        Lampa.SettingsApi.addParam({
            component: 'lmp_ratings',
            param: { name: 'ratings_badge_alpha', type: 'input', "default": RCFG_DEFAULT.ratings_badge_alpha },
            field: { name: 'Прозорість фону під рейтингом', description: 'Прозорість фону. "0" – прозорий, "1" – чорний. Стандарт: "0.15".' },
            onChange: onSettingsChange // [ДОДАНО]
        });
        ...
        Lampa.SettingsApi.addParam({
            component: 'lmp_ratings',
            param: { name: 'ratings_badge_tone', type: 'input', "default": RCFG_DEFAULT.ratings_badge_tone },
            field: { name: 'Яскравість плиток', description: 'Яскравість плиток. "0" – чорний. Від 0 до 255 (напр. "50" - темно-сірий).' },
            onChange: onSettingsChange // [ДОДАНО]
        });
        ...
        Lampa.SettingsApi.addParam({
            component: 'lmp_ratings',
            param: { name: 'ratings_gap_step', type: 'input', "default": RCFG_DEFAULT.ratings_gap_step },
            field: { name: 'Відступи між блоками (крок)', description: 'Додатковий відступ. "0" – стандарт (0.3em). "1" → +0.1em (0.4em).' },
            onChange: onSettingsChange // [ДОДАНО]
        });
    }
    ...
    function initRatingsPluginUI(){
        ...
        // Застосувати стилі одразу при завантаженні Lampa (на випадок, якщо картка вже відкрита)
        applyStylesToAll();
    }
    ...
    //
    initRatingsPluginUI();
    refreshConfigFromStorage(); // Застосувати Ч/Б клас на body
