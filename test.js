(function () {
  ...
  // ВАЖЛИВО: коректний пріоритет ключів — новий key > старий key (fallback)
  function getOriginalTitleEnabled(){
    ...
    return getBool('interface_mod_new_english_data', false);               // старий ключ лише як fallback
  }
  ...
  (function injectBaseCss(){
    ...
    var css = `
      ...
      *{ -webkit-tap-highlight-color:transparent; -webkit-touch-callout:none; box-sizing:border-box; outline:none; -webkit-user-drag:none; }
      ...
      /* ===== ОРИГІНАЛЬНА НАЗВА: сірий текст, -25%, з лівою лінією ===== */
      .ifx-original-title{
        color:#aaa;
        font-size: 0.75em;      /* −25% */
        font-weight: 600;
  ...
  function applyTheme(theme) {
    ...
    var themeCss = {
      ...
      emerald_v2: `
        ...
        .settings__content, .settings-input__content, .selectbox__content, .modal__content {
          background: rgba(10, 24, 29, 0.98) !important; border: 1px solid rgba(38,164,131,.15) !important; border-radius: .9em !important;
        }
      `,
      aurora: `
  ...
  function initInterfaceModSettingsUI(){
    ...
    // ДОДАНО: пункт меню «Оригінальна назва»
    add({ component: 'interface_mod_new',
  ...
  function buildInfoPanel(details, movie, isTvShow, originalDetails){
    ...
    var baseBadge = {
      ...
      'line-height':'1.2ем'.replace('ем','em'), 'margin-right':'0.4em', 'margin-bottom':'0.2em'
    };
    ...
    if ((isTvShow && Array.isArray(movie.seasons))) {
      ...
      if (epsText) {
        var r1 = $('<div>');
        r1.append($('<span>').text(epsText).css($.extend({}, baseBadge, { 'background-color': colors.episodes.bg, color: colors.episodes.text })));
        container.append(r1);
      }
    }
    ...
    if (isTvShow && movie.next_episode_to_air && movie.next_episode_to_air.air_date) {
      ...
      if (txt) {
        var r2 = $('<div>');
        r2.append($('<span>').text(txt).css($.extend({}, baseBadge, { 'background-color': colors.next.bg, color: colors.next.text })));
        container.append(r2);
      }
    }
    ...
    if (!isTvShow && movie.runtime > 0) {
      ...
      if (m > 0) t += (h>0?' ':'') + m + ' хв.';
      var r3 = $('<div>');
      r3.append($('<span>').text(t).css($.extend({}, baseBadge, { 'background-color': colors.duration.bg, color: colors.duration.text })));
      container.append(r3);
    } else if (isTvShow) {
      var avg = calculateAverageEpisodeDuration(movie);
      if (avg > 0) {
        var r3b = $('<div>');
        r3b.append($('<span>').text('Тривалість серії ≈ ' + formatDurationMinutes(avg)).css($.extend({}, baseBadge, { 'background-color': colors.duration.bg, color: colors.duration.text })));
        container.append(r3b);
      }
    }
    ...
    if (isTvShow && seasonsCount > 0) {
      var r4 = $('<div>');
      r4.append($('<span>').text('Сезони: ' + seasonsCount).css($.extend({}, baseBadge, { 'background-color': 'rgba(52,152,219,0.8)', color: 'white' })));
      container.append(r4);
    }
    ...
    if (genreList.length){
      ...
      genreList.forEach(function(gn){
        ...
        r4g.append($('<span>').text(gn).css($.extend({}, baseGenre, { 'background-color': c.bg, color: c.text })));
      });
      container.append(r4g);
    }
  ...
  // ВІК: базова геометрія бейджа (за вимогою беремо реалізацію з другого шматка)
  ...
  /* ============================================================
   *  КОЛЬОРОВІ ВІКОВІ РЕЙТИНГИ (PG) — взято з другого коду
   *  Розширений OBSERVER + кілька проходів після відкриття
   * ============================================================ */
  ...
  // додаткові «проходи» після відкриття картки — щоб зловити кеш/пізні зміни
    ...
    if (!__ageFollowReady){
      ...
      Lampa.Listener.follow('full', function(e){
        if (e.type === 'complite' && getBool('interface_mod_new_colored_age', false)) {
          ...
          scheduleAgePasses(root); // 100/300/800/1600 мс
        }
      });
    }
  }
  ...
  // Слухач картки — тільки для «Оригінальна назва»
  ...
  function startPlugin() {
    injectFallbackCss();          // класи для фолбеків (як у вихідному коді)
