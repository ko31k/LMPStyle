...
/* === Glue: settings menu, translations, and visual toggles for torrents+mod === */
(function(){
  try {
    // Translations
    if (Lampa && Lampa.Lang && Lampa.Lang.add) {
      Lampa.Lang.add({
        torr_mod_frame:        { uk:'Рамка торентів за сидерами', ru:'Рамка торрент-блоков по сидерам', en:'Torrent frame by seeders' },
        torr_mod_frame_desc:   { uk:'Підсвічувати блоки торентів кольоровою рамкою залежно від кількості на роздачі', ru:'Подсвечивать блоки торрентов рамкой в зависимости от числа сидов', en:'Outline torrent rows based on seeder count' },
        torr_mod_bitrate:      { uk:'Кольори за бітрейтом', ru:'Цвета по битрейту', en:'Bitrate-based coloring' },
        torr_mod_bitrate_desc: { uk:'Підсвічувати бітрейт: ≤10 — жовтий, 11–40 — зелений, 41+ — червоний', ru:'Подсветка битрейта: ≤10 — жёлтый, 11–40 — зелёный, 41+ — красный', en:'Color bitrate: ≤10 yellow, 11–40 green, 41+ red' },
        torr_mod_seeds:        { uk:'Кольори за кількістю на роздачі', ru:'Цвета по количеству сидов', en:'Seeder count coloring' },
        torr_mod_seeds_desc:   { uk:'Підсвічувати кількість на роздачі: 0–4 — червоний, 5–14 — жовтий, 15+ — зелений', ru:'Подсветка сидов: 0–4 — красный, 5–14 — жёлтый, 15+ — зелёный', en:'Color seeders: 0–4 red, 5–14 yellow, 15+ green' }
      });
    }

    // Helpers
    function getBool(key, def){
      var v = (Lampa && Lampa.Storage) ? Lampa.Storage.get(key) : undefined;
    ...
    // CSS overrides according to toggles
    function applyTorrentToggleOverrides(){
      try{
        var style = document.getElementById('torr_mod_overrides');
        if(!style){
          style = document.createElement('style');
          style.id = 'torr_mod_overrides';
          document.head.appendChild(style);
        }
        var enableFrame   = getBool('interface_mod_new_tor_frame',   true);
        var enableBitrate = getBool('interface_mod_new_tor_bitrate', true);
        var enableSeeds   = getBool('interface_mod_new_tor_seeds',   true);

        var css = '';
        if(!enableBitrate){
          css += '.torrent-item__bitrate span.low-bitrate, .torrent-item__bitrate span.medium-bitrate, .torrent-item__bitrate span.high-bitrate{ color: inherit !important; font-weight: inherit !important; }\\n';
        }
        if(!enableSeeds){
          css += '.torrent-item__seeds span.low-seeds, .torrent-item__seeds span.medium-seeds, .torrent-item__seeds span.high-seeds{ color: inherit !important; font-weight: inherit !important; }\\n';
        }
        if(!enableFrame){
          css += '.torrent-item.low-seeds, .torrent-item.medium-seeds, .torrent-item.high-seeds{ border: none !important; }\\n';
        }
        style.textContent = css;
      }catch(e){}
    }

    // Settings UI wiring — add to existing "Інтерфейс +" component if present
    function addSettings(){
      try{
        var settings = Lampa && Lampa.Settings && Lampa.Settings.main;
        if(!settings || !settings.add) return;
        function add(o){ try{ settings.add(o); }catch(e){} }
        add({ component: 'interface_mod_new',
    ...
    (function(){
      if(!(Lampa && Lampa.Storage && Lampa.Storage.set)) return;
      var _set = Lampa.Storage.set;
      Lampa.Storage.set = function(key, val){
        var r; try{ r = _set.apply(Lampa.Storage, arguments); }catch(e){}
        if (key === 'interface_mod_new_tor_frame' || key === 'interface_mod_new_tor_bitrate' || key === 'interface_mod_new_tor_seeds') {
          applyTorrentToggleOverrides();
        }
        return r;
      };
    })();
    ...
    setTimeout(applyTorrentToggleOverrides, 0);
    // Expose for manual refresh
    window.runTorrentStyleRefresh = function(){ applyTorrentToggleOverrides(); };
  } catch(e){}
})();
