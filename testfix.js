(function () {
  ...
  function initInterfaceModSettingsUI(){
    ...
    function closeOpenSelects(){ requestAnimationFrame(function(){ try{ Lampa.Settings.update(); }catch(e){} }); }
...
applyIconOnlyClass(__ifx_last.fullRoot || $(document));
  ...
  function rebuildButtonsNow(){
    ...
    if (settings.all_buttons) {
        ...
        // Не форсуємо відкат у поточній картці; порядок повернеться після пере-відкриття
    }
