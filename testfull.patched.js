/* IFX plugin runtime guards */
window.__ifx_started = window.__ifx_started || false;
window.__ifx_need_full_settings_update = window.__ifx_need_full_settings_update || false;
window.__ifx_obs = window.__ifx_obs || {};
window.__ifx_move_done = window.__ifx_move_done || false;
window.__ifx_full_binds = window.__ifx_full_binds || {};
(function () {
  'use strict';
  ...
  function initInterfaceModSettingsUI(){
    ...
    var tries=0, t=setInterval(function(){ moveAfterInterface(); if(++tries>=40) clearInterval(t);
    window.__ifx_move_done = true;
    if (window.__ifx_obs.menu) try{ window.__ifx_obs.menu.disconnect(); }catch(e){}
 }, 150);
  ...
  function setupVoteColorsObserver() {
    ...
    if (window.__ifx_full_binds['default']) return; window.__ifx_full_binds['default']=true;
...
// Preserve scroll position to avoid jump-to-top
var box = document.querySelector('.settings__content');
var __ifx_scrollY = box ? box.scrollTop : 0;
// Close open selectboxes only
document.querySelectorAll('.selectbox').forEach(function(el){ try{ el.remove(); }catch(e){} });
if (window.__ifx_need_full_settings_update) {
    window.__ifx_need_full_settings_update = false;
    Lampa.Settings.update();
    setTimeout(function(){
        var box2 = document.querySelector('.settings__content');
        if (box2 && typeof __ifx_scrollY === 'number') box2.scrollTop = __ifx_scrollY;
    }, 0);
}
...
(function observeTorrents(){
  ...
  function startPlugin() {
    if (window.__ifx_started) return; window.__ifx_started = true;
