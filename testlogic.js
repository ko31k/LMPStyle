(function () {
  ...
  function initInterfaceModSettingsUI(){
    ...
    function closeOpenSelects(){ requestAnimationFrame(function(){ try{ Lampa.Settings.update(); }catch(e){} }); }, 60);
    }
...
function reorderAndShowButtons(fullRoot){
  ...
  // Видаляємо play-кнопку, щоб не ховалася група
  fullRoot.find('.button--play, .button--player, .view--play, .view--player').remove();

  // Джерело кнопок: з видимого контейнера картки + із внутрішнього контейнера джерел
  var $all = fullRoot.find('.buttons--container .full-start__button').add($container.find('.full-start__button'));
  ...
  $all.each(function(){
    ...
    // Фільтруємо play
    if (/(^|\s)(button--play|view--play|button--player|view--player)(\s|$)/.test(cls)) return;

    if (cls.indexOf('online') !== -1)      categories.online.push($btn);
    else if (cls.indexOf('torrent') !== -1)categories.torrent.push($btn);
    else if (cls.indexOf('trailer') !== -1)categories.trailer.push($btn);
    else                                    categories.other.push($btn.clone(true));
  });

  // Порядок: Online → Torrents → Trailers → інше
  var order = ['online','torrent','trailer','other'];

  // Перемальовуємо контейнер
  $container.empty();
  order.forEach(function(group){
    categories[group].forEach(function($b){ $container.append($b); });
  });

  // Залишаємо тільки іконки, якщо ввімкнено
  applyIconOnlyClass(fullRoot);

  // Контролер має «переосмислити» активний блок
