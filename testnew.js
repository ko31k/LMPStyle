...
/* ============================================================
 * КОЛЬОРОВІ ВІКОВІ РЕЙТИНГИ (PG) — СПРОЩЕНА, НАДІЙНА ЛОГІКА
 * ============================================================ */
...
// 1) Базові стилі: увімкнено → без бордера; вимкнено → білий бордер
function setAgeBaseCssEnabled(enabled){
  var idEn = 'interface_mod_age_enabled';
  var idDis = 'interface_mod_age_disabled';
  document.getElementById(idEn) && document.getElementById(idEn).remove();
  ...
  if (enabled){
    ...
    st.textContent =
      AGE_BASE_SEL + '{' +
        'font-size:1.2em!important;' +
        'border:0!important;' + // ← ключова зміна: жодних «прозорих рамок»
        'border-radius:0.2em!important;' +
        'padding:0.3em!important;' +
        'margin-right:0.3em!important;' +
        'margin-left:0!important;' +
    ...
    st.textContent =
      AGE_BASE_SEL + '{' +
        'font-size:1.2em!important;' +
        'border:1px solid #fff!important;' + // ← вимкнено: біла рамка
        'border-radius:0.2em!important;' +
        'padding:0.3em!important;' +
        'margin-right:0.3em!important;' +
        'margin-left:0!important;' +
...
// 2) Нормалізація та категоризація
function normalizeAgeText(raw){
  if (!raw) return '';
  var t = String(raw).toUpperCase();

  // прибираємо HTML, уніфікуємо тире/плюси/пробіли
  t = t.replace(/<[^>]*>/g,' ');
  t = t.replace(/[\u2010-\u2015\u2212]/g,'-');  // будь-які «довгі тире» → '-'
  t = t.replace(/[＋﹢+]/g,'+');                // екзотичні плюси → '+'
  t = t.replace(/\s+/g,' ').trim();

  // часті варіації
  t = t.replace(/\bTV\s*Y7\b/g,'TV-Y7')
       .replace(/\bTV\s*Y\b/g,'TV-Y')
       .replace(/\bTV\s*G\b/g,'TV-G')
       .replace(/\bTV\s*14\b/g,'TV-14')
       .replace(/\bTV\s*MA\b/g,'TV-MA')
       .replace(/\bPG\s*13\b/g,'PG-13');

  // спочатку — точні коди
  var m;
  if ((m = t.match(/\b(TV-(?:Y7|Y|G|14|MA))\b/))) return m[1];
  if ((m = t.match(/\b(NC-17|PG-13|PG|R|G|X)\b/))) return m[1];

  // потім — числові віки (12, 12+, 16+, тощо)
  if ((m = t.match(/(^|\D)(\d{1,2})\s*\+?(\D|$)/))) return m[2] + '+';
  ...
  __ageObserver = new MutationObserver(function(muts){
    ...
    muts.forEach(function(m){
      ...
      if (m.type === 'attributes' && m.target && m.target.nodeType === 1){
        var t = m.target;
        if (t.matches && t.matches(AGE_BASE_SEL)) applyAgeOnceIn(t);
      }
      if (m.type === 'characterData' && m.target && m.target.parentNode){
        var p = m.target.parentNode;
        if (p.matches && p.matches(AGE_BASE_SEL)) applyAgeOnceIn(p);
      }
    });
  });
  ...
  if (!__ageFollowReady){
    ...
    Lampa.Listener.follow('full', function(e){
      if (e.type === 'complite' && getBool('interface_mod_new_colored_age', false)) {
        ...
        [120, 300, 800, 1600].forEach(function(ms){
