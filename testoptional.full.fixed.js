(function () {
  ...
  Lampa.Lang.add({
    ...
    interface_mod_new_colored_ratings_desc: {
      en: 'Enable colored rating highlight in the full card',
    ...
    interface_mod_new_colored_status_desc: {
      en: 'Highlight movie/series status in the full card',
    ...
    interface_mod_new_colored_age_desc: {
      en: 'Highlight age rating in the full card',
    ...
    interface_mod_new_all_buttons_desc: {
      en: 'Show all buttons in the card (Reload required)',
    ...
    interface_mod_new_colored_buttons_desc: {
      en: 'Update icons and colors for online, torrents, trailers buttons',
    ...
    torr_mod_frame: {
      ...
      en: 'Colored torrent frame'
    },
    torr_mod_frame_desc: {
      uk: 'Підсвічувати блоки торентів кольоровою рамкою залежно від кількості сідерів',
      en: 'Highlight torrent blocks with a colored frame based on seeder count'
    },
    torr_mod_bitrate: {
...
0–4 — red, 5–14 — yellow, 15+ — green'
  ...
  function setupVoteColorsObserver() {
    // Initial paint shortly after start (kept for first screen)
    setTimeout(function () {
    ...
    var obs = new MutationObserver(function (mutations) {
      ...
      for (var k = 0; k < mutations.length; k++) {
        ...
        for (var j = 0; j < m.addedNodes.length; j++) {
          ...
          if (n.matches && n.matches(SEL)) elementsToPaint.push(n);
          if (n.querySelectorAll) {
            var q = n.querySelectorAll(SEL);
            if (q && q.length) for (var t = 0; t < q.length; t++) elementsToPaint.push(q[t]);
          }
        }
      }
      if (elementsToPaint.length) {
        requestAnimationFrame(function () {
          for (var i = 0; i < elementsToPaint.length; i++) paint(elementsToPaint[i]);
        });
      }
    });
    try {
...
function injectAll($scope, isScoped){
  ...
  $episodes.each(function(){
    var $ep = $(this);
    if (!S || !S.year_on){
      $ep.find('.full-episode').removeClass('ifx-hide-age');
      $ep.find('.ifx-year-pill').remove();
    } else {
      applyEpisodeCard($ep);
    }
  });
  ...
  function enableObserver(){
    ...
    mo = new MutationObserver(function(muts){
      ...
      for (var i=0;i<muts.length;i++){
        ...
        for (var j=0; j<m.addedNodes.length; j++){
          var n = m.addedNodes[j];
          if (n && n.nodeType === 1) nodesToProcess.push(n);
        }
      }
      if (hasNew){
