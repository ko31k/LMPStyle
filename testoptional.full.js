(function () {
  ...
  function setupVoteColorsObserver() {
    // Initial paint for first screen
    setTimeout(function () {
    ...
    // Keep manual full repaint for explicit calls or on open
    // (updateVoteColors is defined below)
    var obs = new MutationObserver(function (muts) {
      ...
      for (var i = 0; i < muts.length; i++) {
        ...
        m.addedNodes.forEach ? m.addedNodes.forEach(function(n){
          if (!n || n.nodeType !== 1) return;
          if (n.matches && n.matches(SEL)) toPaint.push(n);
          var q = n.querySelectorAll ? n.querySelectorAll(SEL) : null;
          if (q && q.length) q.forEach ? q.forEach(function(e){ toPaint.push(e); }) : toPaint.push.apply(toPaint, Array.prototype.slice.call(q));
        }) : (function(){
          for (var j=0;j<m.addedNodes.length;j++){
            ...
            if (n.matches && n.matches(SEL)) toPaint.push(n);
            var q = n.querySelectorAll ? n.querySelectorAll(SEL) : null;
            if (q && q.length) toPaint.push.apply(toPaint, Array.prototype.slice.call(q));
          }
        })();
      }
      if (toPaint.length) {
        requestAnimationFrame(function () {
          toPaint.forEach(paint);
        });
      }
    });
    ...
    Lampa.Listener.follow('full', function (e) {
      if (e.type === 'complite' && getBool('interface_mod_new_colored_ratings', false)) {
        setTimeout(updateVoteColors, 80);
      }
    });
  }
    });
...
function ensureAltBadgesCss(){
  ...
  function applyTitleYearHide($scope){
    ...
    var $targets = $scope.filter(sel).add($scope.find(sel));
...
function injectAll($scope, isScoped){
  $scope = ($scope && $scope.length) ? $scope : $(document.body);

  // 1) Списки тайтлів
  var $cards = isScoped
    ? $scope.filter('.card').add($scope.find('.card'))
    : $scope.find('.card');

  $cards.each(function(){
    var $c = $(this);
    if ($c.closest('.full-start, .full-start-new, .full, .details').length) return;
    if (typeof applyListCard === 'function') applyListCard($c);
  });

  // 2) Картки епізодів
  var $episodes = isScoped
    ? $scope.filter('.card-episode').add($scope.find('.card-episode'))
    : $scope.find('.card-episode');
  ...
  $episodes.each(function(){
    ...
    } else if (typeof applyEpisodeCard === 'function'){
      applyEpisodeCard($ep);
    }
  });

  // 3) Інше (працюємо в межах скоупа)
  applyNumberOnly(isScoped ? $episodes : $scope);
  applyTitleYearHide(isScoped ? $cards : $scope);
}
  });
  ...
  var mo = null;
  var moDebounce = null;
  var nodesToProcess = []; // OPTIMIZATION: Debounce timer
    ...
    mo = new MutationObserver(function(muts){
      ...
      if (hasNewNodes){
        ...
        moDebounce = setTimeout(function(){
          var $scope = $(nodesToProcess);
          nodesToProcess = [];
          injectAll($scope, true);
        }, 200);
      }
    });
...
// ===== Merged 'full' listener (non-invasive) =====
try{
  Lampa.Listener.follow('full', function (e) {
    if (e.type !== 'complite') return;

    // new info panel (if exists)
    setTimeout(function(){
      try{
        if (typeof buildInfoPanel === 'function'){
          var details = $('.full-start-new__details');
          if (!details.length) details = $('.full-start__details');
          if (details.length){
            var movie = (e.data && e.data.movie) || {};
            var isTvShow = !!(movie && (movie.number_of_seasons > 0 || (movie.seasons && movie.seasons.length) || movie.type === 'tv' || movie.type === 'serial'));
            buildInfoPanel(details, movie, isTvShow, details.html());
          }
        }
      }catch(_){}
    }, 100);
    ...
    if (getBool('interface_mod_new_colored_ratings', false)){
      setTimeout(function(){ try{ updateVoteColors(); }catch(_){}} , 100);
    }

    // colored statuses
    if (getBool('interface_mod_new_colored_status', false) && typeof applyStatusOnceIn === 'function'){
      setTimeout(function(){ try{ applyStatusOnceIn(e.object.activity.render()); }catch(_){}} , 120);
    }

    // optional: torrent style refresh
    setTimeout(function(){ try{ if (typeof runTorrentStyleRefresh === 'function') runTorrentStyleRefresh(); }catch(_){}} , 140);
  });
}catch(_){}

// ===== Localization overrides (EN aligned with UK) =====
try{
  Lampa.Lang.add({
    ...
    interface_mod_new_info_panel_desc: { en: 'Colored and rephrased info line', uk: 'Кольорова та перефразована інформаційна панель' },

    interface_mod_new_colored_ratings: { en: 'Colored rating', uk: 'Кольоровий рейтинг' },
    interface_mod_new_colored_ratings_desc: { en: 'Enable colored rating highlight in the full card', uk: 'Увімкнути кольорове виділення рейтингу в повній картці' },

    interface_mod_new_colored_status: { en: 'Colored statuses', uk: 'Кольорові статуси' },
    interface_mod_new_colored_status_desc: { en: 'Highlight movie/series status in the full card', uk: 'Підсвічувати статус фільму/серіалу в повній картці' },

    interface_mod_new_colored_age: { en: 'Colored age rating', uk: 'Кольоровий віковий рейтинг ' },
    interface_mod_new_colored_age_desc: { en: 'Highlight age rating in the full card', uk: 'Підсвічувати віковий рейтинг в повній картці' },
    ...
    interface_mod_new_en_data_desc: { en: 'Show original title in the card header', uk: 'Показувати оригінальну назву в заголовку картки' },

    interface_mod_new_all_buttons: { en: 'All buttons in card', uk: 'Всі кнопки в картці' },
    interface_mod_new_all_buttons_desc: { en: 'Show all buttons in the card (Reload required)', uk: 'Показує всі кнопки у картці (Потрібне перезавантаження)' },

    interface_mod_new_icon_only: { en: 'Icons only', uk: 'Кнопки без тексту' },
    interface_mod_new_icon_only_desc: { en: 'Hide button labels, keep only icons', uk: 'Ховає підписи на кнопках, лишає тільки іконки' },

    interface_mod_new_colored_buttons: { en: 'Colored buttons', uk: 'Кольорові кнопки' },
    interface_mod_new_colored_buttons_desc: { en: 'Update icons and colors for online, torrents, trailers buttons', uk: 'Оновлює іконки та кольори кнопок онлайн, торенти, трейлери' },

    torr_mod_frame: { en: 'Colored torrent frame', uk: 'Кольорова рамка блоку торентів' },
    torr_mod_frame_desc: { en: 'Highlight torrent blocks with a colored frame based on seeder count', uk: 'Підсвічувати блоки торентів кольоровою рамкою залежно від кількості сідерів' },

    torr_mod_bitrate: { en: 'Colored bitrate', uk: 'Кольоровий бітрейт' },
    torr_mod_bitrate_desc: { en: 'Colorize bitrate based on its value', uk: 'Підсвічувати бітрейт кольором в залежності від розміру' },

    torr_mod_seeds: { en: 'Colored seeder count', uk: 'Кольорова кількість роздаючих' },
    torr_mod_seeds_desc: { en: 'Highlight seeder count: \n0–4 — red, 5–14 — yellow, 15+ — green', uk: 'Підсвічувати кількість сідерів на роздачі: \n0–4 — червоний, 5–14 — жовтий, 15 і вище — зелений' }
  });
}catch(_){}
