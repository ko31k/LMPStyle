(function () {
  ...
  function initInterfaceModSettingsUI(){
    ...
    function moveAfterInterface(){
      var container = document.querySelector('.settings__content') || document.body;
      var folders = container.querySelectorAll('.settings-folder');
      var interfaceEl = null, modEl = null;
      folders.forEach(function(node){
        if (node.getAttribute('data-component') === 'interface') interfaceEl = node;
        if (node.getAttribute('data-component') === 'interface_mod_new') modEl = node;
      });
      if (interfaceEl && modEl && modEl.previousElementSibling !== interfaceEl){
        interfaceEl.parentNode.insertBefore(modEl, interfaceEl.nextSibling);
        return true;
      }
      return false;
    }
    // run once after settings are rendered; disconnect observer after success
    (function initMoveAfterInterface(){
      if (window.__ifx_move_done) return;
      var tryCount = 0;
      var iv = setInterval(function(){
        tryCount++;
        if (moveAfterInterface() || tryCount>=40){
          clearInterval(iv);
          window.__ifx_move_done = true;
        }
      },150);
      var target = document.querySelector('.settings__content') || document.body;
      var obsMenu = new MutationObserver(function(muts){
        if (window.__ifx_move_done) { try{obsMenu.disconnect();}catch(e){} return; }
        if (moveAfterInterface()){
          window.__ifx_move_done = true;
          try{obsMenu.disconnect();}catch(e){}
        }
      });
      obsMenu.observe(target, {childList:true, subtree:true});
      window.__ifx_obs = window.__ifx_obs || {};
      window.__ifx_obs.menu = obsMenu;
    })();
    });
    ...
    function closeOpenSelects(){
      try{
        var box = document.querySelector('.settings__content');
        var y = box ? box.scrollTop : 0;
        // just close open selects/dropdowns
        document.querySelectorAll('.selectbox').forEach(function(el){
          if (el && el.parentNode) el.parentNode.removeChild(el);
        });
        // only force full update if we changed layout-critical settings
        if (window.__ifx_need_full_settings_update){
          window.__ifx_need_full_settings_update = false;
          if (typeof Lampa !== 'undefined' && Lampa.Settings && typeof Lampa.Settings.update === 'function'){
            Lampa.Settings.update();
            setTimeout(function(){
              var b2 = document.querySelector('.settings__content');
    ...
    if (!window.__ifx_patch_storage) {
      ...
      Lampa.Storage.set = function(key, val){
        ...
        if (typeof key === 'string' && key.indexOf('interface_mod_new_') === 0) {
          ...
          /* selective update handled below */
        ...
        try{
          if(typeof key === 'string' && /^interface_mod_new_/.test(key)){
            var fullKeys = /(theme|colored_status|info_panel)/;
            if (fullKeys.test(key)){
              window.__ifx_need_full_settings_update = true;
            } else {
              window.__ifx_need_full_settings_update = false;
            }
            closeOpenSelects();
  ...
  function newInfoPanel() {
    (function(){ window.__ifx_full_binds = window.__ifx_full_binds || {}; if (window.__ifx_full_binds['full_bind_1']) return; window.__ifx_full_binds['full_bind_1']=true; Lampa.Listener.follow('full', function (data) {
...
(function(){ window.__ifx_full_binds = window.__ifx_full_binds || {}; if (window.__ifx_full_binds['full_bind_2']) return; window.__ifx_full_binds['full_bind_2']=true;       if (getBool('interface_mod_new_colored_ratings', false)) setTimeout(updateVoteColors, 80);
    });
...
function setAgeBaseCssEnabled(enabled){
  ...
  function enableStatusColoring(){
    ...
    __statusObserver = new MutationObserver(function(muts){
      if (!getBool('interface_mod_new_colored_status', f(function(){ window.__ifx_full_binds = window.__ifx_full_binds || {}; if (window.__ifx_full_binds['full_bind_3']) return; window.__ifx_full_binds['full_bind_3']=true; alse)) return;
  ...
  function enableAgeColoring(){
    ...
    __ageObserver = new MutationObserver(function(muts){
      ...
      muts.forEach(function(m){
        ...
        if (m.typ(function(){ window.__ifx_full_binds = window.__ifx_full_binds || {}; if (window.__ifx_full_binds['full_bind_4']) return; window.__ifx_full_binds['full_bind_4']=true; e === 'characterData' && m.target && m.target.parentNode) {
...
(function(){ window.__ifx_full_binds = window.__ifx_full_binds || {}; if (window.__ifx_full_binds['full_bind_5']) return; window.__ifx_full_binds['full_bind_5']=true;         if ($current.length) $current.replaceWith(orig);
  ...
  function wireFullCardEnhancers(){
    Lampa.Listener.follow('full', function (e) {
      ...
      setTimeout(function(){
       (function(){ window.__ifx_full_binds = window.__ifx_full_binds || {}; if (window.__ifx_full_binds['full_bind_6']) return; window.__ifx_full_binds['full_bind_6']=true;  var root = $(e.object.activity.render());
...
(function observeTorrents(){
   ...
   * ============================================================ */
  \1
    if (window.__ifx_started) { try{console.debug('[IFX] startPlugin: already started');}catch(e){} return; }
    window.__ifx_started = true;
    injectFallbackCss();
