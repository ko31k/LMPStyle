...
function reorderAndShowButtons(fullRoot){
  ...
  // Target container where we show buttons in the full card
  var $target = fullRoot.find('.full-start-new__buttons, .full-start__buttons').first();
  if (!$target.length) return;

  // Remove Play so sources are not hidden behind it
  fullRoot.find('.button--play').remove();

  // Collect buttons from hidden container and current container (as in Enchanser)
  var $all = fullRoot.find('.buttons--container .full-start__button')
  ...
  $all.each(function(){
    var $btn = $(this);
    var className = ($btn.attr('class')||'').toLowerCase();
    ...
    if (className.indexOf('online') !== -1) {
      ...
      // clone for "other" to avoid breaking default handlers (Favorites/Bell/More)
      categories.other.push($btn.clone(true));
    }
  });

  // Button order same as Enchanser
  var order = Lampa.Storage.get('lme_buttonsort') || ['torrent','online','trailer','other'];

  // Clear target and append in the specified order
  $target.empty();
  order.forEach(function(cat){
    (categories[cat] || []).forEach(function($b){ $target.append($b); });
  });

  // Keep our existing "icons only" / colors & SVG logic untouched
  applyIconOnlyClass(fullRoot);

  // Nudge controller like Enchanser to ensure focus is recalculated
  setTimeout(function(){
  ...
  if (settings.all_buttons) {
    ...
    // No forced restore in current card; will reset on next open
  }
